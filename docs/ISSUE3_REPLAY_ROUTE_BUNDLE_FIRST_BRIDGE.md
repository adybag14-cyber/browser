...
- replay-route already open and explicit bundle paths already pinned: go from the replay-route helper to the replay-route shortcut, reopen `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_SUITE_SURFACE.md` and `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_QUICKSTART.md`, then use the compact bundle-suite surface helper, the bundle-first helper, the bundle flow helper, the delegated runner, and the fixed-list proof path
