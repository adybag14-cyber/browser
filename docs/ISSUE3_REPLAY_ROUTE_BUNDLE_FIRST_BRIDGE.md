...
Use that route when you want the replay-route helper, the narrower replay-route shortcut, the broader attached-page localhost flow, the dedicated Google-shaped attached-page flow, the bundle-first helper, the compact bundle quickstart note, the pinned bundle flow helper, the delegated localhost runner, and the fixed-list screenshot-and-title proof kept on one compact branch.
...
Use that route when the broader validation catalog or replay-route family is still choosing the next branch and you want the broader attached-page helper chain, the Google-shaped attached-page helper chain, and the pinned bundle branch visible together before the delegated bundle runner takes over.
...
- `show_google_issue3_attached_bundle_first_entrypoint.ps1`: use when the replay should stay on the pinned three-page compatibility set before reopening the broader helper chain. Keep `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_QUICKSTART.md` nearby when you want the compact read-first note beside that helper before the delegated bundle runner takes over.
...
- replay-route already open and explicit bundle paths already pinned: go from the replay-route helper to the replay-route shortcut, reopen `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_QUICKSTART.md`, then use the bundle-first helper, the bundle flow helper, the delegated runner, and the fixed-list proof path
- replay-route already open but broader attached-page context still matters: reopen `show_attached_html_validation_flow.ps1` and `show_google_attached_html_validation_flow.ps1` before the bundle-first helper so the broader attached-page recovery path stays visible
- broader validation catalog still choosing the next branch: reopen `show_headed_validation_suites.ps1 -ChangeArea attached-html`, `-ChangeArea google-attached-html`, and `-ChangeArea attached-html-target-bundle` before the replay-route shortcut so the broader router and the pinned bundle branch stay aligned
