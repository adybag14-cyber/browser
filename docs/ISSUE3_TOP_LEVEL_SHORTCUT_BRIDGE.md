...
Use this note when issue `#3` work is re-entering from the top-level headed validation suite router and you want the shortest shortcut-first helper chain without losing the newer attached-page replay route, the dedicated Google-shaped attached-page flow, or the issue-specific Google attached-page bridge.
...
Use `show_google_issue3_windows_full_use_attached_html_route.ps1`, rerun `check_google_issue3_windows_full_use_attached_html_route_validation_surface.ps1`, reopen `show_google_issue3_windows_full_use_validation_router_attached_html_bridge.ps1`, and keep `show_google_issue3_windows_full_use_attached_html_catalog_quickstart.ps1` nearby when the replay was reopened from `docs/WINDOWS_FULL_USE.md` and the broader runbook-side attached-page branch should stay visible before the top-level shortcut helper narrows the route again.

Use `show_google_issue3_validation_router_attached_html_quickstart.ps1` first when the broader attached-page router still matters more than the replay shortcuts and you want the validation-router quickstart kept visible before the top-level shortcut helper takes over.

Use `show_google_attached_html_validation_flow.ps1` and `show_google_issue3_google_attached_html_entrypoint.ps1` first when the current attached inputs are already Google-shaped and you want the dedicated Google-style attached-page guide plus the narrower issue-specific Google bridge kept visible before the shortcut-first helper takes over.

Use `show_google_issue3_top_level_attached_html_quickstart.ps1`, `show_google_issue3_top_level_attached_html_entrypoint.ps1`, `show_google_issue3_top_level_attached_html_catalog_quickstart.ps1`, `show_google_issue3_suite_catalog_top_level_attached_html_catalog_quickstart.ps1`, `docs/ISSUE3_SUITE_CATALOG_ENTRYPOINTS.md`, `show_google_issue3_suite_router_attached_html_quickstart.ps1`, `docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_COMPANION_NOTES.md`, or `docs/ISSUE3_SUITE_CATALOG_ATTACHED_HTML_BRIDGE.md` first when the shorter top-level attached-page bridge, the suite-catalog ladder, the suite-router-side attached-page bridge, and the suite-catalog-side note map should stay visible beside the shortcut-first helper before the route narrows again.

## Replay-side attached-page re-entry

If `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already narrowed the replay to the attached localhost HTML lane and you want the replay-side attached-page ladder kept visible before this top-level shortcut bridge takes over, start with:
...
Use that route when the replay-side attached-page quickstart already made the narrower lane obvious and you want that replay-side ladder, the dedicated Google-shaped attached-page flow, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the broader top-level attached-page bridge, the catalog-side bridge, the suite-catalog-to-top-level attached-html catalog quickstart, the wider suite-catalog guide, the suite-catalog attached-page bridge, the suite-router attached-page bridge, and the top-level shortcut helper all pointing at the same re-entry order before the route narrows again.
...
- Attached localhost HTML route already in focus and the replay was reopened from `docs/WINDOWS_FULL_USE.md`: go from `-ChangeArea attached-html` to `show_google_issue3_windows_full_use_attached_html_route.ps1`, rerun `check_google_issue3_windows_full_use_attached_html_route_validation_surface.ps1`, reopen `show_google_issue3_windows_full_use_validation_router_attached_html_bridge.ps1`, keep `show_google_issue3_windows_full_use_attached_html_catalog_quickstart.ps1` nearby, then narrow through `show_google_issue3_validation_router_attached_html_quickstart.ps1`, `show_google_attached_html_validation_flow.ps1`, `show_google_issue3_google_attached_html_entrypoint.ps1`, `show_google_issue3_top_level_attached_html_quickstart.ps1`, `show_google_issue3_top_level_attached_html_entrypoint.ps1`, `show_google_issue3_top_level_attached_html_catalog_quickstart.ps1`, `show_google_issue3_suite_catalog_top_level_attached_html_catalog_quickstart.ps1`, `show_google_issue3_suite_router_attached_html_quickstart.ps1`, the suite-catalog guide, the suite-catalog attached-html bridge, and only then drop to the top-level shortcut helper.
- Attached localhost HTML route already in focus and the replay was reopened from `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md`: go from `show_google_issue3_windows_replay_attached_html_quickstart.ps1` to `show_google_attached_html_validation_flow.ps1`, then `show_google_issue3_google_attached_html_entrypoint.ps1`, then the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the top-level attached-page catalog quickstart, then the suite-catalog top-level attached-html catalog quickstart, then the suite-catalog guide, then the suite-catalog attached-page bridge, then the suite-router attached-page quickstart, and only then drop to the top-level shortcut helper.
- Attached localhost HTML route already in focus but the broader validation-router bridge matters more than the Windows runbook route: go from `-ChangeArea attached-html` to `show_google_issue3_validation_router_attached_html_quickstart.ps1`, then `show_google_attached_html_validation_flow.ps1`, then `show_google_issue3_google_attached_html_entrypoint.ps1`, then the smaller top-level attached-page quickstart, entrypoint, catalog quickstart, suite-catalog top-level attached-html catalog quickstart, suite-router attached-page quickstart, or companion notes, then the top-level shortcut helper, then the attached-page bridge that best matches the current state.
- Saved summary or repo-root override already present: use the broader attached-page helpers plus the top-level shortcut helper with that same context first, then choose contextual flow, replay-route shortcut, replay shortcuts, the suite-router attached-page quickstart, the suite-catalog top-level attached-html catalog quickstart, or the suite-catalog attached-page bridge only as needed.
