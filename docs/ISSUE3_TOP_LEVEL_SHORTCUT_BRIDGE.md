...
Use that route when the replay-side attached-page quickstart already made the narrower lane obvious and you want that replay-side ladder, the top-level attached-page quickstart, the broader top-level attached-page bridge, the catalog-side bridge, the suite-catalog-to-top-level attached-html catalog quickstart, the wider suite-catalog guide, the suite-catalog attached-page bridge, the suite-router attached-page bridge, and the top-level shortcut helper all pointing at the same re-entry order before the route narrows again.
...
- Attached localhost HTML route already in focus and the replay was reopened from `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md`: go from `show_google_issue3_windows_replay_attached_html_quickstart.ps1` to the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the top-level attached-page catalog quickstart, then the suite-catalog top-level attached-html catalog quickstart, then the suite-catalog guide, then the suite-catalog attached-page bridge, then the suite-router attached-page quickstart, and only then drop to the top-level shortcut helper.
