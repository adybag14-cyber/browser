...
Use this bridge when you want the top-level router commands, the attached-page compatibility branch, and the narrower issue `#3` shortcut helpers kept aligned in one small note before you widen back into the next-step matrix, replay-route helper, bundle-first branch, or safe-route wrappers.
...
Use the attached HTML shortcut first when the attached-page compatibility branch itself should stay visible before you widen back into the matrix, replay-route, or safe-route helpers.
...
Use this when you want the shortest bridge from the top-level shortcut surface into the narrower replay-shortcuts path.
...
Use this when the attached-page compatibility branch should stay visible before you widen back into replay shortcuts, the next-step matrix, or the safe-route map.
...
- no pinned bundle inputs and no saved replay state yet: go from the top-level suite router to the top-level shortcut helper, then the suite-router shortcut helper, then replay shortcuts, then the safe-route map
- attached localhost HTML route already in focus: go from `-ChangeArea attached-html` to the top-level shortcut helper, then the suite-catalog attached-page bridge or attached-page shortcut before widening again
- saved summary or repo-root override already present: use the top-level shortcut helper with that same context first, then choose contextual flow, replay shortcuts, or the suite-catalog attached-page bridge only as needed
