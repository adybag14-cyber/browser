...
Keep `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_REFERENCE.md` nearby when you use that bundle-first route so the exact three-page compatibility set stays visible before the bundle-first helper, the bundle flow helper, and the delegated localhost runner take over.
...
- explicit bundle paths already pinned: reopen `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_REFERENCE.md` first, then stay on the bundle-first helper before widening back into the broader Google-only path
