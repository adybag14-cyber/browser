...
- replay reopened from `docs/WINDOWS_FULL_USE.md`: go from `show_google_issue3_windows_full_use_attached_html_route.ps1` to the top-level attached-page quickstart, then the top-level attached-page bridge, then the suite-router attached-page quickstart, then the suite-catalog attached-page bridge, or the attached-page shortcut before widening again
...
- broader router already surfaced the attached localhost branch and you want the generic attached-page route printed too: go from `-ChangeArea attached-html` to the top-level attached-page quickstart, then the top-level attached-page bridge, then the suite-router attached-page quickstart, then replay shortcuts, then the safe-route map
