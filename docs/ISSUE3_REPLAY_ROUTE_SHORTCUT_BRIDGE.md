...
Use this note when issue `#3` work is already inside the replay-route helper and the next replay should move quickly into the shorter attached-page, replay-shortcuts, next-step-matrix, bundle-first, or safe-route follow-up without reopening the broader route notes first.
...
If attached localhost follow-up is already obvious from the broader Windows-first route, rerun the route-level surface check first, reopen the Windows full-use validation-router bridge, reopen the Windows-side catalog quickstart, reopen the replay-side attached-html quickstart, then keep the validation-router attached-page quickstart, the attached-html change-area quickstart, the compact top-level attached-page quickstart, the broader top-level attached-page bridge, the top-level attached-page catalog quickstart, the top-level shortcut-first helper, the suite-catalog-to-top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the suite-router shortcut-first helper, the suite-router next-step matrix, and the context-preserving helper in view before dropping back into the replay-route shortcut.
...
Use this route when the replay-route helper is already open and you want the narrowest stable bridge back into the attached-page shortcut, the suite-router shortcut-first helper, the replay-shortcuts surface, and the return-to-safe-route helpers.

If the replay-route helper is already open but you still want the compact start-point matrix or the context-preserving helper reprinted before the route collapses further, reopen either of these between the replay-route shortcut helper and the attached-page shortcut or replay-shortcuts surface:
...
Use this when the broader attached-page bridges are already out of the way and you want the compact issue `#3` shortcut surface reprinted before replay shortcuts, the next-step matrix, the bundle-first helper, or the safe-route map.
