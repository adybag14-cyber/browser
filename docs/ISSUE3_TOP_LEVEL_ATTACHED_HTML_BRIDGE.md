...
Keep `docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md` nearby when the replay is being reopened from `docs/WINDOWS_FULL_USE.md` first and you want that broader Windows runbook entrypoint to stay aligned with this narrower top-level attached-page bridge.
...
Use that route when the attached-page compatibility lane is already clear and you want the compact top-level attached-page quickstart, the broader top-level attached-page bridge, and the suite-router-side quickstart all visible before the route narrows back into the shorter issue `#3` replay surfaces.
...
Use that bundle-first route when the known three-page compatibility set should stay pinned all the way through the compact top-level quickstart, the broader top-level bridge, the suite-router quickstart, the helper branch, the flow helper, and the delegated localhost runner before widening back into the broader issue `#3` helper stack.
...
Use this when the route is already narrowed to attached localhost HTML from the top-level suite router and you want the shortest compact helper that still keeps this broader bridge and the suite-router attached-page quickstart visible before the route narrows again.
...
Use this when you want the broader top-level attached-page bridge kept visible before it drops into the suite-router attached-page quickstart, the attached-page shortcut, replay-route shortcut, replay shortcuts, or the safe-route map.
...
4. Suite-router attached HTML quickstart
...
Once `show_headed_validation_suites.ps1` has already narrowed the route to attached localhost HTML follow-up, prefer `show_google_issue3_top_level_attached_html_quickstart.ps1` first, then `show_google_issue3_top_level_attached_html_entrypoint.ps1`, then `show_google_issue3_suite_router_attached_html_quickstart.ps1` before reopening the longer validation-chain notes again.

- no pinned bundle inputs and no saved replay state yet: go from `-ChangeArea attached-html` to the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the suite-router attached-page quickstart, then the attached-page shortcut, then the replay-route shortcut, then replay shortcuts, then the safe-route map
- Google-specific attached-page surface still matters more than the generic shortcut chain: go from `-ChangeArea google-attached-html` to the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the Google attached-page entrypoint, then the suite-router attached-page quickstart before narrowing further
- explicit bundle paths already pinned: stay on the bundle-first helper after the top-level attached-page quickstart, the broader top-level bridge, and the suite-router attached-page quickstart before widening back into the broader Google-only path
- saved summary or repo-root override already present: reopen the top-level attached-page quickstart and the broader top-level attached-page bridge with that same context first, then choose the suite-router attached-page quickstart, replay shortcuts, or the safe-route map only as needed
