...
attached-page flow helper, top-level shortcut surface, compact top-level
quickstart, top-level attached-page catalog quickstart,
suite-catalog-to-top-level attached-page catalog quickstart, broader top-level
bridge, and suite-router-side attached-page quickstart describing the same
re-entry order.
...
quickstart, broader top-level bridge, suite-router-side attached-page
quickstart, issue-specific attached-page bridge, and suite-catalog guide
pointing at the same narrower helper order.
...
- Google-specific attached-page surface still matters more than the generic shortcut chain: reopen `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md`, then go from `-ChangeArea google-attached-html` to the top-level quickstart, top-level attached-page catalog quickstart, suite-catalog-to-top-level attached-page catalog quickstart, broader top-level bridge, suite-router attached-page quickstart, and then the issue-specific attached-page bridge before narrowing further
