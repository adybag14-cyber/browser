...
Start from the top-level `attached-html`, `google-attached-html`, or `attached-html-target-bundle` router surfaces, then move through `show_google_issue3_top_level_attached_html_quickstart.ps1` and `show_google_issue3_top_level_attached_html_entrypoint.ps1` when the next replay is already known to stay on the attached-page follow-up path.

If the route is reopening from the broader Windows full-use branch or from the broader validation-router attached-html bridge first, keep those broader notes visible long enough to rerun the fail-fast surface check and confirm the newer attached-page handoff is still aligned before this narrower top-level bridge takes over.
...
## Top-level quickstart surfacing

Because the attached localhost route now has both a compact top-level quickstart helper and a narrower suite-router attached-page quickstart, reopen those two surfaces before the shorter replay helpers when the main validation catalog has already made attached localhost follow-up obvious.
...
Keep `docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_CATALOG_QUICKSTART.md` and `docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_COMPANION_NOTES.md` nearby when the replay should preserve the compact top-level catalog-side bridge and the shorter written nearby-note map before it collapses back into replay shortcuts.
...
Use that route when the broader Windows runbook already made attached localhost follow-up obvious and you want the route-level surface check, the validation-router attached-html bridge, the Windows-first catalog quickstart, and the narrower top-level note family all describing the same re-entry order.
...
Use that route when the attached-page compatibility lane is already clear and you want the compact top-level attached-page quickstart, the broader top-level attached-page bridge, the catalog-side bridge, and the suite-router-side quickstart all visible before the route narrows back into the shorter issue `#3` replay surfaces.
...
Use that bundle-first route when the known three-page compatibility set should stay pinned all the way through the compact top-level quickstart, the broader top-level bridge, the catalog-side bridge, the suite-router quickstart, the helper branch, the flow helper, and the delegated localhost runner before widening back into the broader issue `#3` helper stack.

## Preserve replay context

If the replay already carries a non-default repo root, a saved summary, or pinned bundle paths, keep that same context attached to the compact top-level quickstart and this broader top-level attached-page helper first:
...
1. Top-level attached HTML quickstart
...
Once `show_headed_validation_suites.ps1` has already narrowed the route to attached localhost HTML follow-up, prefer `show_google_issue3_top_level_attached_html_quickstart.ps1` first, then `show_google_issue3_top_level_attached_html_entrypoint.ps1`, then `show_google_issue3_top_level_attached_html_catalog_quickstart.ps1`, then `show_google_issue3_suite_router_attached_html_quickstart.ps1` before reopening the longer validation-chain notes again.

- replay reopened from `docs/WINDOWS_FULL_USE.md`: rerun `check_google_issue3_windows_full_use_attached_html_route_validation_surface.ps1`, then reopen the Windows full-use validation-router bridge, then the Windows full-use attached-html catalog quickstart, then the top-level attached-page quickstart and this broader bridge before the route narrows again
- no pinned bundle inputs and no saved replay state yet: go from `-ChangeArea attached-html` to the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the top-level attached-page catalog quickstart, then the suite-router attached-page quickstart, then the attached-page shortcut, then the replay-route shortcut, then replay shortcuts, then the safe-route map
- Google-specific attached-page surface still matters more than the generic shortcut chain: go from `-ChangeArea google-attached-html` to the top-level attached-page quickstart, then the broader top-level attached-page bridge, then the Google attached-page entrypoint, then the top-level attached-page catalog quickstart, then the suite-router attached-page quickstart before narrowing further
- explicit bundle paths already pinned: stay on the bundle-first helper after the top-level attached-page quickstart, the broader top-level bridge, the top-level attached-page catalog quickstart, and the suite-router attached-page quickstart before widening back into the broader Google-only path
- saved summary or repo-root override already present: reopen the top-level attached-page quickstart and the broader top-level attached-page bridge with that same context first, then choose the top-level attached-page catalog quickstart, replay shortcuts, or the safe-route map only as needed
