# Issue #3 Suite-Router Attached Bundle Bridge

Use this note when issue `#3` replay is already re-entering from `show_google_issue3_suite_router_attached_html_quickstart.ps1` and the next useful branch should stay pinned to the known three-page attached HTML compatibility bundle before replay widens back into the broader shortcut or safe-route helpers.

Start with the suite-router attached HTML quickstart when you still want the broader attached-page route visible before the bundle branch narrows:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_suite_router_attached_html_quickstart.ps1
```

If the replay already carries a non-default checkout, a saved summary, or explicit bundle inputs, preserve that context directly in the suite-router helper:
...
Use this sequence when the replay is already near the suite-router attached-page helper and the current inputs still look like the known three-page compatibility bundle:
...
If the route still needs the broader attached-page or Google-shaped attached-page context before it commits to the pinned bundle, reopen those helpers first and then narrow again:
...
If repo-root, summary, or pinned bundle paths are already in play, keep the same values attached all the way through the narrower bundle bridge:
...
Only widen back into the broader safe-route notes after the pinned bundle route makes the next failure state clear.
