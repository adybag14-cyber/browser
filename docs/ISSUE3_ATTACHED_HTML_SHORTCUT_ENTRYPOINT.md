...
Use that route when the broader Windows runbook already made attached localhost follow-up obvious and you want the route-level surface check, the Windows-to-validation-router bridge, the Windows-first catalog quickstart, the replay-side attached-html quickstart, the compact top-level attached-page quickstart, this attached-page shortcut bridge, and the suite-router-side shortcut bridge all describing the same re-entry order.
...
Use that route when the replay-side attached-page quickstart already made the attached localhost branch obvious and you want the replay-side ladder, the compact change-area quickstart, the compact top-level attached-page quickstart, this shortcut bridge, the suite-router shortcut bridge, and the replay-shortcuts surface all pointing at the same narrower helper order before the route widens back into the next-step matrix or the safe-route map.
...
- broader attached-page localhost-first helper still matters: go from `show_headed_validation_suites.ps1 -ChangeArea google-attached-html` or `show_google_attached_html_validation_flow.ps1` to the attached-html change-area quickstart, then the attached-page shortcut entrypoint, then the suite-router shortcut entrypoint before narrowing further
