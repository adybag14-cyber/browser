...
Use the generic attached-page route from the top-level headed validation suite router when you want that broader attached localhost branch printed again before you narrow into the issue-specific helper chain:
...
If the replay is already running from a non-default checkout, from an already-saved summary, or from an explicit bundle path, preserve that same context directly in the suite-router attached-page quickstart:
...
If the replay is already narrowed to the attached localhost route and you want the dedicated suite-catalog attached-page bridge kept visible before you choose between the top-level attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, or the safe-route map, use:
...
Use the dedicated attached-page shortcut helper when the replay is already centered on the attached localhost compatibility pages and you want the shortest issue-specific bridge before reopening the wider helper family:

```powershell
powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_attached_html_shortcut_entrypoint.ps1
```

If the replay is running from a non-default checkout, from an already-saved summary, or from an explicit bundle path, preserve that same context directly in the attached-page shortcut helper:
...
If the top-level suite router already narrowed the replay to issue `#3`, but you still want the wider shortcut-first surface that keeps the attached localhost branch visible beside the replay helpers, use:
...
- `show_google_issue3_top_level_shortcut_first_entrypoint.ps1` when you still want the wider shortcut-first surface that keeps the attached localhost branch visible beside the replay helpers.
