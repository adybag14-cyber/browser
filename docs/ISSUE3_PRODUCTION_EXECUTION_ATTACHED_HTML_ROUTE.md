...
newer Windows-first surface check, validation-router bridge, catalog
quickstarts, top-level bridges, suite-router bridges, and the pinned three-page
...
its broader validation-router bridge, the Windows-side catalog quickstart, and
the attached-page helper surface visible for as long as practical before
dropping back into the wrapper-heavy safe-route chain.
...
route narrows into the top-level attached-page notes, the suite-catalog bridge,
or the shorter replay surfaces.
...
its validation-router bridge, then the Windows-side catalog quickstart before
reopening the smaller attached-page helper surface. Only reopen the longer
validation-chain notes after the replay has narrowed back into the wrapper-heavy
safe-route path.