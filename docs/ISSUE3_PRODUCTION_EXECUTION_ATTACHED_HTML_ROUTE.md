...
newer attached-page quickstarts, top-level bridges, suite-router bridges, and
...
Use that route when the next replay should stay on the attached-page helper
surface for as long as possible before dropping back into the wrapper-heavy
safe-route chain.
...
in play, carry that same context through the quickstart surface first:
...
localhost follow-up, keep the route on the attached-page helper surface first.
Only reopen the longer validation-chain notes after the replay has narrowed back
into the wrapper-heavy safe-route path.