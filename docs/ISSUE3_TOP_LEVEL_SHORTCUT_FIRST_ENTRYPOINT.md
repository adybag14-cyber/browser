...
Start from the higher-level validation router, keep the top-level shortcut bridge visible, and keep the broader attached-page flow helper and the dedicated Google-shaped attached-page flow easy to reopen before the route widens into the longer attached-page notes.
...
- Attached localhost HTML route already in focus but the broader validation-router bridge matters more than the Windows runbook route: go from `-ChangeArea attached-html` and `-ChangeArea google-attached-html` to `show_google_issue3_validation_router_attached_html_quickstart.ps1`, `show_google_attached_html_validation_flow.ps1`, then the smaller top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog top-level attached-html catalog quickstart, the suite-router attached-page quickstart, or the companion notes, then the top-level shortcut helper, then the attached-page bridge that best matches the current state.
