...
3. Reopen the replay-route shortcut helper next when the route is already known and you want the smaller attached-page, replay-shortcuts, next-step-matrix, bundle-first, and safe-route companion surface.
4. Stay on the bundle-first helper whenever explicit `InputPath` values are already pinned to the three-page compatibility set.
5. Reopen the longer validation-chain notes only after the replay has narrowed into the wrapper-heavy safe route again.
