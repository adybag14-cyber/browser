...
If the suite-catalog guide or any delegated attached-html helper may have moved, fail fast before trusting the handoff:
...
Run that checker before relying on the suite-catalog-side handoff after branch updates, helper renames, or when replay is resuming from a different checkout and you want the compact route to catch missing notes or delegated helpers before the narrower attached-html ladder opens.
...
- you want the suite-catalog surface checker to fail fast on missing notes or delegated helpers before the route leans on the narrower attached-html ladder
...
- you want the Windows-side route, the broader suite-catalog guide, the suite-catalog surface checker, the Windows-first attached-html catalog quickstart, the generic attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side attached HTML quickstart, the validation-router bridge, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, and this suite-catalog-to-top-level catalog quickstart describing the same handoff order
...
- you want the suite-catalog surface checker to validate that checkout before the narrower attached-html handoff proceeds
...
3. `check_google_issue3_suite_catalog_entrypoints_validation_surface.ps1`

Use this when the suite-catalog guide, delegated attached-html helpers, or note paths may have drifted and you want the fail-fast surface check to catch that before the route narrows into the compact top-level and suite-catalog attached-page helpers.
...
10. `show_google_issue3_safe_route_entrypoints.ps1`
...
Once the suite-catalog surface or its dedicated entrypoints guide already made attached localhost follow-up obvious, prefer the suite-catalog surface checker after the guide, then the attached-html change-area quickstart, then the broader attached-page flow helper, then the dedicated Google attached-page flow helper, then the replay-side attached HTML quickstart, then the validation-router attached-page quickstart, then the compact top-level attached-page quickstart, then the top-level attached-page catalog quickstart, then the suite-catalog attached-page bridge before reopening the longer validation-chain notes.
...
- saved summary or repo-root override already present: reopen the broader suite-catalog helper with that same context first, rerun the suite-catalog surface checker for that checkout, then this quickstart, then the attached-html change-area quickstart, then the dedicated Google attached-page flow helper, then choose the suite-catalog bridge, replay shortcuts, the next-step matrix, contextual flow, or the safe-route map only as needed
