...
Use this note when the suite-catalog surface is already open and you want the generic attached-html change-area bridge, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side attached HTML ladder, the compact top-level attached HTML quickstart, and the top-level attached HTML catalog quickstart visible together before the route narrows again.
...
Start from `docs/ISSUE3_SUITE_CATALOG_ENTRYPOINTS.md`, `show_google_issue3_suite_catalog_entrypoints.ps1`, or the broader suite-catalog change-area surfaces, then move through the generic attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side attached HTML quickstart, the validation-router attached-page quickstart, the compact top-level attached-page quickstart, and the top-level attached-page catalog quickstart before the route narrows into the suite-catalog attached-page bridge, the broader Google-shaped attached-page bridge, the shorter attached-page shortcut, replay shortcuts, the next-step matrix, or the safe-route map.
...
- you still want the broader attached-html change-area bridge, the broader attached-page flow helper, the dedicated Google-shaped attached-page flow helper, the replay-side attached HTML quickstart, the validation-router bridge, the compact top-level attached-page quickstart, and the top-level attached-page catalog quickstart visible before the route commits to the suite-catalog attached-page bridge
- you want the broader Google-shaped attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, and the safe-route map kept nearby before the route widens again

## Windows-first and replay-side re-entry

If the replay is reopening from `docs/WINDOWS_FULL_USE.md` first, or if `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already narrowed the route to attached localhost follow-up, keep the broader Windows-first path, the Windows-first attached-html catalog quickstart, the generic attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side attached HTML ladder, and the top-level attached-page catalog route visible before this suite-catalog quickstart collapses back into the narrower helper chain:
...
- you want the Windows-side route, the broader suite-catalog guide, the Windows-first attached-html catalog quickstart, the generic attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side attached HTML quickstart, the validation-router bridge, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, and this suite-catalog-to-top-level catalog quickstart describing the same handoff order
...
1. `show_google_issue3_attached_html_change_area_quickstart.ps1`

Use this when the replay still needs the generic attached-page bridge surfaced before the broader attached-page localhost flow helper, the dedicated Google attached-page flow helper, or the suite-catalog-to-top-level route narrows again.

2. `show_google_attached_html_validation_flow.ps1`

Use this when the replay already includes a Google-like attached page and you want the dedicated Google-shaped surface checker, asset-closure route, and helper output reprinted before the route narrows into the replay-side ladder, the suite-catalog bridge, or the attached-page shortcut.

3. `show_google_issue3_suite_catalog_attached_html_entrypoint.ps1`

Use this as the default next helper when no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first.

4. `show_google_issue3_google_attached_html_entrypoint.ps1`

Use this when you still want the broader Google-shaped attached-page bridge reprinted before narrowing again.
...
Once the suite-catalog surface or its dedicated entrypoints guide already made attached localhost follow-up obvious, prefer the attached-html change-area quickstart, then the broader attached-page flow helper, then the dedicated Google attached-page flow helper, then the replay-side attached HTML quickstart, then the validation-router attached-page quickstart, then the compact top-level attached-page quickstart, then the top-level attached-page catalog quickstart, then the suite-catalog attached-page bridge before reopening the longer validation-chain notes.

- no pinned bundle inputs and no saved replay state yet: go from the suite-catalog surface to the attached-html change-area quickstart, then the broader attached-page flow helper, then the dedicated Google attached-page flow helper, then the replay-side attached HTML quickstart, then the validation-router bridge, then the compact top-level quickstart, then the top-level catalog quickstart, then the suite-catalog attached-page bridge, then the broader Google-shaped attached-page bridge, then the attached-page shortcut, then replay shortcuts, then the next-step matrix, then the safe-route map
- replay reopened from `docs/WINDOWS_FULL_USE.md` or `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md`: keep the broader Windows-side route, the Windows-first attached-html catalog quickstart, the generic attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the replay-side quickstart, and the validation-router quickstart visible before this suite-catalog quickstart narrows into the top-level attached-page note family and the smaller attached-page bridges
...
- saved summary or repo-root override already present: reopen the broader suite-catalog helper with that same context first, then this quickstart, then the attached-html change-area quickstart, then the dedicated Google attached-page flow helper, then choose the suite-catalog bridge, replay shortcuts, the next-step matrix, contextual flow, or the safe-route map only as needed
