...
- explicit `InputPath` values are already pinned to the known attached three-page compatibility bundle and the broader attached-page flow helper, the attached-pages launcher companion surface checker, the attached-pages launcher companion helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the suite-catalog fail-fast checker, and the wider suite-catalog entrypoints guide should stay on that same set before the route narrows again
