...
Use this note when `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already narrowed the next replay to the attached localhost HTML lane and you want the shortest written ladder that matches `show_google_issue3_windows_replay_attached_html_quickstart.ps1` while keeping the newer top-level shortcut bridge, the broader attached-page flow helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the compact bundle-suite surface helper, and the replay-route shortcut bridge visible before the route collapses into the narrowest shortcut helpers.

This keeps the replay-quickstart fail-fast check, the route-level fail-fast check, the Windows-full-use bridge, the Windows-full-use attached-html catalog quickstart, the validation-router attached-page quickstart, the attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the compact bundle-suite surface re-entry, the smaller top-level attached-page helpers, the broader top-level companion-note map, the wider suite-catalog entrypoints guide, the suite-catalog-to-top-level attached-html catalog bridge, the newer top-level shortcut-first bridge, and the replay-route shortcut bridge aligned in one read-first guide before the route narrows into the smallest shortcut surface or widens back into the safe-route stack.
...
- you still want the attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the compact top-level attached-page quickstart, the broader top-level attached-page bridge, the top-level attached-page catalog quickstart, the broader top-level companion-note map, the suite-catalog-to-top-level attached-html catalog quickstart, the wider suite-catalog entrypoints guide, the suite-catalog attached-page bridge, the suite-router sidecar helper, the newer top-level shortcut bridge, and the replay-route shortcut bridge visible before the route collapses into the narrower attached-page shortcut surface or replay-route follow-up
...
- explicit `InputPath` values are already pinned to the known attached three-page compatibility bundle and the broader attached-page flow helper should stay on that same set before the route narrows again
...
Use this when the replay still needs the issue-specific Google attached-page bridge kept visible after the dedicated Google attached-page flow and the suite-router-side helper but before the top-level shortcut bridge, replay-route shortcut, attached-page shortcut, or replay shortcuts.
...
When `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already made attached localhost follow-up the next obvious branch, rerun the replay quickstart surface check first, then rerun the route-level surface check, then keep the Windows-full-use bridge, the Windows-full-use attached-html catalog quickstart, the validation-router attached-page quickstart, the attached-html change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the compact bundle-suite surface helper, the top-level attached-page quickstarts, the broader top-level companion-note map, the suite-catalog-to-top-level attached-html catalog bridge, the wider suite-catalog entrypoints guide, the suite-catalog attached-page bridge, the newer top-level shortcut bridge, and the replay-route shortcut bridge in view before dropping to the shorter attached-page shortcut surface.
...
- replay already narrowed to `-ChangeArea attached-html`: keep `show_google_issue3_attached_html_change_area_quickstart.ps1`, `show_attached_html_validation_flow.ps1`, `show_google_attached_html_validation_flow.ps1`, and the broader attached-page flow helper beside this note so the shorter change-area bridge and the dedicated Google attached-page flow stay visible before the route widens again
...
- saved summary or repo-root override already present: pass the same replay context through the broader route guard, the Windows-side catalog quickstart, the narrower helper chain, the broader attached-page flow helper, the dedicated Google attached-page flow guide, the issue-specific Google attached-page bridge, the compact bundle-suite helper, the suite-catalog-to-top-level attached-html catalog quickstart, the wider suite-catalog entrypoints guide, the suite-catalog attached-page bridge, the top-level shortcut bridge, and the replay-route shortcut bridge so later follow-ups stay aligned
