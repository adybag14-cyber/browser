...
This keeps the route-level fail-fast check, the Windows-full-use bridge, the validation-router attached-page quickstart, the attached-html change-area quickstart, and the smaller top-level attached-page helpers aligned in one read-first guide before the route narrows into shortcuts or widens back into the safe-route stack.
...
- you still want the attached-html change-area quickstart, the compact top-level attached-page quickstart, the broader top-level attached-page bridge, the top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, and the suite-router sidecar helper visible before the route collapses into the narrower shortcut surface
...
9. `show_google_issue3_suite_router_attached_html_quickstart.ps1`
...
When `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already made attached localhost follow-up the next obvious branch, reopen the route-level surface check first, then keep the Windows-full-use bridge, the validation-router attached-page quickstart, the attached-html change-area quickstart, and the top-level attached-page quickstarts in view before dropping to the shorter shortcut surface.
...
- saved summary or repo-root override already present: pass the same replay context through the broader route guard and the narrower helper chain so later follow-ups stay aligned
