# Issue #3 Windows Full-Use Attached Bundle Bridge

Use this note when `docs/WINDOWS_FULL_USE.md` or the broader Windows-first attached-page route has already narrowed the next replay to attached localhost follow-up and the current inputs are still the known three-page compatibility bundle.

This guide keeps the Windows-first route, the compact bundle-suite helper, the replay-route bundle bridge, the narrower bundle-first helper, and the delegated bundle runner on one read-first path.
...
- `docs/ISSUE3_SUITE_ROUTER_ATTACHED_BUNDLE_BRIDGE.md`
...
## Windows-first pinned bundle route

Use this route when `docs/WINDOWS_FULL_USE.md` was the last broader note you opened and the next replay should stay on the exact three-page bundle:
...
- the broader Windows-first runbook already made attached localhost replay the next obvious issue `#3` branch
- the replay should keep the compact bundle-suite helper visible before it narrows into the bundle-first helper
- the exact pinned three-page route matters more than reopening the broader attached-page helper family first

## Broader router-first re-entry
...
- the next run started from the shared validation catalog instead of the narrower Windows-first route
- you want the broader attached-page and Google-shaped attached-page flows visible before the bundle-specific handoff takes over
- the pinned bundle branch is still the right destination, but it should be rediscovered from the higher-level router first
...
- explicit `InputPath` values are already pinned to the known three-page bundle and should stay visible on every helper hop
