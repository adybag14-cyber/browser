...
If the replay already carries a non-default checkout, a saved summary, or explicit attached bundle paths and you want the broader attached-page lane, the Google-shaped attached-page lane, and the pinned bundle lane printed on one compact surface before you choose the next helper, start with:
...
1. Attached HTML context surface
...
Use this when branch state may have moved and you want the issue-specific attached-page helper surface checked before trusting the narrower Google-shaped lane.
...
5. Validation-router attached HTML quickstart
...
10. Attached HTML target-bundle suite surface

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_attached_html_target_bundle_suite_surface.ps1
```

Use this when the current pages are still the pinned three-page compatibility bundle and you want the compact bundle route surfaced before the bundle-first helper takes over.
...
12. Replay shortcuts, next-step matrix, or safe-route entrypoints
...
Use these when the route is already clearly inside issue `#3` and you want the compact shortcut map, the executable branch matrix, or the wrapper-heavy safe-route surface reprinted before deciding whether to widen again.

## Practical rule

Once `show_headed_validation_suites.ps1 -ChangeArea attached-html` has already made attached localhost follow-up the next obvious branch, prefer `show_google_issue3_attached_html_change_area_quickstart.ps1` first.
