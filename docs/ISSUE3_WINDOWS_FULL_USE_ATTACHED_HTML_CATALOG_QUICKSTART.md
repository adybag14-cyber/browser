...
Use this note when `docs/WINDOWS_FULL_USE.md` has already narrowed the next headed replay to attached localhost follow-up and you want one compact Windows-first route that keeps the broader attached-page ladder, the wrapper-backed attached-pages preflight, and the replay-side quickstarts visible together.

This route is intentionally sidecar-first, then asset-audit, then strict manifest gating, then replay. That keeps missing export bundles and missing local assets visible before the browser gets blamed for the next failure.
...
Use that route when the dedicated Google-shaped guide should remain the main nearby fallback, but you still want the cheaper sidecar and asset preflight steps before trusting the narrower replay ladder.
