...
HTML quickstart visible immediately beside the broader Windows full-use route.
...
then move into `show_google_issue3_windows_full_use_attached_html_catalog_quickstart.ps1`
and keep `show_google_issue3_windows_replay_attached_html_quickstart.ps1`,
...
Windows-first attached-page catalog quickstart, the replay-side attached-page
quickstart, the broader attached-page and Google-shaped flow helpers, the newer
...
ladder stays visible before widening again.
...
- no pinned bundle inputs and no saved replay state yet: go from the Windows
  ...
  validation-router bridge, then the Windows full-use catalog quickstart, then
  the replay-side attached-html quickstart, then the broader attached-page
  flow helper, then the dedicated Google-shaped attached-page flow helper,
  then the top-level attached-page catalog quickstart, then the newer
  Top-level shortcut bridge, then the replay-route shortcut bridge, then the
  compact attached-bundle suite surface, then the suite-catalog top-level
...
- broader Google-shaped attached-page surface still matters more than the
  ...
  surface check, then the Windows full-use validation-router bridge, then
  ...
  html quickstart, then the top-level attached-page catalog quickstart, then
  the newer Top-level shortcut bridge, then the replay-route shortcut bridge,
...
- explicit bundle paths already pinned: stay on the validation-router bridge,
  the Windows full-use catalog quickstart, the replay-side attached-html
  quickstart, the compact attached-bundle suite surface, and the bundle-first
  helper before widening back into the broader Google-only path
- saved summary or repo-root override already present: reopen the Windows
  full-use validation-router bridge, Windows full-use catalog quickstart,
  replay-side attached-html quickstart, the broader attached-page flow helper,
  ...
  suite surface with that same context first, then choose replay shortcuts,
  the next-step matrix, contextual flow, or the safe-route map only as needed
