...
- `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` when the replay is ready to widen into replay shortcuts, bundle-first, or safe-route helpers after the replay-side attached-page step is out of the way
...
3. Reopen the Windows full-use route note, rerun `check_google_issue3_windows_full_use_attached_html_route_validation_surface.ps1`, and keep the Windows full-use validation-router bridge or attached-html catalog quickstart nearby when `docs/WINDOWS_FULL_USE.md` was the last broader route and you want that Windows-first branch written back down before the narrower top-level chain takes over.
...
9. Reopen the replay discovery handoff or the broader Windows replay quickstart only after the replay-side attached-page quickstart, attached-page bridge, catalog-side bridge, or shortcut-first bridge is out of the way.
10. Return to the full validation chain only when the next step needs the wrapper-heavy safe route again.
