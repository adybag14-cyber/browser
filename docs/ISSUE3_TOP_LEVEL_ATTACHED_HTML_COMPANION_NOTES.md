...
5. Reopen `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md` and keep `show_headed_validation_suites.ps1 -ChangeArea google-attached-html` plus `show_google_attached_html_validation_flow.ps1` visible when the current attached-page set already includes a Google-like page and the Google-style route should stay in view before the top-level chain narrows back into the shorter issue `#3` helper ladder.
