...
- `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md` when the current attached-page inputs already include a Google-like page and you want the Google-style attached-page guide plus its helper ladder visible before the top-level attached-page chain narrows again
...
5. Reopen `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md` and `docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md`, and keep `show_headed_validation_suites.ps1 -ChangeArea google-attached-html` plus `show_google_attached_html_validation_flow.ps1` and `show_google_issue3_google_attached_html_entrypoint.ps1` visible when the current attached-page set already includes a Google-like page and the Google-style route should stay in view before the top-level chain narrows back into the shorter issue `#3` helper ladder.
