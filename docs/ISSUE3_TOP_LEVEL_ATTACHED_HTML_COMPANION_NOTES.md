...
2. Reopen the Windows full-use route note or the top-level quickstart only if the broader route needs to be written down again.
3. Reopen `docs/ISSUE3_TOP_LEVEL_SHORTCUT_BRIDGE.md` when the top-level router already made issue `#3` obvious and the shortcut-first branch should stay visible beside the attached-page chain.
4. Drop to the suite-router attached-page quickstart when the replay is already narrowed.
5. Reopen the Windows replay quickstart only after the attached-page bridge or shortcut-first bridge is out of the way.
6. Return to the full validation chain only when the next step needs the wrapper-heavy safe route again.