...
Use it when you start from `show_headed_validation_suites.ps1` and want the next helper chosen quickly without reopening the longer Windows runbook or the full wrapper-heavy safe-route notes first, while still keeping the broader attached-page flow, the issue-specific Google-shaped attached-page checker and bridge, the dedicated Google-shaped attached-page flow, the suite-router handoff bridge, the replay-route shortcut bridge, and the compact bundle-suite route visible before the path narrows too far.
...
If the replay is already running from a non-default checkout or from explicit attached bundle paths, preserve that same context directly on the issue-specific checker, the Google-shaped flow helper, the issue-specific entrypoint helper, and the suite-router handoff checker too:
...
Keep `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md`, `docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md`, and `docs/ISSUE3_SUITE_ROUTER_SHORTCUT_BRIDGE.md` nearby when you want the written companions for that narrower issue-specific Google-shaped attached-page lane before the route falls back into the compact attached-page helpers.
...
If the replay is already entering from the broader Windows full-use attached-page route, or from the attached-html change-area quickstart after that helper is already in view, print the validation-router attached-page quickstart next so the Windows-first route, the route guard, the Windows-side catalog quickstart, the broader attached-page flow helper, the issue-specific Google attached-page checker, the dedicated Google-shaped attached-page flow helper, the issue-specific Google attached-page bridge, the suite-router handoff checker, the suite-router handoff bridge, and the narrower replay-side ladder stay aligned:
...
If replay-route follow-up is already open and you want the shorter replay-route-specific bridge visible before the bundle-first helper or the safe-route map takes over, print:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_replay_route_shortcut_entrypoint.ps1
```

When the replay is running from a non-default checkout, from an already-saved summary, or from explicit attached-bundle paths, preserve that same context on the replay-route shortcut helper too:
...
Keep `docs/ISSUE3_REPLAY_ROUTE_SHORTCUT_BRIDGE.md` nearby when you want the written companion for that narrower replay-route bridge before the route widens back into the bundle-first or safe-route helpers.
...
- `show_google_issue3_attached_html_change_area_quickstart.ps1` immediately after `show_headed_validation_suites.ps1 -ChangeArea attached-html` when you want the shortest attached-page handoff plus the broader attached-page flow helper, the issue-specific Google attached-page checker, the dedicated Google attached-page flow helper, the issue-specific Google attached-page bridge, the suite-router handoff checker, and the suite-router handoff bridge visible before the validation-router and replay-side ladders narrow the route again.
- `check_google_issue3_google_attached_html_entrypoint_validation_surface.ps1` immediately after the broader attached-page flow helper when the route already looks Google-shaped and you want the narrower issue-specific lane to fail fast before reopening the dedicated flow or the issue-specific bridge.
- `show_google_issue3_google_attached_html_entrypoint.ps1` immediately after the issue-specific checker or dedicated Google attached-page flow helper when you want the shortest issue-specific bridge kept visible before widening into the suite-router handoff checker, the suite-router handoff bridge, the validation-router quickstart, the replay-side quickstart, replay route, replay-route shortcut, or the safe-route map.
- `check_google_issue3_suite_router_handoff_validation_surface.ps1` immediately before `show_google_issue3_suite_router_handoff.ps1` when you want the compact suite-router handoff to fail fast on missing bridge notes, attached-page helpers, replay-route companions, or bundle-aware follow-up before the narrower helper ladder takes over.
- `show_google_issue3_suite_router_handoff.ps1` immediately after the issue-specific Google attached-page bridge when you want the compact suite-router handoff to keep the narrower checker, replay shortcuts, the next-step matrix, the replay-route bridge, and bundle-aware follow-up visible on one issue-specific surface before the replay-route shortcut or safe-route map take over.
- `show_google_issue3_validation_router_attached_html_quickstart.ps1` immediately after the Windows full-use attached-page route, the attached-html change-area quickstart, or the suite-router handoff when you want the shortest validation-router-side bridge from the main validation catalog into the narrower attached-page helper ladder while keeping the broader attached-page flow, the issue-specific Google attached-page checker, the dedicated Google-shaped attached-page flow, the issue-specific bridge, and the suite-router handoff in view.
...
- `show_google_issue3_replay_shortcuts.ps1` as the direct next helper once the route is already narrow enough and you want the tightest shortcut surface before deciding whether to widen into replay route, the replay-route shortcut helper, the bundle-first helper, or the safe-route entrypoints.
- `show_google_issue3_replay_route_shortcut_entrypoint.ps1` once replay-route follow-up is already open and you want the shorter replay-route-specific bridge kept visible before the bundle-first helper or the safe-route map takes over.
...
Use this compact sequence when the replay is already narrowed to the attached localhost HTML route and should stay on the attached-page-specific bridge while keeping the broader attached-page flow helper, the issue-specific Google attached-page checker, the dedicated Google-shaped attached-page flow, the issue-specific bridge, the suite-router handoff checker, and the suite-router handoff visible before widening again:
...
- replay reopened from `docs/WINDOWS_FULL_USE.md`: run `show_google_issue3_windows_full_use_attached_html_route.ps1` first, then the route-level surface check, then the Windows-to-validation-router bridge, then the Windows full-use attached-html catalog quickstart, then the validation-router attached-page quickstart, then the replay-side attached-page quickstart, then the top-level attached-page quickstart, then the top-level attached-page catalog quickstart, then the suite-catalog-to-top-level attached-page catalog quickstart, then the broader top-level attached-page bridge, then the suite-router and suite-catalog attached-page bridges, then the replay-route shortcut helper before widening into the safe-route map.
- broader issue `#3` router already surfaced the attached localhost branch: go from `-SuiteName google-recommended` or `-ChangeArea google-input` to the top-level shortcut helper when the route is still generic, reopen `show_google_issue3_suite_catalog_entrypoints.ps1` plus `docs/ISSUE3_SUITE_CATALOG_ENTRYPOINTS.md` when you want the broader suite-catalog re-entry surface kept visible before the narrower suite-router-side ladder, or go directly to the attached-html change-area quickstart when attached localhost follow-up is already the next obvious branch, then reopen the broader attached-page flow helper, the issue-specific Google attached-page checker, the dedicated Google-shaped attached-page flow helper, the issue-specific Google attached-page bridge, the suite-router handoff checker, the suite-router handoff bridge, the validation-router attached-page quickstart, the replay-side attached-page quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, and the replay-route shortcut helper as soon as the route needs the narrower replay-route bridge back on screen.
- current replay already narrowed to the attached localhost HTML route: reopen `show_headed_validation_suites.ps1 -ChangeArea attached-html` first, then keep the attached-html change-area quickstart, the broader attached-page flow helper, the issue-specific Google attached-page checker, the dedicated Google-shaped attached-page flow helper, the issue-specific Google attached-page bridge, the suite-router handoff checker, the suite-router handoff bridge, the validation-router attached-page quickstart, the replay-side attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the broader top-level attached-page bridge, the suite-router attached-page quickstart, the suite-catalog attached-page bridge, and the replay-route shortcut helper visible before choosing between the attached-page shortcut, replay shortcuts, the bundle-first branch, or the safe-route map.
- broader Google-shaped attached-page surface still matters: reopen `show_headed_validation_suites.ps1 -ChangeArea google-attached-html` first, then `check_google_issue3_google_attached_html_entrypoint_validation_surface.ps1`, then `show_google_attached_html_validation_flow.ps1`, then `show_google_issue3_google_attached_html_entrypoint.ps1`, then `check_google_issue3_suite_router_handoff_validation_surface.ps1`, then `show_google_issue3_suite_router_handoff.ps1`, then the validation-router attached-page quickstart, then the replay-side attached-page quickstart, then the top-level attached-page quickstart, then the top-level attached-page catalog quickstart, then the suite-catalog-to-top-level attached-page catalog quickstart, then the replay-route shortcut helper before narrowing further.
...
- saved summary or repo-root override already present: reopen the attached-html change-area quickstart, the broader attached-page flow helper when useful, the issue-specific Google attached-page checker when the route is already Google-shaped, the dedicated Google-shaped attached-page flow helper, the issue-specific Google attached-page bridge, the suite-router handoff checker, the suite-router handoff bridge, the validation-router attached-page quickstart, the replay-side attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the compact bundle-suite helper when the route is already close to the pinned bundle, the replay-route shortcut helper, and the broader suite-catalog re-entry helper with that same context first, then choose replay shortcuts, contextual flow, or the safe-route map only as needed.
