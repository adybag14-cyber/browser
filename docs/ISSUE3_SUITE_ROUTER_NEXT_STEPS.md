...
route, replay shortcuts, validation-router attached-html quickstart,
suite-router attached-html quickstart, Google-attached-html entrypoint,
...
That keeps the higher-level router, the validation-router attached-html bridge,
the compact issue `#3` helper chooser, the shorter suite-router-side attached-page
bridge, the dedicated Google-shaped attached-page helper, and the replay
shortcuts aligned on the same current branch guidance.
