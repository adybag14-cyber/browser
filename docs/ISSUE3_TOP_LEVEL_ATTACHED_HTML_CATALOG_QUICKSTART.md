...
Use this note when issue `#3` replay is already re-entering from the top-level attached localhost HTML route and you want the suite-catalog-to-top-level attached-page catalog quickstart, the launcher-backed sibling `_files` sidecar audit, the broader attached-page flow helper, the dedicated Google attached-page flow helper, and the suite-catalog attached-page bridge kept visible before the helper chain narrows into the shorter issue-specific follow-ups.

If you want that compact top-level attached-page plus suite-catalog catalog handoff first, start with:
...
Start from `show_headed_validation_suites.ps1 -ChangeArea attached-html`, `show_headed_validation_suites.ps1 -ChangeArea google-attached-html`, or `show_headed_validation_suites.ps1 -ChangeArea attached-html-target-bundle`, then move into `show_google_issue3_top_level_attached_html_catalog_quickstart.ps1` when the attached-page route is already in focus and you want the suite-catalog-to-top-level attached-page catalog quickstart, the launcher-backed sidecar audit, the broader attached-page flow helper, and the dedicated Google attached-page flow helper surfaced beside the compact top-level attached-page quickstart.

If the replay is reopening specifically from `show_headed_validation_suites.ps1 -ChangeArea attached-html`, keep `show_google_issue3_attached_html_change_area_quickstart.ps1` nearby as the read-first bridge that still surfaces the broader attached-page flow helper and the launcher-backed sidecar audit before this catalog quickstart narrows the route again.

If the replay is reopening specifically from `show_headed_validation_suites.ps1 -ChangeArea google-attached-html`, keep `docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md` nearby, run the lighter sidecar-bundle audit first, and rerun `show_google_attached_html_validation_flow.ps1` before this catalog quickstart narrows back into the shorter top-level or suite-catalog ladders.

If the replay is reopening from the broader Windows full-use branch or from the replay-side attached-html quickstart, keep those broader helper ladders visible long enough to rerun the route-level surface check, rerun the launcher-backed sidecar audit, and confirm the narrower top-level catalog route is still aligned before this note takes over.
...
If the replay is already reopening from the generic attached localhost branch, use the change-area quickstart first so the broader attached-page flow helper, the launcher-backed sidecar audit, and the dedicated Google attached-page flow helper stay visible before the catalog quickstart narrows the route:
...
- you still want the broader attached-page flow helper, the launcher-backed sidecar audit, and the dedicated Google attached-page flow helper surfaced before the replay commits to the suite-catalog-to-top-level catalog quickstart and the suite-catalog attached-page bridge
...
If the replay is reopening from `docs/WINDOWS_FULL_USE.md` first and you want the broader Windows-first branch kept aligned with the narrower top-level attached-page catalog route, start with:
...
Use that route when the broader Windows runbook already made attached localhost follow-up obvious and you want the route-level surface check, the Windows-to-validation-router bridge, the Windows-first catalog quickstart, the launcher-backed sidecar audit, the compact top-level attached-page quickstart, the dedicated Google attached-page flow helper, this catalog quickstart, the suite-catalog-to-top-level catalog quickstart, and the suite-catalog bridge all describing the same re-entry order.

## Replay-side attached HTML re-entry

If `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md` already narrowed the replay to the attached localhost lane and you want the replay-side helper ladder kept visible before this top-level catalog route narrows further, start with:
...
Use that route when the replay-side attached-page quickstart already made the attached localhost branch obvious and you want the replay-side ladder, the launcher-backed sidecar audit, the dedicated Google attached-page flow helper, the compact top-level quickstart, this catalog quickstart, the suite-catalog-to-top-level catalog quickstart, the suite-catalog bridge, and the shorter attached-page shortcut all pointing at the same narrower helper order before the route widens back into replay shortcuts or the safe-route map.
...
Use that route when the attached localhost compatibility lane is already obvious and you want the change-area quickstart, the launcher-backed sidecar audit, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the compact top-level attached-page quickstart, the suite-catalog-to-top-level catalog quickstart, the dedicated suite-catalog guide, and the suite-catalog attached-page bridge visible before you choose between the narrower attached-page helper chain, replay shortcuts, or the safe-route map.
...
Use that route when the broader issue-specific attached-page bridge, the launcher-backed sidecar audit, and the dedicated Google attached-page flow should stay visible before you drop to the shorter attached-page shortcut or replay-shortcuts surface.
...
2. Attached pages sidecar audit

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\start_attached_pages_catalog.ps1 -InputPath '<attached-html-root>' -AuditSidecars
```

Use this when the current export may still be missing its whole sibling `_files` bundle and you want that simpler failure mode ruled in or out before the dedicated Google attached-page flow, the suite-catalog bridge, or the shorter attached-page shortcut take over.

3. Google attached HTML validation flow

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_attached_html_validation_flow.ps1
```

Use this when the replay already includes a Google-like attached page and you want the dedicated Google-shaped surface checker, asset-closure route, and helper output reprinted before the route narrows into the top-level catalog, the suite-catalog bridge, or the attached-page shortcut.
...
Use this when you want the narrower suite-catalog-to-top-level catalog handoff printed immediately after this top-level catalog quickstart so the replay-side and suite-catalog ladders stay aligned before the route narrows into the suite-catalog attached-page bridge.

5. Suite-catalog entrypoints guide

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_suite_catalog_entrypoints.ps1
```

Use this when you want the dedicated suite-catalog guide reprinted before the narrower suite-catalog attached-page bridge so the broader catalog-side route map stays visible beside this top-level catalog quickstart.
...
7. Google attached HTML entrypoint
...
Once the top-level attached localhost HTML route is already in focus, prefer `show_google_issue3_attached_html_change_area_quickstart.ps1` first, then keep the launcher-backed sidecar audit, the broader attached-page flow helper, the dedicated Google attached-page flow helper, this catalog quickstart, the suite-catalog-to-top-level catalog quickstart, the dedicated suite-catalog guide, and the suite-catalog attached-page bridge visible before reopening the longer validation-chain notes again.

- replay reopened from `docs/WINDOWS_FULL_USE.md`: go from the Windows full-use route to the fail-fast surface check, then the Windows full-use validation-router bridge, then the Windows full-use attached-html catalog quickstart, then the launcher-backed sidecar audit, then the dedicated Google attached-page flow helper, then the compact top-level attached-page quickstart, then this top-level catalog quickstart, then the suite-catalog-to-top-level catalog quickstart, then the dedicated suite-catalog guide, then the suite-catalog attached-page bridge before narrowing further
- replay reopened from `docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md`: reopen `show_google_issue3_windows_replay_attached_html_quickstart.ps1` first, then the launcher-backed sidecar audit, then the dedicated Google attached-page flow helper, then the compact top-level attached-page quickstart, then this top-level catalog quickstart, then the suite-catalog-to-top-level catalog quickstart, then the dedicated suite-catalog guide, then the suite-catalog bridge, then the attached-page shortcut before widening again
- no pinned bundle inputs and no saved replay state yet: go from the top-level attached-page route to the attached-html change-area quickstart, then the launcher-backed sidecar audit, then the broader attached-page flow helper, then the dedicated Google attached-page flow helper, then the top-level attached-page quickstart, then the catalog quickstart, then the suite-catalog-to-top-level catalog quickstart, then the dedicated suite-catalog guide, then the suite-catalog attached-page bridge, then the broader Google-shaped attached-page bridge, then the attached-page shortcut, then replay shortcuts, then the safe-route map
- broader Google-shaped attached-page surface still matters more than the generic shortcut chain: go from `-ChangeArea google-attached-html` to the launcher-backed sidecar audit, then `show_google_attached_html_validation_flow.ps1`, then the catalog quickstart, then the suite-catalog-to-top-level catalog quickstart, then the dedicated suite-catalog guide, then the suite-catalog attached-page bridge, then the Google attached-page entrypoint before narrowing further
- explicit bundle paths already pinned: keep the change-area quickstart, the launcher-backed sidecar audit, the broader attached-page flow helper, the dedicated Google attached-page flow helper, this catalog quickstart, the suite-catalog-to-top-level catalog quickstart, and `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_REFERENCE.md` in front of the bundle-first helper and the delegated bundle runner so the known three-page compatibility set stays fixed before widening back into the broader Google-only path
- saved summary or repo-root override already present: reopen the attached-html change-area quickstart or the catalog quickstart with that same context first, then `show_attached_html_validation_flow.ps1`, then the launcher-backed sidecar audit, then `show_google_attached_html_validation_flow.ps1`, then the suite-catalog-to-top-level catalog quickstart, then the dedicated suite-catalog guide, then choose replay shortcuts, the next-step matrix, contextual flow, or the safe-route map only as needed
