...
pub fn build(b: *std.Build) void {
    ...
    const cpp_flags: []const []const u8 = if (is_msvc) &.{ "-std=c++17", "-Wno-unused-command-line-argument", "-DNOMINMAX=1", "-DWIN32_LEAN_AND_MEAN=1", "-DOPENSSL_NO_ASM=1" } else &.{"-std=c++17"};
    ...
    const libpki = blk: {
        ...
        mod.addCSourceFiles(.{
            .root = ssl_source.path("."),
            .files = pki_sources,
            .flags = if (is_msvc) &.{ "-D_BORINGSSL_LIBPKI_", "-std=c++17", "-Wno-unused-command-line-argument", "-DNOMINMAX=1", "-DWIN32_LEAN_AND_MEAN=1", "-DOPENSSL_NO_ASM=1" } else &.{ "-D_BORINGSSL_LIBPKI_", "-std=c++17" },
        });
