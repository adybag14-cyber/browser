...
if [[ "${JSON}" -eq 1 ]]; then
    ...
    printf '    %s,\n' "$(json_escape "Run windows_runtime_surface first so missing PowerShell helpers or nearby docs fail fast before the broader Windows route reopens.")"
    printf '    %s,\n' "$(json_escape "Run windows_runtime_route next when you want the fuller replay ladder, narrower helper order, and nearby-note guidance on one Windows surface.")"
