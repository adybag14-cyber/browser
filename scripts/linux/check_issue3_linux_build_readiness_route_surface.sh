...
declare -a REFERENCE_PATHS=(
    ...
    "docs/ISSUE3_WORKSPACE_CONTEXT_ROUTE.md|file|Workspace-context note that should stay visible when nested or restored checkouts need surfaced roots before later route overrides are rebuilt by hand."
    ...
    "docs/ISSUE3_SAVED_RUST_ARCHIVE_CANDIDATES_ROUTE.md|file|Read-first saved Rust archive-candidate note when the route still needs the preferred saved Rust restore path."
    "docs/ISSUE3_STAGED_RUST_TOOLCHAIN_CANDIDATES_ROUTE.md|file|Read-first staged Rust candidate note when the route may already have a reusable Rust toolchain under ../toolchains."
...
declare -a CONTENT_EXPECTATIONS=(
    ...
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|docs/ISSUE3_SAVED_RUST_ARCHIVE_CANDIDATES_ROUTE.md|The Linux build-readiness note keeps the saved Rust archive-candidate note visible before rebuilding the saved Rust restore path."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|docs/ISSUE3_STAGED_RUST_TOOLCHAIN_CANDIDATES_ROUTE.md|The Linux build-readiness note keeps the staged Rust candidate note visible before host Rust or restore drift is blamed."
    ...
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|bash ./scripts/linux/check_issue3_saved_rust_archive_candidates_route_surface.sh|The Linux build-readiness note keeps the exact saved Rust archive-candidate surface-check command visible before rebuilding the restore path."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|bash ./scripts/linux/show_issue3_saved_rust_archive_candidates_route.sh|The Linux build-readiness note keeps the exact saved Rust archive-candidate route command visible before rebuilding the restore path."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|bash ./scripts/linux/check_issue3_staged_rust_toolchain_candidates_route_surface.sh|The Linux build-readiness note keeps the exact staged Rust surface-check command visible before host Rust or restore drift is blamed."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|bash ./scripts/linux/show_issue3_staged_rust_toolchain_candidates_route.sh|The Linux build-readiness note keeps the exact staged Rust route command visible before host Rust or restore drift is blamed."
    ...
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|check_issue3_saved_rust_archive_candidates_route_surface.sh|The Linux build-readiness note keeps the saved Rust archive-candidate route surface checker named explicitly."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|show_issue3_saved_rust_archive_candidates_route.sh|The Linux build-readiness note keeps the saved Rust archive-candidate route named explicitly."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|check_issue3_staged_rust_toolchain_candidates_route_surface.sh|The Linux build-readiness note keeps the staged Rust route surface checker named explicitly."
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|show_issue3_staged_rust_toolchain_candidates_route.sh|The Linux build-readiness note keeps the staged Rust route named explicitly."
    ...
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|docs/ISSUE3_SAVED_RUST_ARCHIVE_CANDIDATES_ROUTE.md|The Linux route printer keeps the saved Rust archive-candidate note in its read-first companion list."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|docs/ISSUE3_STAGED_RUST_TOOLCHAIN_CANDIDATES_ROUTE.md|The Linux route printer keeps the staged Rust candidate note in its read-first companion list."
    ...
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|check_issue3_saved_rust_archive_candidates_route_surface.sh|The Linux route printer keeps a dedicated saved Rust archive route surface-check command before the saved Rust restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|show_issue3_saved_rust_archive_candidates_route.sh|The Linux route printer keeps a dedicated saved Rust archive route before the saved Rust restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|check_issue3_staged_rust_toolchain_candidates_route_surface.sh|The Linux route printer keeps a dedicated staged Rust route surface-check command before host Rust or restore drift is blamed."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|show_issue3_staged_rust_toolchain_candidates_route.sh|The Linux route printer keeps a dedicated staged Rust route before host Rust or restore drift is blamed."
    ...
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Saved Rust archive route surface check:|The Linux route printer still prints the saved Rust archive surface-check step before the restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Saved Rust archive route:|The Linux route printer still prints the saved Rust archive route step before the restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Staged Rust route surface check:|The Linux route printer still prints the staged Rust route surface-check step before host Rust or restore drift is blamed."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Staged Rust route:|The Linux route printer still prints the staged Rust route step before host Rust or restore drift is blamed."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|resolve_first_existing_path "'"'"'${REPO_ROOT}'"'"'" "'"'"'memory/repo_archives/browser'"'"'"|The Linux route printer keeps ancestor saved-archives discovery visible before it falls back to sibling Memory layout."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|resolve_first_existing_path "'"'"'${REPO_ROOT}'"'"'" "'"'"'browser-memory-snapshot'"'"'"|The Linux route printer keeps ancestor restored-checkout discovery visible before it falls back to sibling workspace layout."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|resolve_first_existing_path "'"'"'${REPO_ROOT}'"'"'" "'"'"'toolchains'"'"'"|The Linux route printer keeps ancestor toolchains discovery visible before it falls back to sibling workspace layout."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|resolve_first_existing_path "'"'"'${REPO_ROOT}'"'"'" "'"'"'offline-deps'"'"'"|The Linux route printer keeps ancestor offline-deps discovery visible before it falls back to sibling workspace layout."
    ...
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Saved Rust archive route surface check:|The Linux route printer still prints the saved Rust archive surface-check step before the restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Saved Rust archive route:|The Linux route printer still prints the saved Rust archive step before the restore path is rebuilt."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Staged Rust route surface check:|The Linux route printer still prints the staged Rust route surface-check step before host Rust or restore drift is blamed."
    "scripts/linux/show_issue3_linux_build_readiness_route.sh|Staged Rust route:|The Linux route printer still prints the staged Rust route step before host Rust or restore drift is blamed."
