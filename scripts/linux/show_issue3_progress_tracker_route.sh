...
NESTED_WORKSPACE_SAVED_MEMORY_PREFLIGHT_COMMAND="bash $(format_shell_arg "${HELPER_ROOT}/scripts/linux/run_issue11_nested_workspace_saved_memory_preflight.sh") --repo-root $(format_shell_arg "${REPO_ROOT}")"
...
STAGED_ZIG_ROUTE_SURFACE_COMMAND="bash $(format_shell_arg "${HELPER_ROOT}/scripts/linux/check_issue3_staged_zig_toolchain_candidates_route_surface.sh") --repo-root $(format_shell_arg "${REPO_ROOT}") --toolchains-root $(format_shell_arg "${TOOLCHAINS_ROOT}")"
STAGED_ZIG_ROUTE_COMMAND="bash $(format_shell_arg "${HELPER_ROOT}/scripts/linux/show_issue3_staged_zig_toolchain_candidates_route.sh") --repo-root $(format_shell_arg "${REPO_ROOT}") --toolchains-root $(format_shell_arg "${TOOLCHAINS_ROOT}") --saved-archives-root $(format_shell_arg "${SAVED_ARCHIVES_ROOT}")"
...
TOOLCHAINS_ROOT_CANDIDATES_COMMAND="python3 $(format_shell_arg "${HELPER_ROOT}/scripts/check_issue11_toolchains_root_candidates.py") --repo-root $(format_shell_arg "${REPO_ROOT}")"
...
if [[ -n "${FALLBACK_ZIG_ARCHIVE}" ]]; then
    WORKSPACE_CONTEXT_ROUTE_COMMAND+=" --fallback-zig-archive $(format_shell_arg "${FALLBACK_ZIG_ARCHIVE}")"
    SAVED_MEMORY_ROUTE_COMMAND+=" --fallback-zig-archive $(format_shell_arg "${FALLBACK_ZIG_ARCHIVE}")"
    NESTED_WORKSPACE_SAVED_MEMORY_PREFLIGHT_COMMAND+=" --fallback-zig-archive $(format_shell_arg "${FALLBACK_ZIG_ARCHIVE}")"
...
if [[ "${JSON}" -eq 1 ]]; then
    ...
    printf '    "workspace_context_route_surface": %s,\n' "$(json_escape "${WORKSPACE_CONTEXT_ROUTE_SURFACE_COMMAND}")"
    printf '    "workspace_context_route": %s,\n' "$(json_escape "${WORKSPACE_CONTEXT_ROUTE_COMMAND}")"
    printf '    "saved_memory_inputs_route": %s,\n' "$(json_escape "${SAVED_MEMORY_ROUTE_COMMAND}")"
    printf '    "nested_workspace_saved_memory_preflight": %s,\n' "$(json_escape "${NESTED_WORKSPACE_SAVED_MEMORY_PREFLIGHT_COMMAND}")"
    printf '    "saved_archive_integrity_route": %s,\n' "$(json_escape "${SAVED_ARCHIVE_INTEGRITY_ROUTE_COMMAND}")"
    printf '    "saved_rust_build_readiness_route_surface": %s,\n' "$(json_escape "${SAVED_RUST_BRIDGE_ROUTE_SURFACE_COMMAND}")"
    ...
    printf '    "staged_zig_route_surface": %s,\n' "$(json_escape "${STAGED_ZIG_ROUTE_SURFACE_COMMAND}")"
    printf '    "staged_zig_toolchain_candidates_route": %s,\n' "$(json_escape "${STAGED_ZIG_ROUTE_COMMAND}")"
    printf '    "staged_zig_toolchain_candidates": %s,\n' "$(json_escape "${STAGED_ZIG_CANDIDATES_COMMAND}")"
    printf '    "toolchains_root_candidates": %s,\n' "$(json_escape "${TOOLCHAINS_ROOT_CANDIDATES_COMMAND}")"
    ...
    printf '    %s,\n' "$(json_escape "When the immediate slice is about nested or restored checkout layout, surface the workspace-context route before saved-memory, saved-Rust, saved-Zig, build-readiness, or Zig recovery helpers are trusted with guessed roots.")"
    ...
    printf '    %s,\n' "$(json_escape "When the immediate slice is about threading surfaced helper, Memory, agent-files, and restored-checkout roots into the saved-memory preflight through one branch-local command, run the nested-workspace saved-Memory preflight helper before the route widens into saved-Rust or broader Linux/WSL follow-ups.")"
    ...
    printf '    %s,\n' "$(json_escape "When both toolchains/ and .toolchains/ may be visible, run the toolchains-root candidate helper before build-readiness rerun, matching-line, Linux or WSL build-readiness, or Zig recovery helpers so they all reuse the same surfaced root.")"
    ...
    printf '    %s,\n' "$(json_escape "When the immediate slice is about reusing a staged Zig candidate before any archive restore, fail fast on the narrower staged-Zig route and print its compact handoff before the broader saved-Zig or recovery routes are trusted.")"
...
===============
  ...
  Nested-workspace saved-Memory preflight:
    ${NESTED_WORKSPACE_SAVED_MEMORY_PREFLIGHT_COMMAND}
  ...
  Saved Rust archive route surface check:
    ${SAVED_RUST_ARCHIVE_ROUTE_SURFACE_COMMAND}
...
=============
  ...
  - When the immediate slice is about nested or restored checkout layout, surface the workspace-context route before saved-memory, saved-Rust, saved-Zig, build-readiness, or Zig recovery helpers are trusted with guessed roots.
  ...
  - When the immediate slice is about threading surfaced helper, Memory, agent-files, and restored-checkout roots into the saved-memory preflight through one branch-local command, run the nested-workspace saved-Memory preflight helper before the route widens into saved-Rust or broader Linux/WSL follow-ups.
  ...
  - When both toolchains/ and .toolchains/ may be visible, run the toolchains-root candidate helper before build-readiness rerun, matching-line, Linux or WSL build-readiness, or Zig recovery helpers so they all reuse the same surfaced root.
  ...
  - When the immediate slice is about reusing a staged Zig candidate before any archive restore, fail fast on the narrower staged-Zig route and print its compact handoff before the broader saved-Zig or recovery routes are trusted.
  ...
  - Thread --fallback-zig-archive through this route when the attached archive is not beside the repo workspace so nested workspace-context, nested-workspace saved-memory preflight, saved-memory, saved-archive-integrity, saved-Zig, staged-Zig, build-readiness-rerun, matching-line, and Zig recovery helpers all inspect the same surfaced path.
  - Run the matching-line gate after any saved Zig restore so the staged toolchains root has to prove a real branch-compatible 0.15.x executable exists.
  ...
  - When the next step is still environment-gated, follow the workspace-context, nested-workspace saved-memory preflight, saved-memory, saved-archive-integrity, saved-Rust, staged-Rust, saved-Zig, staged-Zig, build-readiness-rerun, build-readiness, matching-line, or Zig recovery routes instead of reopening the direct Page.zig plus win32_backend.zig patch.
EOF2
