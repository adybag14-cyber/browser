...
declare -a REFERENCE_PATHS=(
    ...
    "scripts/linux/check_issue3_zig_toolchain_match.sh|file|Dedicated matching-line gate that proves a staged Zig 0.15.x toolchain exists before broader readiness is trusted again."
...
declare -a CONTENT_EXPECTATIONS=(
    ...
    "docs/ISSUE3_ZIG_TOOLCHAIN_RECOVERY_ROUTE.md|scripts/linux/check_issue3_zig_toolchain_match.sh|The Zig recovery note keeps the dedicated matching-line gate visible."
    ...
    "docs/ISSUE3_LINUX_BUILD_READINESS_ROUTE.md|scripts/linux/check_issue3_zig_toolchain_match.sh|The Linux build-readiness note still points at the dedicated Zig matching-line gate."
    ...
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|check_issue3_zig_toolchain_match.sh|The Zig recovery route still points at the dedicated matching-line gate."
    ...
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|matching_line_gate|The Zig recovery route JSON output still carries the dedicated matching-line gate command."
    ...
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|saved_archive_helper_warning|The Zig recovery route JSON output still exposes saved-archive helper warnings for regression control."
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|Matching-line gate|The Zig recovery route still prints the dedicated matching-line gate section."
    ...
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|Saved archive helper warning|The Zig recovery route still prints a dedicated helper-warning section when saved archive discovery fails."
    "scripts/linux/show_issue3_zig_toolchain_recovery_route.sh|Re-run the saved archive candidate helper once its warning is resolved|The Zig recovery route still teaches reruns how to recover from saved-archive helper failures."
