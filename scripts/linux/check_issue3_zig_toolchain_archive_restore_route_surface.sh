...
declare -a CONTENT_EXPECTATIONS=(
    "docs/ISSUE3_ZIG_TOOLCHAIN_ARCHIVE_RESTORE_ROUTE.md|scripts/linux/check_issue3_zig_toolchain_archive_restore_route_surface.sh|The archive-restore note keeps the new surface checker named explicitly."
    "docs/ISSUE3_ZIG_TOOLCHAIN_ARCHIVE_RESTORE_ROUTE.md|Run The Surface Check First|The archive-restore note tells reruns to fail fast before touching toolchain staging."
    "docs/ISSUE3_ZIG_TOOLCHAIN_ARCHIVE_RESTORE_ROUTE.md|bash ./scripts/linux/check_issue3_zig_toolchain_archive_restore_route_surface.sh|The archive-restore note prints the exact surface-check command."
    "docs/ISSUE3_ZIG_TOOLCHAIN_ARCHIVE_RESTORE_ROUTE.md|bash ./scripts/linux/restore_zig_toolchain_archive.sh --archive /path/to/zig-0.15.2.tar.xz --check-only|The archive-restore note keeps the check-only restore command visible."
