...
$entrypoint = [ordered]@{
    ...
    purpose = 'Keep the top-level attached-page route, the newer attached-html change-area quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the dedicated suite-catalog guide, and the suite-catalog attached-page bridge visible on one compact helper before the replay narrows into the issue-specific attached-page entrypoints, shortcuts, or bundle-first path.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Use this helper when the top-level attached HTML route is already in focus but you still want the newer change-area quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the dedicated suite-catalog guide, and the suite-catalog attached-page bridge printed on the same compact surface.',
        ...
        'Use attached_html_flow when the replay still needs the broader attached-page localhost helper visible from that same change-area branch before you commit to the suite-catalog-to-top-level handoff or the suite-catalog attached-page bridge.',
        ...
        'Use contextual_flow when RepoRoot or SummaryPath is already in play and the next helper surface should keep that replay context aligned while you choose between the suite-catalog-to-top-level catalog handoff, the dedicated suite-catalog guide, the suite-catalog attached-page bridge, the Google-shaped attached-page helper, replay shortcuts, the next-step matrix, or the safe-route map.',
        'Keep the attached-html change-area quickstart note, the top-level attached-page quickstart note, the top-level attached-page catalog quickstart note, the suite-catalog-to-top-level attached-page catalog quickstart note, the dedicated suite-catalog guide, the suite-catalog attached-page bridge note, the validation-chain note, and the Windows runbook nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between the suite-catalog-to-top-level catalog handoff, the dedicated suite-catalog guide, the suite-catalog attached-page bridge, replay shortcuts, the next-step matrix, or the safe-route map.'
} else {
    'No pinned bundle inputs, saved summary, or non-default repo root are in play yet, so jump straight from the catalog quickstart to the suite-catalog-to-top-level catalog handoff and keep the broader catalog-side route visible before the narrower suite-catalog attached-page bridge takes over.'
}
...
Write-Host (("  6. Top-level entrypoint:       {0}") -f $entrypoint.top_level_commands.top_level_attached_html_entrypoint)
Write-Host (("  7. Top-level quickstart:       {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host (("  8. Router quickstart:          {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host (("  9. Catalog top-level quick:    {0}") -f $entrypoint.helper_commands.suite_catalog_top_level_attached_html_catalog_quickstart)
Write-Host ((" 10. Catalog guide:              {0}") -f $entrypoint.helper_commands.suite_catalog_entrypoints)
Write-Host ((" 11. Catalog bridge:             {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
Write-Host ((" 12. Google attached route:      {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
...
Write-Host ((" 18. Safe-route map:             {0}") -f $entrypoint.helper_commands.safe_route_entrypoints)
Write-Host ''
