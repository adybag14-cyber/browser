...
$entrypoint = [ordered]@{
    ...
    purpose = 'Keep the top-level attached-page route, the newer attached-html change-area quickstart, and the suite-catalog attached-page bridge visible on one compact helper before the replay narrows into the issue-specific attached-page entrypoints, shortcuts, or bundle-first path.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Use this helper when the top-level attached HTML route is already in focus but you still want the newer change-area quickstart and the suite-catalog attached-page bridge printed on the same compact surface.',
        ...
        'Use attached_html_flow when the replay still needs the broader attached-page localhost helper visible from that same change-area branch before you commit to the suite-catalog attached-page bridge.',
        ...
        'Keep the attached-html change-area quickstart note, the top-level attached-page quickstart note, the top-level attached-page catalog quickstart note, the suite-catalog attached-page bridge note, the validation-chain note, and the Windows runbook nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'No pinned bundle inputs, saved summary, or non-default repo root are in play yet, so jump straight from the catalog quickstart to the suite-catalog attached-page bridge while still keeping the change-area quickstart and broader attached-page flow helper visible on the same surface.'
}
...
Write-Host (("  9. Catalog bridge:          {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
Write-Host ((" 10. Google attached route:   {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
...
Write-Host ((" 16. Safe-route map:          {0}") -f $entrypoint.helper_commands.safe_route_entrypoints)
Write-Host ''
