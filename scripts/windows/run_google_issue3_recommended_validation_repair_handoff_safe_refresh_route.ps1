...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-handoff-safe-refresh-route.json'
}
...
if ($summaryExistsAfterRepairChain) {
    ...
    $handoffSafeRefreshRouteStep = New-SkippedStep -Name 'handoff-safe-refresh-route' -ScriptPath $handoffSafeRefreshRouteScript -Arguments $handoffSafeRefreshRouteArguments -Reason 'The combined repair chain did not leave a current summary artifact, so the safe handoff refresh-route check had no replay state to inspect.' -RecommendedCommand $repairChainCommand -RecommendedGuideCommand $summaryGuideCommand -NextFocus 'Get the broader issue #3 validation chain to leave a fresh summary artifact before checking the safe handoff refresh route.' -NextArtifactToOpen $ArtifactPath
    $steps.Add($handoffSafeRefreshRouteStep) | Out-Null
}
...
if (-not $summaryExistsAfterRepairChain) {
    ...
    $reason = 'The broader issue #3 validation repair chain did not leave a summary artifact, so the follow-up handoff safe refresh-route check could not continue.'
} elseif (-not $repairChainStep.success) {
    $status = 'repair-chain-failed'
    $reason = 'The broader issue #3 validation repair chain did not finish cleanly, so the next replay still needs direct repair-chain attention before the bounded handoff refresh route is trusted.'
} elseif (-not $handoffSafeRefreshRouteStep.success) {
    $status = 'handoff-safe-refresh-route-check-failed'
    $reason = 'The validation repair chain completed, but the handoff safe refresh-route helper did not finish cleanly, so the next replay should stay on the saved repair artifact first.'
} elseif ($handoffSafeRefreshRouteStep.status -eq 'ready-for-handoff') {
    $status = 'ready-for-handoff'
    $reason = 'The broader validation repair chain completed, and the handoff safe refresh-route helper says the narrower issue #3 handoff path is ready for the next Windows replay.'
} elseif ($handoffSafeRefreshRouteStep.status -eq 'refresh-status-safe-follow-up-ready') {
    $status = 'refresh-follow-up-ready'
    $reason = 'The broader validation repair chain completed, and the handoff safe refresh-route helper reopened the safe refresh checkpoint successfully for the next narrowed replay step.'
} else {
    $status = 'follow-up-needed'
    $reason = Get-FirstNonEmptyValue -Values @(
        if ($handoffSafeRefreshRouteStep) { $handoffSafeRefreshRouteStep.reason },
        if ($repairChainStep) { $repairChainStep.reason },
        'The broader validation repair chain and handoff safe refresh-route check completed, but the next issue #3 replay still needs the follow-up command reported by the saved artifacts.'
    )
}
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    ...
    if (-not $summaryExistsAfterRepairChain) { 'Regenerate the issue #3 recommended validation summary before asking the handoff safe refresh-route helper to narrow the next replay.' }
)
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation repair plus handoff safe refresh route'
    purpose = 'Run the broader issue #3 validation repair chain, then immediately continue through the newer handoff safe refresh route so the next Windows replay can trust the latest bounded checkpoint in one step.'
...
if ($Json) {
    ...
    if (@('repair-chain-no-summary', 'repair-chain-failed', 'handoff-safe-refresh-route-check-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation repair plus handoff safe refresh route'
Write-Host ''
