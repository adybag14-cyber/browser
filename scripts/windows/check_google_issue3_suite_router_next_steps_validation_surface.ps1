...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_next_steps_validation_surface.ps1' -Purpose "Replay-discovery handoff keeps the suite-router next-step fail-fast checker visible before the executable matrix is reused."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_next_steps.ps1' -Purpose "Replay-discovery handoff keeps the executable suite-router next-step matrix visible after the issue-specific Google attached-html bridge is reopened."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md" -Snippet '- `docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_COMPANION_NOTES.md` for the shortest written map of which top-level attached-page notes should stay nearby' -Purpose "The suite-router next-steps note keeps the top-level attached companion-note map visible beside the catalog-side attached-page route."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_next_steps.ps1" -Snippet "command = Format-HelperCommand -ScriptName 'show_google_issue3_google_attached_html_entrypoint.ps1' -Arguments $bundleArguments" -Purpose "Suite-router matrix command output keeps the google-attached-html branch executable through the issue-specific entrypoint helper."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_next_steps.ps1" -Snippet 'Write-Host (("Top-level attached companions:{0}") -f (" $($helper.top_level_attached_html_companion_notes_path)"))' -Purpose "Printed suite-router output keeps the top-level attached companion-note map visible beside the other attached-page references.")
)
