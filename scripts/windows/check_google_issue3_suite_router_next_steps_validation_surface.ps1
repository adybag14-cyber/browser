...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_CATALOG_QUICKSTART.md" -Kind "file" -Purpose "Top-level attached-html catalog quickstart note that should remain visible when the compact next-step matrix widens toward the suite-catalog route."),
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_CATALOG_ENTRYPOINTS.md" -Kind "file" -Purpose "Suite-catalog guide note that the compact next-step matrix should keep nearby when attached-page follow-up widens back into the broader catalog route."),
    ...
    (New-ValidationReference -Path "docs/ISSUE3_VALIDATION_ROUTER_ATTACHED_HTML_QUICKSTART.md" -Kind "file" -Purpose "Validation-router attached-html quickstart note that should stay visible beside the compact next-step matrix when the replay widens back into the broader attached-page ladder."),
