...
$entrypoint = [ordered]@{
    ...
    purpose = 'Keep the broader Windows full-use attached-page route and the newer top-level attached-page catalog quickstart visible on one compact helper before the replay narrows into the suite-catalog attached-page bridge, the shorter attached-page shortcut, the pinned bundle-first path, or the wrapper-heavy safe-route map.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Use this helper when docs/WINDOWS_FULL_USE.md has already narrowed the next replay to attached localhost follow-up and you want the top-level attached-page catalog quickstart reprinted before the route drops into the suite-catalog attached-page bridge.',
        'Use attached_html_change_area_quickstart after the Windows full-use route when the broader attached-page router surface should stay visible before the compact top-level quickstart narrows the route again.',
        'Use top_level_attached_html_quickstart as the default next helper when no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first, because it keeps the shortest top-level attached-page bridge visible before the broader top-level attached-page bridge, the top-level attached-page catalog quickstart, the suite-router attached-page quickstart, the suite-catalog attached-page bridge, the shorter attached-page shortcut, replay shortcuts, or the safe-route map.',
        ...
        'Keep the Windows runbook, the Windows full-use attached-page route note, the top-level attached-page catalog quickstart note, the suite-catalog attached-page bridge note, and the Windows replay quickstart nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing whether to reopen the top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, replay shortcuts, the next-step matrix, or the safe-route map.'
} else {
    'No pinned bundle inputs, saved summary, or non-default repo root are in play yet, so jump straight from the Windows full-use route into the compact top-level attached-page quickstart before widening into the top-level catalog quickstart.'
}
...
Write-Host (("  3. Google attached HTML:  {0}") -f $entrypoint.top_level_commands.google_attached_html_change_area)
Write-Host (("  4. Attached bundle:       {0}") -f $entrypoint.top_level_commands.attached_bundle_change_area)
Write-Host (("  5. Change-area bridge:    {0}") -f $entrypoint.helper_commands.attached_html_change_area_quickstart)
Write-Host (("  6. Top-level quickstart:  {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host (("  7. Top-level bridge:      {0}") -f $entrypoint.helper_commands.top_level_attached_html_entrypoint)
Write-Host (("  8. Catalog quickstart:    {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host (("  9. Router quickstart:     {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host ((" 10. Catalog bridge:        {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
Write-Host ((" 11. Google attached:       {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
...
Write-Host ((" 17. Safe-route map:        {0}") -f $entrypoint.helper_commands.safe_route_entrypoints)
Write-Host ''
