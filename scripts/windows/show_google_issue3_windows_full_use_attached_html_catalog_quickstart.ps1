...
$entrypoint = [ordered]@{
    ...
    notes = @(
        'Use this helper when docs/WINDOWS_FULL_USE.md has already narrowed the next replay to attached localhost follow-up and you want the route-level surface check, the Windows-to-validation-router bridge, and the replay-side attached-page quickstart reprinted beside the newer top-level attached-page catalog quickstart.',
        ...
        'Keep the Windows runbook, the Windows full-use attached-page route note, the Windows validation-router attached-page bridge note, the Windows replay attached-page quickstart note, the top-level attached-page catalog quickstart note, the suite-catalog attached-page bridge note, and the Windows replay quickstart nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'No pinned bundle inputs, saved summary, or non-default repo root are in play yet, so jump straight from the Windows full-use route into the replay-side attached-page quickstart before widening into the top-level attached-page quickstart and the top-level catalog quickstart.'
}
...
Write-Host (("  8. Replay quickstart:     {0}") -f $entrypoint.helper_commands.windows_replay_attached_html_quickstart)
Write-Host (("  9. Top-level quickstart:  {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host ((" 10. Top-level bridge:      {0}") -f $entrypoint.helper_commands.top_level_attached_html_entrypoint)
Write-Host ((" 11. Catalog quickstart:    {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host ((" 12. Router quickstart:     {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
