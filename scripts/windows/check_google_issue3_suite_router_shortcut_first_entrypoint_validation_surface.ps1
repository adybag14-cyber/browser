...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host ((" 13. Issue-specific check:   {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)' -Purpose "Printed shortcut-first route keeps the issue-specific Google checker visible before the narrower issue #3 bridge takes over."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host ((" 14. Google entrypoint:      {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)' -Purpose "Printed shortcut-first route keeps the issue-specific Google attached-page bridge visible before the attached-page shortcut and replay-shortcuts ladder."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host ((" 17. Replay-route shortcut:  {0}") -f $entrypoint.helper_commands.replay_route_shortcut)' -Purpose "Printed shortcut-first route keeps the replay-route shortcut bridge visible beside replay shortcuts."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Use google_attached_html_entrypoint when the Google-shaped attached-page route is already confirmed and you want the shortest issue-specific bridge back into the narrower shortcut-first, replay-shortcuts, context-preserving, or bundle-aware helpers.' -Purpose "Usage notes preserve when to reopen the issue-specific Google attached-page bridge from the compact shortcut-first surface."),
