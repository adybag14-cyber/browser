...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md" -Kind "file" -Purpose "Executable suite-router next-step matrix note that should stay visible from the shortcut-first bridge."),
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_HANDOFF.md" -Kind "file" -Purpose "Compact suite-router handoff note that should stay visible from the shortcut-first bridge before replay narrows further."),
    ...
    (New-ValidationReference -Path "scripts/windows/check_google_issue3_suite_router_handoff_validation_surface.ps1" -Kind "file" -Purpose "Fail-fast checker for the wider suite-router handoff surface that should remain visible beside the shortcut-first bridge."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'suite_router_handoff = Format-HelperCommand -ScriptName ''show_google_issue3_suite_router_handoff.ps1'' -Arguments $bundleArguments' -Purpose "Shortcut-first helper keeps the wider suite-router handoff helper surfaced beside the narrower bridge family."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'replay_route = Format-HelperCommand -ScriptName ''show_google_issue3_replay_route.ps1'' -Arguments $bundleArguments' -Purpose "Shortcut-first helper keeps the broader replay-route helper surfaced beside the narrower replay-route shortcut bridge."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("  Next-step matrix:         {0}") -f $entrypoint.helper_commands.suite_router_next_steps)' -Purpose "Companion helper output keeps the executable next-step matrix visible beside the shortcut-first bridge."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("  Suite-router handoff:     {0}") -f $entrypoint.helper_commands.suite_router_handoff)' -Purpose "Companion helper output keeps the wider suite-router handoff helper visible beside the shortcut-first bridge."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("  Replay route:             {0}") -f $entrypoint.helper_commands.replay_route)' -Purpose "Companion helper output keeps the broader replay-route helper visible beside the shortcut-first bridge."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("Google flow note:        {0}") -f $entrypoint.google_attached_html_flow_note_path)' -Purpose "Printed shortcut-first note surface keeps the broader Google attached-page flow note visible beside the compact bridge."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("Google guide note:       {0}") -f $entrypoint.google_attached_html_entrypoint_note_path)' -Purpose "Printed shortcut-first note surface keeps the issue-specific Google attached-page guide note visible beside the compact bridge."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_shortcut_first_entrypoint.ps1" -Snippet 'Write-Host (("Validation chain:        {0}") -f $entrypoint.validation_chain_note_path)' -Purpose "Printed shortcut-first note surface keeps the broader validation-chain fallback visible beside the compact bridge."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_handoff_validation_surface.ps1' -Purpose "Replay quickstart keeps the suite-router handoff fail-fast checker visible before the compact handoff bridge is trusted."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_handoff.ps1' -Purpose "Replay quickstart keeps the suite-router handoff helper visible beside the shortcut-first bridge when the route widens slightly again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md" -Snippet '- `docs/ISSUE3_SUITE_ROUTER_HANDOFF.md`' -Purpose "Replay quickstart keeps the written suite-router handoff note visible as a companion reference."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_handoff_validation_surface.ps1' -Purpose "Replay-discovery handoff keeps the suite-router handoff fail-fast checker visible before the compact handoff bridge is trusted."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_handoff.ps1' -Purpose "Replay-discovery handoff keeps the suite-router handoff helper visible beside the shortcut-first bridge when the route widens slightly again."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_SUITE_ROUTER_SHORTCUT_BRIDGE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_next_steps.ps1' -Purpose "Companion bridge note keeps the executable next-step matrix visible beside the compact shortcut route."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_SUITE_ROUTER_SHORTCUT_BRIDGE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_handoff.ps1' -Purpose "Companion bridge note keeps the wider suite-router handoff helper visible beside the compact shortcut route."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_SUITE_ROUTER_SHORTCUT_BRIDGE.md" -Snippet 'docs/ISSUE3_SUITE_ROUTER_HANDOFF.md' -Purpose "Companion bridge note keeps the written suite-router handoff note visible beside the compact shortcut route.")
)
...
Write-Host "Repair the missing shortcut note, next-step or handoff companion, helper output contract, attached-page companion helper, replay-route shortcut, or safe-route fallback before trusting the compact issue #3 shortcut-first bridge."
exit 1
