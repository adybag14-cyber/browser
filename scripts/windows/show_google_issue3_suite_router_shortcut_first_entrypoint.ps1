...
$entrypoint = [ordered]@{
    ...
    notes = @(
        ...
        'Use suite_router_attached_html_quickstart when the replay has already shifted toward attached localhost compatibility follow-up and you want the adjacent suite-router attached-page bridge printed before choosing whether to widen into the broader attached-page flow helper, the broader Google-shaped attached-page guard and flow, the issue-specific Google attached-page checker, or the attached-page shortcut.',
        ...
        'Use attached_html_flow when you want the broader attached-page localhost helper reprinted directly from the compact suite-router surface before choosing between the broader Google-shaped attached-page guard and flow, the issue-specific Google attached-page checker, the issue-specific Google attached-page bridge, the attached-page shortcut, replay shortcuts, the replay-route shortcut bridge, contextual flow, attached_bundle_first, or the safe-route map.',
        ...
        'Use google_attached_html_surface_check when the current attached-page set already includes a Google-like page and you want the broader dedicated fail-fast checker reprinted before the issue-specific checker or bridge takes over.',
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between the broader attached-page flows, the broader and issue-specific Google attached-page checks, the issue-specific Google attached-page bridge, the attached-page shortcut, replay shortcuts, the replay-route shortcut bridge, replay route, the attached bundle branch, or the safe-route wrappers.'
} else {
...
Write-Host ((" 13. Issue-specific check:   {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)
Write-Host ((" 14. Google entrypoint:      {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
...
Write-Host ((" 17. Replay-route shortcut:  {0}") -f $entrypoint.helper_commands.replay_route_shortcut)
