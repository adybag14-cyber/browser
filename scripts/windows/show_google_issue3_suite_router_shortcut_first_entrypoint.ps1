...
$googleAttachedHtmlAssetClosureCommand = Format-HelperCommandWithRepoRootEnv -ScriptName 'check_attached_html_local_asset_closure.ps1' -Switches @('GoogleStyle') -RepoRootOverride $RepoRoot
...
$entrypoint = [ordered]@{
    ...
    purpose = 'Keep the shortest top-level route from the headed validation suite router into the newer replay-shortcuts helper, while surfacing the route''s own fail-fast checker, the adjacent suite-router attached-html quickstart and its fail-fast checker, the broader attached-page localhost flow, the broader Google-shaped attached-page surface check, the deeper Google-style asset-closure audit, the broader Google-shaped attached-page flow, the issue #3-specific Google attached-page checker and bridge, the issue #3-specific attached-page shortcut, and the replay-route shortcut bridge beside the pinned-bundle branch and preserving repo-root, saved-summary, and pinned bundle-input context when it is already in play.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Start with google_input_change_area when the route is already known to stay inside issue #3 and you want the same top-level helper family reprinted before dropping into suite_router_attached_html_quickstart, suite_router_attached_html_surface_check, attached_html_flow, google_attached_html_flow, google_attached_html_surface_check, google_attached_html_asset_audit, google_issue3_attached_html_surface_check, google_attached_html_entrypoint, attached_html_shortcut, replay_shortcuts, replay_route_shortcut, contextual_flow, attached_bundle_first, or the wrapper-heavy safe-route helpers.',
        ...
        'Use suite_router_attached_html_quickstart when the replay has already shifted toward attached localhost compatibility follow-up and you want the adjacent suite-router attached-page bridge printed before choosing whether to widen into the broader attached-page flow helper, the broader Google-shaped attached-page guard, the deeper Google-style asset audit, the broader Google-shaped flow, the issue-specific Google attached-page checker, or the attached-page shortcut.',
        'Use suite_router_attached_html_surface_check right after the attached-html quickstart when you want that adjacent compact suite-router attached-page surface to fail fast before you trust it as the narrower companion route.',
        'Use attached_html_flow when you want the broader attached-page localhost helper reprinted directly from the compact suite-router surface before choosing between the broader Google-shaped attached-page guard, the deeper Google-style asset audit, the broader Google-shaped flow, the issue-specific Google attached-page checker, the issue-specific Google attached-page bridge, the attached-page shortcut, replay shortcuts, the replay-route shortcut bridge, contextual flow, attached_bundle_first, or the safe-route map.',
        'Use google_attached_html_flow when the current attached inputs are already Google-shaped and you want that narrower helper printed directly from the compact suite-router surface before deciding whether to rerun the broader Google-shaped checker, rerun the deeper Google-style asset audit, rerun the issue-specific checker, narrow into the issue-specific attached-page bridge, the attached-page shortcut, replay shortcuts, the replay-route shortcut bridge, contextual_flow, attached_bundle_first, or the wrapper-heavy safe-route helpers.',
        'Use google_attached_html_surface_check when the current attached-page set already includes a Google-like page and you want the broader dedicated fail-fast checker reprinted before the asset audit, issue-specific checker, or bridge takes over.',
        ...
        'Use google_issue3_attached_html_surface_check when the replay is already narrowed to the issue #3-specific Google attached-page route and you want the narrower fail-fast checker reprinted before the issue-specific bridge takes over.',
        'Use google_attached_html_entrypoint when the Google-shaped attached-page route is already confirmed and you want the shortest issue-specific bridge back into the narrower shortcut-first, replay-shortcuts, context-preserving, or bundle-aware helpers.',
        ...
        'Use contextual_flow instead when RepoRoot or SummaryPath is already in play and you want the next surface to keep that replay context aligned before choosing between suite_router_attached_html_quickstart, attached_html_flow, google_attached_html_flow, google_attached_html_surface_check, google_attached_html_asset_audit, google_issue3_attached_html_surface_check, google_attached_html_entrypoint, attached_html_shortcut, replay_shortcuts, replay_route_shortcut, replay_route, attached_bundle_first, or the later wrapper-heavy helpers.',
        ...
        'Keep the quickstart, suite-router bridge, suite-router attached-html quickstart, suite-catalog guide, replay-route shortcut bridge, Google attached-page flow note, issue-specific Google attached-page entrypoint note, and validation-chain notes nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between the broader attached-page flows, the broader and issue-specific Google attached-page checks, the deeper Google-style asset audit, the issue-specific Google attached-page bridge, the attached-page shortcut, replay shortcuts, the replay-route shortcut bridge, replay route, the attached bundle branch, or the safe-route wrappers.'
} else {
...
Write-Host (("  8. Attached quickstart:    {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host (("  9. Attached quick check:   {0}") -f $entrypoint.helper_commands.suite_router_attached_html_surface_check)
Write-Host ((" 10. Attached-page flow:     {0}") -f $entrypoint.helper_commands.attached_html_flow)
Write-Host ((" 11. Google attached flow:   {0}") -f $entrypoint.helper_commands.google_attached_html_flow)
...
Write-Host ((" 20. Next-step matrix:       {0}") -f $entrypoint.helper_commands.suite_router_next_steps)
...
Write-Host (("  Attached quickstart:      {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host (("  Attached quick check:     {0}") -f $entrypoint.helper_commands.suite_router_attached_html_surface_check)
...
Write-Host (("  Google asset audit:       {0}") -f $entrypoint.helper_commands.google_attached_html_asset_audit)
Write-Host (("  Issue-specific check:     {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)
Write-Host (("  Google entrypoint:        {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
...
Write-Host (("Suite-router attached:   {0}") -f $entrypoint.suite_router_attached_html_quickstart_note_path)
...
Write-Host (("Google guide note:       {0}") -f $entrypoint.google_attached_html_entrypoint_note_path)
