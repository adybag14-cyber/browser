...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_HANDOFF.md" -Kind "file" -Purpose "Compact suite-router handoff note that higher-level replay notes should keep visible before the replay-route shortcut surface narrows further."),
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md" -Kind "file" -Purpose "Executable suite-router next-steps note that higher-level replay notes should keep visible before the replay-route shortcut surface narrows further."),
    ...
    (New-ValidationReference -Path "docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_PROOF_ENTRYPOINT.md" -Kind "file" -Purpose "Proof-entry note that should stay discoverable from the replay-route bundle-first bridge once the pinned bundle route narrows into fixed-list proof."),
    ...
    (New-ValidationReference -Path "scripts/windows/check_google_issue3_suite_router_next_steps_validation_surface.ps1" -Kind "file" -Purpose "Fail-fast checker for the executable suite-router next-step matrix that higher-level replay notes should keep visible before the replay-route shortcut surface narrows further."),
    ...
    (New-ValidationReference -Path "scripts/windows/check_google_issue3_suite_router_handoff_validation_surface.ps1" -Kind "file" -Purpose "Fail-fast checker for the compact suite-router handoff that higher-level replay notes should keep visible before the replay-route shortcut surface narrows further."),
    (New-ValidationReference -Path "scripts/windows/show_google_issue3_suite_router_handoff.ps1" -Kind "file" -Purpose "Compact suite-router handoff helper surfaced from the replay-route shortcut surface before the narrower helper branch takes over."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md" -Snippet '- `docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md`' -Purpose "Windows replay quickstart keeps the written suite-router next-steps note visible as a companion reference."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_next_steps_validation_surface.ps1' -Purpose "Replay discovery handoff keeps the suite-router next-step checker visible before the executable branch matrix is trusted again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_next_steps.ps1' -Purpose "Replay discovery handoff keeps the executable suite-router next-step matrix visible before the replay-route shortcut branch narrows again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_handoff_validation_surface.ps1' -Purpose "Replay discovery handoff keeps the compact suite-router handoff checker visible before the replay-route shortcut branch narrows again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_handoff.ps1' -Purpose "Replay discovery handoff keeps the compact suite-router handoff helper visible before the replay-route shortcut branch narrows again."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet '- `docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md`' -Purpose "Replay discovery handoff keeps the written suite-router next-steps note visible as a companion reference."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_ROUTE_BUNDLE_FIRST_BRIDGE.md" -Snippet '- `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_PROOF_ENTRYPOINT.md`' -Purpose "Replay-route bundle-first bridge keeps the read-first proof note visible in its nearby companion list once the pinned bundle route narrows into proof collection."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_ROUTE_BUNDLE_FIRST_BRIDGE.md" -Snippet 'Keep `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_SUITE_SURFACE.md`, `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_QUICKSTART.md`, and `docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_PROOF_ENTRYPOINT.md` nearby' -Purpose "Replay-route bundle-first bridge keeps the proof note visible beside the suite-surface and quickstart companions in its bundle-first helper guidance."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'attached_html_target_bundle_proof_note_path = ''docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_PROOF_ENTRYPOINT.md''' -Purpose "Replay-route shortcut helper keeps the proof note wired into its nearby written route surface."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_REFERENCE.md plus docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_PROOF_ENTRYPOINT.md reopened first' -Purpose "Replay-route shortcut helper reminds bundle-pinned replays to reopen both the bundle reference and the proof note before narrowing further."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'attached-html target bundle reference, attached-html target bundle proof note, validation-chain, and Windows runbook notes nearby' -Purpose "Replay-route shortcut helper keeps the proof note in its nearby companion-note summary."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'Write-Host (("Replay discovery:      {0}") -f $entrypoint.replay_discovery_note_path)' -Purpose "Replay-route shortcut helper prints the replay-discovery handoff note directly from the compact replay-route surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'Write-Host (("Replay shortcut note:  {0}") -f ''docs/ISSUE3_REPLAY_ROUTE_SHORTCUT_BRIDGE.md'')' -Purpose "Replay-route shortcut helper prints the written replay-route shortcut note directly from the compact replay-route surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'Write-Host (("Bundle reference:      {0}") -f $entrypoint.attached_html_target_bundle_reference_note_path)' -Purpose "Replay-route shortcut helper prints the pinned bundle reference note directly from the compact replay-route surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_replay_route_shortcut_entrypoint.ps1" -Snippet 'Write-Host (("  Bundle proof note:     {0}") -f $entrypoint.attached_html_target_bundle_proof_note_path)' -Purpose "Replay-route shortcut helper prints the proof note directly from the compact replay-route surface."),
...
Write-Host "Repair the missing replay-route note, higher-level replay companion, suite-router handoff or next-step surface, helper output contract, attached-page helper, replay-side bridge, bundle-aware follow-up, proof-note companion, or safe-route companion before trusting this compact issue #3 replay-route branch."
exit 1