...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-artifact-bundle-safe.json'
}
...
$artifactBundleSafeArguments = @('-SummaryPath', $SummaryPath, '-Json')
...
if ($summaryExistsAfterRepairChain) {
    $artifactBundleSafeStep = Invoke-ScriptStep -Name 'artifact-bundle-safe-status' -ScriptPath $artifactBundleSafeScript -Arguments $artifactBundleSafeArguments -ExpectJson
    $steps.Add($artifactBundleSafeStep) | Out-Null
} else {
    $artifactBundleSafeStep = New-SkippedStep -Name 'artifact-bundle-safe-status' -ScriptPath $artifactBundleSafeScript -Arguments $artifactBundleSafeArguments -Reason 'The combined repair chain did not leave a current summary artifact, so the safe artifact-bundle check had no replay state to inspect.' -RecommendedCommand $repairChainCommand -RecommendedGuideCommand $summaryGuideCommand -NextFocus 'Get the broader issue #3 validation chain to leave a fresh summary artifact before checking the artifact-bundle safe gate.' -NextArtifactToOpen $ArtifactPath
    $steps.Add($artifactBundleSafeStep) | Out-Null
}
...
if (-not $summaryExistsAfterRepairChain) {
    ...
    $reason = 'The broader issue #3 validation repair chain did not finish cleanly, so the next replay still needs direct repair-chain attention before the artifact-bundle path is trusted.'
} elseif (-not $artifactBundleSafeStep.success) {
    $status = 'artifact-bundle-safe-check-failed'
    $reason = 'The validation repair chain completed, but the safe artifact-bundle helper did not finish cleanly, so the next replay should stay on the saved repair artifact first.'
} elseif ($artifactBundleSafeStep.status -eq 'safe-to-run-existing-helper') {
    $status = 'ready-for-artifact-bundle'
    $reason = 'The broader validation repair chain completed, and the safe artifact-bundle helper says the raw issue #3 artifact-bundle helper is ready for the next Windows replay.'
} else {
    $status = 'follow-up-needed'
    $reason = Get-FirstNonEmptyValue -Values @(
        if ($artifactBundleSafeStep) { $artifactBundleSafeStep.reason },
        if ($repairChainStep) { $repairChainStep.reason },
        'The broader validation repair chain and safe artifact-bundle check completed, but the next issue #3 replay still needs the follow-up command reported by the saved artifacts.'
    )
}

$recommendedCommand = Get-FirstNonEmptyValue -Values @(
    if ($artifactBundleSafeStep) { $artifactBundleSafeStep.recommended_command },
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    if ($artifactBundleSafeStep) { $artifactBundleSafeStep.next_focus },
...
$nextArtifactToOpen = Get-FirstNonEmptyValue -Values @(
    if ($artifactBundleSafeStep) { $artifactBundleSafeStep.next_artifact_to_open },
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation repair plus safe artifact bundle'
    purpose = 'Run the broader issue #3 validation repair chain, then immediately confirm whether the safe artifact-bundle checkpoint is clear enough for the next Windows replay to trust the raw artifact-bundle helper.'
    ...
    artifact_bundle_safe_status = $artifactBundleSafeStep.status
    recommended_command = $recommendedCommand
    recommended_guide_command = $recommendedGuideCommand
...
if ($Json) {
    ...
    if (@('repair-chain-no-summary', 'repair-chain-failed', 'artifact-bundle-safe-check-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation repair plus safe artifact bundle'
Write-Host ''
...
Write-Host ("Artifact bundle safe: {0}" -f $report.artifact_bundle_safe_status)
Write-Host ''
