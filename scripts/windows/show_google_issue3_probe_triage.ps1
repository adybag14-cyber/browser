...
$guide = [ordered]@{
    ...
    purpose = 'Route the newer issue #3 manifest, artifact-bundle audit, title, reduced-home submit-path, and shared Enter-order outputs into the next smallest headed validation step before widening back out to attached HTML or live Google replay.'
    ...
    triage_order = @(
        ...
        '2. If the manifest, guide, or boundary handoff looks stale or incomplete, run the saved artifact-bundle audit before trusting older helper output.',
        '3. Read the saved phase-boundary helper when you need the exact last-pass / first-fail handoff after the manifest points at a failing checkpoint.',
        '4. Start with the title wrapper when the failure point is still unclear and you need the earliest bounded real-surface markers.',
        '5. Move to the reduced-home submit-path ladder only after focus and typed text look stable.',
        '6. Use the shared Enter-order gate when Google-shaped probes look green but keydown, keypress, and submit ordering still needs proof.',
        '7. Widen to attached HTML or live Google replay only after the bounded slices and saved artifact handoff agree on the failing stage.'
    )
    quick_diagnosis = @(
        ...
        'If the saved artifact bundle reports missing-core-artifacts, missing-phase-artifacts, or stale-cross-references, refresh the current helper chain before trusting older guide or boundary output.',
    ...
    next_step = 'Run the manifest helper first. If it points to an incomplete or stale saved handoff, run the artifact-bundle audit next; otherwise open its next_artifact_to_open path and rerun only the narrower bounded helper or phase that matches that saved boundary.'
}
