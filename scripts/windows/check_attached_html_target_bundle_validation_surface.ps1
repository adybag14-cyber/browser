...
if ($missing.Count -eq 0) {
    Write-Host "Attached HTML target-bundle validation surface is intact, including the bundle reference note, the bundle quickstart, the proof-entry note, the compact bundle suite-surface note and helper, the bundle-first helper, the proof-entrypoint surface checker, the bundle proof entrypoint, the Google attached-page companion note, the issue #3 next-step matrix and replay-shortcuts follow-up helpers, the pinned manual checklist, the reusable local fixture probe, and the proof-note surfacing contract on the bundle suite, bundle-first helper, and quickstart surfaces."
    exit 0
}

Write-Host ("Missing {0} attached HTML target-bundle validation path or source-contract check(s)." -f $missing.Count)
Write-Host "Repair the missing guide, reference note, quickstart, proof-entry note, compact suite-surface note or helper, bundle-first helper, proof-entrypoint surface checker, bundle proof entrypoint, Google attached-page companion note, issue #3 next-step or replay-shortcuts helper, checklist, reusable fixture probe surface, or bundle proof-note surfacing contract before trusting the bundle-pinned localhost route."
exit 1
