...
$route = [ordered]@{
    ...
    purpose = 'Bridge the high-level headed validation catalog, the bounded Google flow helper, the attached three-page compatibility bundle branch, and the current safe-route replay wrappers in one place while preserving repo-root and summary-path context across the printed handoff commands.'
    ...
    notes = @(
        ...
        'When LIGHTPANDA_REPO_ROOT or a saved SummaryPath is already guiding the replay, the emitted read-first, attached-bundle, safe-route, and runner-next-step commands preserve that same context so the replay route stays aligned with the newer entrypoint helpers.'
    )
}
