...
$helper = [ordered]@{
    issue = 'Google issue #3 suite-catalog replay attached HTML handoff'
    purpose = 'Print the direct bridge from the suite-catalog entry surface into the replay-side attached-html ladder so future issue #3 reruns can keep the replay-side surface check, the replay attached-html quickstart, the validation-router quickstart, and the top-level attached-page helpers visible before dropping into shortcuts or the safe-route map.'
    repo_root = $RepoRoot
    summary_path = $SummaryPath
    explicit_input_path_count = if ($InputPath) { @($InputPath).Count } else { 0 }
    note_path = 'docs/ISSUE3_SUITE_CATALOG_REPLAY_ATTACHED_HTML_HANDOFF.md'
    suite_catalog_note_path = 'docs/ISSUE3_SUITE_CATALOG_ENTRYPOINTS.md'
    windows_replay_attached_html_note_path = 'docs/ISSUE3_WINDOWS_REPLAY_ATTACHED_HTML_QUICKSTART.md'
    ...
    notes = @(
        'Start here when the suite-catalog surface is already open and you want a smaller direct bridge into the replay-side attached-html ladder without reopening the wider router notes first.',
        'Use windows_replay_attached_html_quickstart as the default next helper when no pinned bundle inputs, non-default repo root, or saved summary need to take precedence, because it keeps the narrower replay-side attached-html route visible before you widen into validation-router, suite-catalog-to-top-level catalog, top-level attached-page, or shortcut helpers.',
        'Use contextual_flow when RepoRoot, SummaryPath, or pinned InputPath values already matter and you want the next helper surface to preserve that replay context before narrowing again.',
        'Use attached_bundle_first when explicit InputPath values are already pinned to the known three-page compatibility bundle and the replay should stay on that locked route before widening again.',
        'Keep the suite-catalog guide, the replay-side attached-html quickstart note, the validation-router attached-html quickstart note, the suite-catalog-to-top-level catalog quickstart note, the suite-catalog attached-html bridge note, and the top-level attached-page quickstart notes nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between the replay-side attached-html quickstart, the validation-router quickstart, or the later shortcut and safe-route helpers.'
} else {
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so reopen the replay-side attached-html quickstart first and keep the narrower attached-page ladder visible before widening again.'
}
...
Write-Host 'Google issue #3 suite-catalog replay attached HTML handoff'
Write-Host ''
if ($helper.repo_root) {
...
Write-Host ''
Write-Host 'Replay-side bridge:'
Write-Host (("  Suite catalog surface:       {0}") -f $helper.commands.suite_catalog_entrypoints)
...
Write-Host (("  Replay attached quickstart:  {0}") -f $helper.commands.windows_replay_attached_html_quickstart)
Write-Host (("  Validation-router quick:     {0}") -f $helper.commands.validation_router_attached_html_quickstart)
Write-Host (("  Catalog quickstart:          {0}") -f $helper.commands.suite_catalog_top_level_attached_html_catalog_quickstart)
...
Write-Host ''
Write-Host (("Handoff note:                 {0}") -f $helper.note_path)
Write-Host (("Suite-catalog guide:          {0}") -f $helper.suite_catalog_note_path)
Write-Host (("Replay attached note:         {0}") -f $helper.windows_replay_attached_html_note_path)
Write-Host (("Validation-router note:       {0}") -f $helper.validation_router_attached_html_note_path)
Write-Host (("Catalog quickstart note:      {0}") -f $helper.suite_catalog_top_level_attached_html_catalog_quickstart_note_path)
