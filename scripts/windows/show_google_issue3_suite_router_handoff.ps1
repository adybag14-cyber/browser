...
$handoff = [ordered]@{
    ...
    purpose = 'Keep the higher-level suite-router entrypoints, the attached-page route, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the new shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the suite-catalog bridge, the replay-route helper, the replay-route shortcut helper, and the current issue #3 replay helpers on one command surface before the replay narrows further, without dropping pinned bundle-input context.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use suite_catalog_entrypoints when you want the exact top-level suite-router entrypoints, the attached-page route, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the current replay helpers reprinted together before narrowing further, while keeping SummaryPath and InputPath context attached when they are already pinned.',
        ...
        'Use the read-first bridge when you want the exact route from the higher-level suite router into the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the companion bundle-first branch printed in one place before reopening any longer notes.',
        'Keep replay_discovery_note_path nearby when you want the shortest written bridge from the top-level Windows validation catalog into this suite-router handoff, the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the bundle-first branch without reopening the longer validation-chain notes first.',
...
Write-Host (" 13. Replay-route shortcut:         {0}" -f $handoff.bridge_sequence.replay_route_shortcut)
Write-Host (" 14. Bundle-first branch:           {0}" -f $handoff.bridge_sequence.attached_bundle_change_area)
Write-Host ''
