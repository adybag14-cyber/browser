...
$handoff = [ordered]@{
    ...
    purpose = 'Keep the higher-level suite-router entrypoints, the attached-page route, the dedicated Google-shaped attached-page route, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the issue-specific Google attached-page bridge, the new shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the suite-catalog bridge, the replay-route helper, the replay-route shortcut helper, and the current issue #3 replay helpers on one command surface before the replay narrows further, without dropping pinned bundle-input context.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use suite_catalog_entrypoints when you want the exact top-level suite-router entrypoints, the attached-page route, the dedicated Google-shaped attached-page route, the newer suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the current replay helpers reprinted together before narrowing further, while keeping SummaryPath and InputPath context attached when they are already pinned.',
        'Use replay_shortcuts as the default follow-up after the higher-level suite router, the attached-page route, the dedicated Google-shaped attached-page route, the suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, or the suite-catalog bridge when the route is already known to stay inside issue #3 and no pinned bundle inputs, saved summary, or repo-root override need to take precedence first.',
        'Use suite_router_next_steps when you still want the explicit start-point table after the attached-page bridges, the dedicated Google attached-page bridge, or the shortcut-first bridge, or when you want the helper recommendation reprinted before deciding whether to widen into replay_route, replay_route_shortcut, stay pinned to attached_bundle_first, or reopen the safe-route map.',
        'Use replay_route when you want the slightly broader bridge after replay_shortcuts, the dedicated Google-shaped attached-page route, the suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, or the next-step matrix so the attached-bundle branch, the replay-route shortcut, the safe-route map, and repo-root-aware runner-state choices stay together before narrowing again.',
        ...
        'Use the read-first bridge when you want the exact route from the higher-level suite router into the attached-page route, the dedicated Google-shaped attached-page route, the suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the companion bundle-first branch printed in one place before reopening any longer notes.',
        'Keep replay_discovery_note_path nearby when you want the shortest written bridge from the top-level Windows validation catalog into this suite-router handoff, the attached-page route, the dedicated Google-shaped attached-page route, the suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, the replay-route shortcut helper, and the bundle-first branch without reopening the longer validation-chain notes first.',
        ...
        'Keep suite_router_bridge_note_path nearby when you want the shortest written bridge from the higher-level suite router into suite_router_shortcut_entrypoint, replay_shortcuts, suite_router_next_steps, suite_catalog_entrypoints, replay_route, replay_route_shortcut, contextual_flow, google_attached_html_entrypoint, or the attached-page route without reopening the longer Windows runbook or validation-chain notes first.',
        'Keep suite_router_attached_html_quickstart_note_path nearby when you want the written suite-router-side attached-page bridge beside the helper output before reopening the top-level attached-page helpers, the dedicated Google attached-page bridge, or the wider replay-route notes.',
        'Keep suite_catalog_entrypoint_note_path nearby when you want the shortest written bridge from the higher-level suite router into the newer suite-catalog helper order before reopening replay_shortcuts, suite_router_next_steps, replay_route, replay_route_shortcut, google_attached_html_entrypoint, or contextual_flow.',
        ...
        'Use safe_route_entrypoints only after the higher-level suite router, the attached-page route, the dedicated Google-shaped attached-page route, the suite-router attached-page quickstart, the issue-specific Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, next-step matrix, suite-catalog bridge, replay-route helper, replay-route shortcut helper, contextual_flow surface, or attached bundle-first helper has already narrowed the replay into the current wrapper-heavy issue #3 path, and keep the same InputPath values attached when the bundle is already pinned.'
    )
}
...
$recommendedNextHelperReason = 'No explicit input paths, saved summary, or repo-root override are in play yet, so reopen the shortcut-first entrypoint next and keep the shorter issue #3 bridge visible before widening into replay shortcuts, the attached-page quickstarts, the dedicated Google attached-page bridge, the next-step matrix, the suite-catalog bridge, replay route, the replay-route shortcut, or the safe-route map.'
if ($handoff.explicit_input_path_count -gt 0) {
    ...
    $recommendedNextHelperReason = 'A non-default RepoRoot or saved SummaryPath is already in play, so reopen the context-preserving flow next and keep that replay state aligned while you choose between the suite-router attached-page quickstart, the dedicated Google attached-page bridge, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first entrypoint, replay shortcuts, next-step matrix, suite-catalog bridge, attached bundle, replay route, replay-route shortcut, safe-route map, the broader recommended runner, or the later trace and follow-up helpers.'
}
...
Write-Host ("  5. Suite-router attached quickstart: {0}" -f $handoff.bridge_sequence.suite_router_attached_html_quickstart)
Write-Host ("  6. Google attached bridge:        {0}" -f $handoff.bridge_sequence.google_attached_html_entrypoint)
Write-Host ("  7. Top-level attached quickstart: {0}" -f $handoff.bridge_sequence.top_level_attached_html_quickstart)
Write-Host ("  8. Top-level attached bridge:     {0}" -f $handoff.bridge_sequence.top_level_attached_html_entrypoint)
Write-Host ("  9. Shortcut entry:                {0}" -f $handoff.bridge_sequence.suite_router_shortcut_entrypoint)
Write-Host (" 10. Replay shortcuts:              {0}" -f $handoff.bridge_sequence.replay_shortcuts)
Write-Host (" 11. Next-step matrix:              {0}" -f $handoff.bridge_sequence.suite_router_next_steps)
Write-Host (" 12. Suite-catalog:                 {0}" -f $handoff.bridge_sequence.suite_catalog_entrypoints)
Write-Host (" 13. Google flow:                   {0}" -f $handoff.bridge_sequence.google_flow)
Write-Host (" 14. Replay route:                  {0}" -f $handoff.bridge_sequence.replay_route)
Write-Host (" 15. Replay-route shortcut:         {0}" -f $handoff.bridge_sequence.replay_route_shortcut)
Write-Host (" 16. Bundle-first branch:           {0}" -f $handoff.bridge_sequence.attached_bundle_change_area)
Write-Host ''
