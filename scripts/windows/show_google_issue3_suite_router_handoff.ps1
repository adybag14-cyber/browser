...
$handoff = [ordered]@{
    ...
    notes = @(
        ...
        'Use suite_router_next_steps when you want the current helper recommendation printed right after the higher-level suite router and Google flow before you decide whether to narrow into replay_shortcuts, widen into replay_route, stay pinned to attached_bundle_first, or reopen the safe-route map.',
        ...
        'Use the read-first bridge when you want the exact route from the higher-level suite router into the next-step matrix, the replay-route helper, the replay-shortcuts helper, and the companion bundle-first branch printed in one place before reopening any longer notes.',
        ...
        'Use safe_route_entrypoints only after the higher-level suite router, next-step matrix, replay-route helper, replay-shortcuts helper, contextual_flow surface, or attached bundle-first helper has already narrowed the replay into the current wrapper-heavy issue #3 path, and keep the same InputPath values attached when the bundle is already pinned.'
    )
}
...
if ($handoff.explicit_input_path_count -gt 0) {
    ...
    $recommendedNextHelperReason = 'A saved SummaryPath is already in play, so reopen the context-preserving flow next and keep that replay state aligned while you choose between the next-step matrix, replay-route, replay-shortcuts, the broader recommended runner, or the later attached and trace follow-up helpers.'
}
