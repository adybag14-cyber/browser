...
$handoff = [ordered]@{
    ...
    purpose = 'Keep the higher-level suite-router entrypoints, the attached-page route, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the new shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the suite-catalog bridge, and the current issue #3 replay helpers on one command surface before the replay narrows further, without dropping pinned bundle-input context.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use attached_html_change_area when the next replay is already narrowed to the attached-page compatibility route and you want that top-level branch visible before deciding whether to jump into the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first helper, or stay pinned to the bundle-first path.',
        ...
        'Use top_level_attached_html_entrypoint when the route is already clearly inside the issue-specific attached-page branch and you want the shorter top-level attached-page bridge kept visible before replay_shortcuts, the next-step matrix, suite_catalog_entrypoints, replay_route, or the safe-route map.',
        ...
        'Use suite_catalog_entrypoints when you want the exact top-level suite-router entrypoints, the attached-page route, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the compact next-step matrix, the broader Google flow helper, and the current replay helpers reprinted together before narrowing further, while keeping SummaryPath and InputPath context attached when they are already pinned.',
        'Use replay_shortcuts as the default follow-up after the higher-level suite router, the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, or the suite-catalog bridge when the route is already known to stay inside issue #3 and no pinned bundle inputs, saved summary, or repo-root override need to take precedence first.',
        'Use suite_router_next_steps when you still want the explicit start-point table after the attached-page bridges or the shortcut-first bridge, or when you want the helper recommendation reprinted before deciding whether to widen into replay_route, stay pinned to attached_bundle_first, or reopen the safe-route map.',
        'Use replay_route when you want the slightly broader bridge after replay_shortcuts, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, or the next-step matrix so the attached-bundle branch, safe-route map, and repo-root-aware runner-state choices stay together before narrowing again.',
        ...
        'Use the read-first bridge when you want the exact route from the higher-level suite router into the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, and the companion bundle-first branch printed in one place before reopening any longer notes.',
        'Keep replay_discovery_note_path nearby when you want the shortest written bridge from the top-level Windows validation catalog into this suite-router handoff, the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, the next-step matrix, the suite-catalog bridge, the broader Google flow helper, the replay-route helper, and the bundle-first branch without reopening the longer validation-chain notes first.',
        ...
        'Keep top_level_attached_html_bridge_note_path nearby when you want the written broader top-level attached-page route beside this handoff before reopening replay_shortcuts, the next-step matrix, or the wider safe-route notes.',
        ...
        'Use safe_route_entrypoints only after the higher-level suite router, the attached-page route, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first suite-router entrypoint, the shortcut-first replay helper, next-step matrix, suite-catalog bridge, replay-route helper, contextual_flow surface, or attached bundle-first helper has already narrowed the replay into the current wrapper-heavy issue #3 path, and keep the same InputPath values attached when the bundle is already pinned.'
    )
}
...
if ($handoff.explicit_input_path_count -gt 0) {
    ...
    $recommendedNextHelperReason = 'A non-default RepoRoot or saved SummaryPath is already in play, so reopen the context-preserving flow next and keep that replay state aligned while you choose between the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the shortcut-first entrypoint, replay shortcuts, next-step matrix, suite-catalog bridge, attached bundle, safe-route map, the broader recommended runner, or the later trace and follow-up helpers.'
}
...
Write-Host ("  4. Suite-router attached quickstart: {0}" -f $handoff.bridge_sequence.suite_router_attached_html_quickstart)
Write-Host ("  5. Top-level attached quickstart: {0}" -f $handoff.bridge_sequence.top_level_attached_html_quickstart)
Write-Host ("  6. Top-level attached bridge:     {0}" -f $handoff.bridge_sequence.top_level_attached_html_entrypoint)
Write-Host ("  7. Shortcut entry:                {0}" -f $handoff.bridge_sequence.suite_router_shortcut_entrypoint)
Write-Host ("  8. Replay shortcuts:              {0}" -f $handoff.bridge_sequence.replay_shortcuts)
Write-Host ("  9. Next-step matrix:              {0}" -f $handoff.bridge_sequence.suite_router_next_steps)
Write-Host (" 10. Suite-catalog:                 {0}" -f $handoff.bridge_sequence.suite_catalog_entrypoints)
Write-Host (" 11. Google flow:                   {0}" -f $handoff.bridge_sequence.google_flow)
Write-Host (" 12. Replay route:                  {0}" -f $handoff.bridge_sequence.replay_route)
Write-Host (" 13. Bundle-first branch:           {0}" -f $handoff.bridge_sequence.attached_bundle_change_area)
Write-Host ''
