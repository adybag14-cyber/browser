...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-manifest-contract-handoff-safe.json'
}
...
foreach ($helperPath in @($recommendedRunnerScript, $manifestContractRepairScript, $manifestSafeScript, $handoffSafeScript)) {
...
$summaryExistsAfterManifestSafe = Test-Path -LiteralPath $SummaryPath -PathType Leaf
$handoffSafeStep = $null
if ($summaryExistsAfterManifestSafe -and $manifestSafeStep.success -and $manifestSafeStep.status -eq 'safe-to-run-manifest-guide') {
    ...
    $handoffSafeStep = New-SkippedStep -Name 'handoff-safe-status' -ScriptPath $handoffSafeScript -Arguments $handoffSafeArguments -Reason 'The safe manifest check did not clear the narrower manifest checkpoint yet, so the safe handoff step would be premature for this replay.' -RecommendedCommand $manifestSafeStep.recommended_command -RecommendedGuideCommand $manifestSafeStep.recommended_guide_command -NextFocus $manifestSafeStep.next_focus -NextArtifactToOpen $(Get-FirstNonEmptyValue -Values @($manifestSafeStep.next_artifact_to_open, $ArtifactPath))
    $steps.Add($handoffSafeStep) | Out-Null
}
...
if (-not $summaryExistsAfterRunner) {
    ...
    $reason = 'The manifest-side checkpoints completed, but the safe handoff helper did not finish cleanly, so the next replay should stay on the saved handoff-safe artifact first.'
} elseif ($handoffSafeStep.status -eq 'safe-to-run-handoff') {
    $status = 'ready-for-handoff'
    $reason = 'The broader runner completed, the manifest contract was repaired, the safe manifest gate cleared, and the safe handoff helper says the narrower handoff guide is ready for the next Windows replay.'
} elseif (-not $runnerStep.success) {
    $status = 'runner-failed-follow-up-ready'
    $reason = 'The broader runner still failed, but it left enough saved state for the manifest and handoff safety helpers to narrow the next issue #3 follow-up step.'
} else {
    ...
    $reason = Get-FirstNonEmptyValue -Values @(
        ...
        'The broader runner, manifest repair, manifest-safe check, and handoff-safe check completed, but the next issue #3 replay still needs the follow-up command reported by the saved artifacts.'
    )
}
...
$recommendedGuideCommand = Get-FirstNonEmptyValue -Values @(
    ...
    if ($handoffSafeStep -and $handoffSafeStep.status -eq 'safe-to-run-handoff') { $handoffGuideCommand },
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    ...
    if (-not $runnerStep.success) { 'Inspect the broader runner output first, then use the manifest and handoff safety results to stay on the narrowest next replay.' }
)
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation plus manifest-contract repair and safe handoff check'
    purpose = 'Run the broader issue #3 recommended validation replay, repair the saved manifest contract, confirm the safe manifest checkpoint, and then immediately ask the safe handoff helper for the next narrowed Windows replay step.'
    ...
    summary_exists_after_manifest_safe = [bool]$summaryExistsAfterManifestSafe
    recommended_command = $recommendedCommand
    recommended_guide_command = $recommendedGuideCommand
    next_focus = $nextFocus
    next_artifact_to_open = $nextArtifactToOpen
...
if ($Json) {
    ...
    if (@('runner-failed-no-summary', 'manifest-contract-repair-failed', 'manifest-safe-check-failed', 'handoff-safe-check-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation plus manifest-contract repair and safe handoff check'
Write-Host ''
...
Write-Host ("Summary exists after manifest safe: {0}" -f $report.summary_exists_after_manifest_safe)
Write-Host ''
