...
if ($missing.Count -eq 0) {
    Write-Host 'Google issue #3 attached-pages launcher companion surface is intact, including the strict sidecar-plus-asset launch gates, the preferred-first-page Python parity, and the pinned proof-route bridge back into the three-page compatibility bundle.'
    exit 0
}

Write-Host (("Missing {0} attached-pages launcher companion path or source contract check(s).") -f $missing.Count)
Write-Host 'Repair the replay note, launcher companion helper, proof-route bridge, wrapper-backed launcher paths, preferred-first-page Python wiring, or cross-platform launcher references before trusting the attached-pages launcher companion route.'
exit 1