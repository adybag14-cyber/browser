...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'wrapper_strict_assets = Format-HelperCommand -ScriptName ''start_attached_pages_catalog.ps1'' -Arguments $wrapperArguments -Switches @(''RequireCompleteAssets'')' -Purpose "Launcher companion helper keeps the Windows strict-asset launch gate visible in the printed ladder."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'python_strict_assets = Format-PythonLauncherCommand -RepoRootOverride $resolvedRepoRoot -InputValues $InputPath -Flags @(''--require-complete-assets'')' -Purpose "Launcher companion helper keeps the cross-platform strict-asset launch gate visible in the printed ladder."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'Write-Host (("  5. Strict assets:      {0}") -f $helper.helper_commands.wrapper_strict_assets)' -Purpose "Launcher companion helper prints the Windows strict-asset launch gate beside the rest of the wrapper ladder."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'Write-Host (("  5. Strict assets:      {0}") -f $helper.helper_commands.python_strict_assets)' -Purpose "Launcher companion helper prints the cross-platform strict-asset launch gate beside the rest of the Python ladder."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/start_attached_pages_catalog.ps1" -Snippet '$launcherArgs += "--require-complete-assets"' -Purpose "Windows wrapper now forwards the strict-asset launch gate that the launcher companion surfaces beside the rest of the preflight ladder.")
)
...
Write-Host "Repair the missing note, helper output contract, wrapper-backed launcher ladder, or launcher guide before trusting the issue #3 attached-pages launcher companion route."
exit 1
