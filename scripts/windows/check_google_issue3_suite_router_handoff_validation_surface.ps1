...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_NEXT_STEPS.md" -Kind "file" -Purpose "Written suite-router next-steps note that should stay visible beside the compact handoff and higher-level replay notes."),
    ...
    (New-ValidationReference -Path "scripts/windows/check_google_issue3_suite_router_next_steps_validation_surface.ps1" -Kind "file" -Purpose "Fail-fast checker for the executable next-step matrix that higher-level replay notes should keep visible beside the handoff."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_catalog_entrypoints_validation_surface.ps1' -Purpose "Replay discovery handoff keeps the suite-catalog fail-fast checker visible before the broader suite-catalog helper is trusted again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_catalog_entrypoints.ps1' -Purpose "Replay discovery handoff keeps the suite-catalog helper visible after the suite-catalog checker is rerun."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_suite_router_next_steps_validation_surface.ps1' -Purpose "Replay discovery handoff keeps the executable next-step matrix checker visible before the compact helper chooser is reused."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_suite_router_next_steps.ps1' -Purpose "Replay discovery handoff keeps the executable next-step matrix visible after the issue-specific Google branch is reopened."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\check_google_issue3_replay_route_shortcut_validation_surface.ps1' -Purpose "Replay discovery handoff keeps the replay-route shortcut fail-fast checker visible before the shorter replay-route bridge is trusted again."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_REPLAY_DISCOVERY_HANDOFF.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\show_google_issue3_replay_route_shortcut_entrypoint.ps1' -Purpose "Replay discovery handoff keeps the replay-route shortcut helper visible after the replay-route checker is rerun."),
...
Write-Host "Repair the written suite-router handoff note, the higher-level replay companions, the executable next-step matrix surfacing, the compact handoff helper, the top-level attached-html helper, the issue-specific Google attached-html checker, the Google attached-html flow helper, the Google attached-html bridge, the replay-route shortcut, or the later-stage fallback before trusting the issue #3 suite-router handoff surface."
exit 1
