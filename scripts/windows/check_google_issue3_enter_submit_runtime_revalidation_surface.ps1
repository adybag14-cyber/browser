...
$references = @(
    ...
    (New-ValidationReference -Path "scripts/linux/check_issue3_restored_checkout_reentry_route_surface.sh" -Kind "file" -Purpose "Fail-fast restored-checkout surface checker used before helper replay trusts a restored follow-up root."),
    (New-ValidationReference -Path "scripts/linux/show_issue3_restored_checkout_reentry_route.sh" -Kind "file" -Purpose "Compact restored-checkout route printer used after the saved-browser-snapshot restore route succeeds."),
    (New-ValidationReference -Path "scripts/linux/check_issue3_saved_archive_integrity_route_surface.sh" -Kind "file" -Purpose "Fail-fast surface checker for the saved-archive-integrity route used before restored-checkout verification is trusted."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet '"docs/ISSUE3_RESTORED_CHECKOUT_REENTRY_ROUTE.md"' -Purpose "The helper keeps the restored-checkout route note on the same read-first surface."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet 'saved_browser_snapshot_route_synced = $savedBrowserSnapshotSyncedRouteCommand' -Purpose "The helper prints the synced saved-browser-snapshot route when the restored checkout should carry a copied helper surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet 'restored_checkout_surface = $restoredCheckoutRouteSurfaceCommand' -Purpose "The helper prints the restored-checkout route surface checker before restored follow-up commands are trusted."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet 'restored_checkout_route = $restoredCheckoutRouteCommand' -Purpose "The helper prints the compact restored-checkout re-entry route after the saved-browser-snapshot restore route succeeds."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet 'restored_checkout_readiness = $restoredCheckoutReadinessCommand' -Purpose "The helper prints the direct restored-checkout readiness command before saved-memory preflight begins."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet 'restored_checkout_synced_readiness = $restoredCheckoutSyncedReadinessCommand' -Purpose "The helper prints the synced-helper-surface restored-checkout readiness command for drift checks."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Saved snapshot route (sync helper surface):" -Purpose "The helper output prints the synced saved-browser-snapshot route when the restored checkout should carry a copied helper surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Restored-checkout surface:" -Purpose "The helper output prints the restored-checkout route surface checker before restored follow-up commands."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Restored-checkout route:" -Purpose "The helper output prints the compact restored-checkout route."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Restored-checkout readiness:" -Purpose "The helper output prints the direct restored-checkout readiness command."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Restored-checkout synced-helper-surface readiness:" -Purpose "The helper output prints the synced-helper-surface restored-checkout readiness command."),
