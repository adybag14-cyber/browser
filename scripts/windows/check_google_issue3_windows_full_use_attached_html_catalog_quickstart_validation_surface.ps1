...
$contentExpectations = @(
  ...
  -RequireCompleteSidecars' -Purpose "Attached-pages launcher guide keeps the wrapper-backed strict sidecar gate visible before manifest or launch steps."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_catalog_quickstart.ps1" -Snippet 'attached_pages_google_sidecar_audit = $attachedPagesGoogleSidecarAuditCommand' -Purpose "Windows-first catalog helper keeps the Google-style wrapper-backed sidecar audit wired into the helper command map."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_catalog_quickstart.ps1" -Snippet 'Write-Host ((\" 24. Google sidecars:       {0}\") -f $entrypoint.helper_commands.attached_pages_google_sidecar_audit)' -Purpose "Windows-first catalog helper keeps the printed Google-style sidecar audit aligned with the wrapper-backed launcher surface."),
