...
Write-Host "Repair the missing route note, launcher guide, launcher entrypoint, Windows wrapper, wrapper-backed sidecar audit contract, attached-html change-area bridge, catalog quickstart helper, companion top-level or suite-catalog bridge, Google-shaped fallback, bundle re-entry helper, or safe-route return script before trusting this narrower attached-page ladder."
exit 1