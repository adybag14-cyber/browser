...
$helper = [ordered]@{
    ...
    purpose = 'Print the shortest bridge from the top-level headed validation suite router into the current issue #3 top-level shortcut-first entrypoint, top-level attached HTML bridge, suite-router attached HTML quickstart, top-level attached HTML quickstart, suite-catalog attached HTML, issue-specific attached HTML suite entrypoint, attached-HTML shortcut, replay-shortcuts, attached-bundle, and safe-route helpers, while preserving repo-root, saved-summary, and pinned attached-bundle context when it already exists.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use attached_html_suite when the next replay is already narrowed to attached-page compatibility follow-up and you want that top-level route reprinted before dropping into the issue-specific helper chain.',
        'Use google_attached_html_suite when the next replay is already narrowed to the issue-specific attached-page follow-up route and you want that dedicated suite-router entrypoint visible before choosing between the top-level attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the suite-catalog attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, or the bundle-first route.',
        ...
        'Use attached_html_shortcut immediately after the top-level commands, the suite-router attached-page quickstart, the top-level attached-page bridge, or the top-level attached-page quickstart when the route is already clearly inside issue #3 and no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first.',
        'Use suite_router_next_steps when the route still needs the compact matrix before you choose between replay shortcuts, the top-level attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the suite-catalog attached-page bridge, the attached-page shortcut, contextual flow, attached bundle, or the wrapper-heavy safe path.',
        ...
        'Use safe_route_entrypoints after the shortcut-first bridge, top-level attached-page bridge, suite-router attached-page quickstart, top-level attached-page quickstart, suite-catalog attached-page bridge, next-step matrix, attached-page shortcut, or contextual flow helper when you want the current wrapper-heavy issue #3 commands and state helpers surfaced in one place.',
        'Keep the quickstart, replay quickstart shortcut bridge, suite-router entrypoint guide, suite-router shortcut bridge, suite-router attached HTML quickstart, suite-catalog attached HTML bridge, top-level attached HTML bridge, top-level attached HTML quickstart, and safe-route note nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between replay shortcuts, the top-level attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the suite-catalog attached-page bridge, the attached-page shortcut, the next-step matrix, the bundle-first route, or the safe-route helper.'
} else {
