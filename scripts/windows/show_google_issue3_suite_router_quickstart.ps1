...
$helper = [ordered]@{
    ...
    purpose = 'Print the shortest bridge from the top-level headed validation suite router into the current issue #3 validation-router attached HTML quickstart, top-level shortcut-first entrypoint, top-level attached HTML bridge, Google-shaped attached-page entrypoint, suite-router attached HTML quickstart, top-level attached HTML quickstart, top-level attached HTML catalog quickstart, suite-catalog entrypoints helper, suite-catalog attached HTML, issue-specific attached HTML shortcut, replay-shortcuts, attached-bundle, and safe-route helpers, while preserving repo-root, saved-summary, and pinned attached-bundle context when it already exists.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use google_attached_html_suite when the next replay is already narrowed to the issue-specific attached-page follow-up route and you want that dedicated suite-router entrypoint visible before choosing between the validation-router attached-page quickstart, the top-level attached-page bridge, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, or the bundle-first route.',
        ...
        'Use top_level_attached_html when the replay is already narrowed to attached-page follow-up and you want the broader top-level attached-page bridge kept visible before choosing between the validation-router attached-page quickstart, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, the bundle-first route, or the safe-route helper.',
        'Use google_attached_html_entrypoint when the route is already clearly inside the narrower Google-shaped attached-page follow-up and you want that issue-specific bridge visible before deciding whether to drop into the attached-page shortcut or widen back into replay shortcuts, the next-step matrix, or the bundle-first route.',
        ...
        'Use top_level_attached_html_quickstart when the replay is already narrowed to attached-page follow-up from the top-level suite router and you want the shorter top-level attached-page quickstart kept visible before reopening the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, replay shortcuts, the next-step matrix, the bundle-first route, or the safe-route helper.',
        'Use top_level_attached_html_catalog_quickstart when you want the shorter top-level attached-page quickstart, the suite-catalog re-entry surface, and the suite-catalog attached-page bridge kept visible together before the replay narrows into the attached-page shortcut, replay shortcuts, the next-step matrix, the bundle-first route, or the safe-route helper.',
        'Use suite_catalog_entrypoints when the broader suite-catalog re-entry surface should stay visible beside the compact suite-router bridge before the route narrows into the attached-page shortcut, replay shortcuts, the next-step matrix, the bundle-first route, or the safe-route helper.',
        ...
        'Use attached_html_shortcut immediately after the top-level commands, the validation-router attached-page quickstart, the suite-router attached-page quickstart, the top-level attached-page bridge, the Google-shaped attached-page entrypoint, the top-level attached-page quickstart, or the top-level attached-page catalog quickstart when the route is already clearly inside issue #3 and no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first.',
        'Use suite_router_next_steps when the route still needs the compact matrix before you choose between replay shortcuts, the validation-router attached-page quickstart, the top-level attached-page bridge, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, contextual flow, attached bundle, or the wrapper-heavy safe path.',
        ...
        'Use safe_route_entrypoints after the shortcut-first bridge, validation-router attached-page quickstart, top-level attached-page bridge, the Google-shaped attached-page entrypoint, suite-router attached-page quickstart, top-level attached-page quickstart, top-level attached-page catalog quickstart, suite-catalog entrypoints helper, suite-catalog attached-page bridge, next-step matrix, attached-page shortcut, or contextual flow helper when you want the current wrapper-heavy issue #3 commands and state helpers surfaced in one place.',
        'Keep the quickstart, replay quickstart shortcut bridge, suite-router entrypoint guide, suite-router shortcut bridge, validation-router attached HTML quickstart note, suite-router attached HTML quickstart, suite-catalog entrypoints guide, suite-catalog attached HTML bridge, Google attached HTML flow note, top-level attached HTML bridge, top-level attached HTML quickstart, top-level attached HTML catalog quickstart, and safe-route note nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between replay shortcuts, the validation-router attached-page quickstart, the top-level attached-page bridge, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, the next-step matrix, the bundle-first route, or the safe-route helper.'
} else {
...
Write-Host (("  Google attached:      {0}") -f $helper.commands.google_attached_html_entrypoint)
...
Write-Host (("  Catalog entrypoints:  {0}") -f $helper.commands.suite_catalog_entrypoints)
...
Write-Host (("Catalog guide:         {0}") -f $helper.suite_catalog_entrypoints_note_path)
