...
$helper = [ordered]@{
    ...
    notes = @(
        ...
        'Use attached_html_shortcut immediately after the top-level commands, the validation-router attached-page quickstart, the suite-router attached-page quickstart, the top-level attached-page bridge, the dedicated Google attached-page flow helper, the Google-shaped attached-page entrypoint, the top-level attached-page quickstart, or the top-level attached-page catalog quickstart when the route is already clearly inside issue #3 and no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first.',
        'Use suite_router_next_steps when the route still needs the compact matrix before you choose between replay shortcuts, the validation-router attached-page quickstart, the top-level attached-page bridge, the dedicated Google attached-page flow helper, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, contextual flow, attached bundle, or the wrapper-heavy safe path.',
        ...
        'Use safe_route_entrypoints after the shortcut-first bridge, validation-router attached-page quickstart, top-level attached-page bridge, the dedicated Google attached-page flow helper, the Google-shaped attached-page entrypoint, suite-router attached-page quickstart, top-level attached-page quickstart, top-level attached-page catalog quickstart, suite-catalog entrypoints helper, suite-catalog attached-page bridge, next-step matrix, attached-page shortcut, or contextual flow helper when you want the current wrapper-heavy issue #3 commands and state helpers surfaced in one place.',
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between replay shortcuts, the validation-router attached-page quickstart, the top-level attached-page bridge, the dedicated Google attached-page flow helper, the Google-shaped attached-page entrypoint, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog entrypoints helper, the suite-catalog attached-page bridge, the attached-page shortcut, the next-step matrix, the bundle-first route, or the safe-route helper.'
} else {
