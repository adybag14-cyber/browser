...
function Get-GoogleAttachedHtmlValidationMetadata {
    param(
...
if ($Json) {
    switch ($PSCmdlet.ParameterSetName) {
        "PageRoot" {
            $summaryJson = (& $runner @arguments -PageRoot $resolvedPageRoot) -join [Environment]::NewLine
        }
        default {
            $summaryJson = (& $runner @arguments -InputPath $resolvedInputPath) -join [Environment]::NewLine
        }
    }
