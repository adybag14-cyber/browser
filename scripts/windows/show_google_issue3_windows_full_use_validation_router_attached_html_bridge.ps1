...
$helper = [ordered]@{
    ...
    purpose = 'Print the shortest bridge from the Windows full-use attached-page route into the validation-router attached-html quickstart, while keeping the attached-html change-area route, the top-level attached-page quickstarts, the suite-router attached-page quickstart, and the replay shortcuts visible for the next narrow replay step.'
    repo_root = $RepoRoot
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay state aligned before choosing whether to reopen the validation-router quickstart, the change-area quickstart, the compact top-level attached-page route, or the replay shortcuts.'
} else {
...
Write-Host (("  5. Top-level quickstart:   {0}") -f $helper.commands.top_level_attached_html_quickstart)
Write-Host (("  6. Catalog quickstart:     {0}") -f $helper.commands.top_level_attached_html_catalog_quickstart)
Write-Host (("  7. Router quickstart:      {0}") -f $helper.commands.suite_router_attached_html_quickstart)
Write-Host (("  8. Attached shortcut:      {0}") -f $helper.commands.attached_html_shortcut)
Write-Host (("  9. Replay shortcuts:       {0}") -f $helper.commands.replay_shortcuts)
Write-Host ''
