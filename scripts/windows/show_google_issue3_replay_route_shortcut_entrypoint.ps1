...
$replayShortcutsWindowsReplayAttachedHtmlBridgeCommand = Format-HelperCommand -ScriptName 'show_google_issue3_replay_shortcuts_windows_replay_attached_html_bridge.ps1' -Arguments $bundleArguments
...
$entrypoint = [ordered]@{
    ...
    notes = @(
        ...
        'Use replay_route_shortcut_surface_check before trusting this compact replay-route branch after branch moves or from another checkout, because it fails fast on missing replay-route notes, attached-page helpers, replay-side bridges, bundle-aware follow-up commands, and safe-route companions.',
        ...
        'Use top_level_attached_html_entrypoint, top_level_attached_html_quickstart, top_level_attached_html_catalog_quickstart, or top_level_shortcut_first_entrypoint when the replay-route shortcut still needs the broader top-level attached-page route visible before the route collapses back to the issue-specific shortcut chain, replay shortcuts, the next-step matrix, or the safe-route map.',
        ...
        'Use windows_replay_attached_html_surface_check before trusting the narrower Windows replay attached-page ladder from another checkout or after branch moves, because it fails fast on missing replay-side notes, helper scripts, and downstream attached-page surfaces.',
        ...
        'Use attached_html_target_bundle_suite_surface when the current replay is already close to the known three-page compatibility bundle and you want the compact suite-level bundle surface printed before the narrower attached_bundle_first helper or the wider safe-route map.',
        ...
        'Keep the replay-discovery, suite-router bridge, suite-catalog guide, Google attached-page flow note, replay-shortcuts Windows replay attached-page bridge note, Windows replay attached-page quickstart note, top-level attached-page quickstart note, top-level attached-page bridge note, top-level attached-page catalog quickstart note, top-level shortcut-first note, attached-html target bundle reference, attached-html target-bundle suite-surface note, attached-html target bundle proof note, validation-chain, and Windows runbook notes nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_helper_reason = if ($entrypoint.recommended_next_helper_key -eq 'attached_bundle_first') {
    ...
    'No explicit bundle inputs, non-default repo root, or saved summary are already pinned, so jump straight from replay route to the smaller attached-page shortcut helper and keep the shorter issue #3 bridge visible from there.'
}
...
Write-Host (("  2. Shortcut check:    {0}") -f $entrypoint.top_level_commands.replay_route_shortcut_surface_check)
Write-Host (("  3. Attached HTML:     {0}") -f $entrypoint.top_level_commands.attached_html_change_area)
Write-Host (("  4. Attached bundle:   {0}") -f $entrypoint.top_level_commands.attached_bundle_change_area)
Write-Host (("  5. Attached shortcut: {0}") -f $entrypoint.top_level_commands.attached_html_shortcut)
Write-Host (("  6. Shortcut helper:   {0}") -f $entrypoint.top_level_commands.suite_router_shortcut_entrypoint)
Write-Host (("  7. Replay shortcuts:  {0}") -f $entrypoint.helper_commands.replay_shortcuts)
...
Write-Host ((" 12. Top-level quick:   {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host ((" 13. Top-level catalog: {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host ((" 14. Top-level short:   {0}") -f $entrypoint.helper_commands.top_level_shortcut_first_entrypoint)
Write-Host ((" 15. Next-step matrix:  {0}") -f $entrypoint.helper_commands.suite_router_next_steps)
Write-Host ((" 16. Contextual flow:   {0}") -f $entrypoint.helper_commands.contextual_flow)
Write-Host ''
Write-Host 'Companion helpers:'
Write-Host (("  Shortcut surface:     {0}") -f $entrypoint.helper_commands.replay_route_shortcut_surface_check)
...
Write-Host (("  Top-level catalog:    {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host (("  Top-level shortcut:   {0}") -f $entrypoint.helper_commands.top_level_shortcut_first_entrypoint)
...
Write-Host (("Replay shortcut note:  {0}") -f 'docs/ISSUE3_REPLAY_ROUTE_SHORTCUT_BRIDGE.md')
...
Write-Host (("Top-level catalog:     {0}") -f $entrypoint.top_level_attached_html_catalog_quickstart_note_path)
Write-Host (("Top-level shortcut:    {0}") -f $entrypoint.top_level_shortcut_first_entrypoint_note_path)
