...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-runner-output-wiring-status-repair-safe-route.json'
}
...
foreach ($helperPath in @($runnerOutputContractRepairScript, $runnerOutputWiringSafeScript)) {
...
$runnerOutputContractReadyForWiringSafe = [bool](
    $runnerOutputContractStep.success -and
    @('updated', 'noop') -contains $runnerOutputContractStep.status
)

$runnerOutputWiringSafeStep = $null
if ($runnerOutputContractReadyForWiringSafe) {
    $runnerOutputWiringSafeStep = Invoke-JsonHelper -Name 'runner-output-wiring-safe' -ScriptPath $runnerOutputWiringSafeScript -Arguments @('-SummaryPath', $SummaryPath, '-Json')
} else {
    $runnerOutputWiringSafeStep = New-SkippedHelperStep -Name 'runner-output-wiring-safe' -ScriptPath $runnerOutputWiringSafeScript -Arguments @('-SummaryPath', $SummaryPath, '-Json') -RecommendedCommand $(Get-FirstNonEmptyValue -Values @($runnerOutputContractStep.recommended_command, $runnerOutputContractRepairCommand)) -RecommendedGuideCommand $(Get-FirstNonEmptyValue -Values @($runnerOutputContractStep.recommended_guide_command, $runnerOutputPatchTargetsSafeRouteCommand)) -NextFocus $(Get-FirstNonEmptyValue -Values @($runnerOutputContractStep.next_focus, 'Finish the bounded runner-output contract repair flow before reopening the safe wiring audit.')) -NextArtifactToOpen $(Get-FirstNonEmptyValue -Values @($runnerOutputContractStep.next_artifact_to_open, $SummaryPath)) -Reason 'The runner-output contract repair helper did not clear the saved outputs for the narrower safe wiring audit yet, so this wrapper kept the next replay on the repair-side guidance.'
}
...
if (-not $runnerOutputContractStep.success) {
    ...
    $reason = 'The runner-output contract repair helper did not finish cleanly, so the next Windows replay should stay on that bounded repair checkpoint before reopening the safe wiring audit.'
} elseif (-not $runnerOutputContractReadyForWiringSafe) {
    $status = 'runner-output-contract-follow-up-needed'
    $reason = Get-FirstNonEmptyValue -Values @(
        $runnerOutputContractStep.reason,
        'The runner-output contract repair helper completed, but it still recommends another bounded checkpoint before the safe wiring audit is trusted again.'
    )
} elseif (-not $runnerOutputWiringSafeStep.success) {
    $status = 'runner-output-wiring-safe-failed'
    $reason = 'The runner-output contract repair helper cleared the saved outputs, but the safe wiring audit did not finish cleanly, so the next replay should reopen that safe wiring checkpoint directly.'
} else {
    $status = 'runner-output-wiring-safe-ready'
    $reason = 'The runner-output contract repair helper cleared the saved outputs enough to reopen the safe wiring audit, and this wrapper carried the replay forward to that narrower checkpoint.'
}

$recommendedCommand = Get-FirstNonEmptyValue -Values @(
    if ($runnerOutputContractReadyForWiringSafe) { $runnerOutputWiringSafeStep.recommended_command },
    $runnerOutputContractStep.recommended_command,
    if ($runnerOutputContractReadyForWiringSafe) { $runnerOutputWiringSafeCommand },
    $runnerOutputContractRepairCommand
)
$recommendedGuideCommand = Get-FirstNonEmptyValue -Values @(
    if ($runnerOutputContractReadyForWiringSafe) { $runnerOutputWiringSafeStep.recommended_guide_command },
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    if ($runnerOutputContractReadyForWiringSafe) { $runnerOutputWiringSafeStep.next_focus },
    $runnerOutputContractStep.next_focus,
    if ($runnerOutputContractReadyForWiringSafe) { 'Use the reopened safe wiring guidance from this run before widening back out to broader issue #3 replay loops.' },
    'Keep the next replay on the bounded runner-output contract repair side until the safe wiring audit is ready again.'
)
$nextArtifactToOpen = Get-FirstNonEmptyValue -Values @(
    if ($runnerOutputContractReadyForWiringSafe) { $runnerOutputWiringSafeStep.next_artifact_to_open },
...
$report = [ordered]@{
    issue = 'Google issue #3 runner output wiring status repair safe route'
    purpose = 'Run the bounded issue #3 runner-output contract repair first, then immediately reopen the safe runner-output wiring audit when the saved summary and manifest are healthy enough.'
    ...
    runner_output_contract_ready_for_wiring_safe = [bool]$runnerOutputContractReadyForWiringSafe
    recommended_command = $recommendedCommand
    recommended_guide_command = $recommendedGuideCommand
    next_focus = $nextFocus
    next_artifact_to_open = $nextArtifactToOpen
...
if ($Json) {
    ...
    if (@('runner-output-contract-repair-failed', 'runner-output-wiring-safe-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 runner output wiring status repair safe route'
Write-Host ''
Write-Host ("Summary:   {0}" -f $report.summary_path)
Write-Host ("Artifact:  {0}" -f $report.artifact_path)
Write-Host ("Status:    {0}" -f $report.status)
Write-Host ("Repair cleared safe wiring audit: {0}" -f $report.runner_output_contract_ready_for_wiring_safe)
Write-Host ''
