...
$references = @(
    ...
    (New-ValidationReference -Path "scripts/windows/start_attached_pages_catalog.ps1" -Kind "file" -Purpose "Windows wrapper-backed sidecar-bundle audit surfaced before the broader Google-shaped surface checker and deeper local asset crawl."),
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/start_attached_pages_catalog.py" -Kind "file" -Purpose "Underlying Python launcher that the wrapper-backed sidecar audit delegates to after the Windows helper validates repo-root and input handling."),
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/attached_pages_sidecar_audit.py" -Kind "file" -Purpose "Underlying sidecar audit helper that the wrapper-backed route depends on before the deeper local asset crawl.")
)

$contentExpectations = @(
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet '[string]$PreferredInitialPage,' -Purpose "Issue-specific Google attached-html entrypoint accepts a preferred initial page so pinned bundle replay can keep the same first page while the route narrows."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet '[string]$BrowserExe,' -Purpose "Issue-specific Google attached-html entrypoint accepts a non-default headed binary so bundle-aware replay can preserve the executable choice through narrower helpers."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Add-SharedArgument -Arguments $bundleArguments -Name PreferredInitialPage -Value $PreferredInitialPage' -Purpose "Bundle-aware helper commands keep the preferred initial page pinned when the issue-specific Google attached-html route narrows into bundle surfaces."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Add-SharedArgument -Arguments $bundleArguments -Name BrowserExe -Value $BrowserExe' -Purpose "Bundle-aware helper commands keep the non-default headed binary pinned when the issue-specific Google attached-html route narrows into bundle surfaces."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Add-SharedArgument -Arguments $googleAttachedHtmlFlowArguments -Name PreferredInitialPage -Value $PreferredInitialPage' -Purpose "The broader Google attached-html flow helper preserves the preferred initial page when replay context is already pinned."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Add-SharedArgument -Arguments $googleAttachedHtmlFlowArguments -Name BrowserExe -Value $BrowserExe' -Purpose "The broader Google attached-html flow helper preserves the non-default headed binary when replay context is already pinned."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'return Format-HelperCommand -ScriptName ''start_attached_pages_catalog.ps1'' -Arguments $wrapperArguments -Switches @(''GoogleStyle'', ''AuditSidecars'')' -Purpose "The issue-specific Google attached-html entrypoint now prints the Windows wrapper-backed sidecar audit instead of a raw Python launcher command."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Write-Host ((" 10. Google attached flow: {0}") -f $entrypoint.helper_commands.google_attached_html_validation_flow)' -Purpose "Top-level bridge output keeps the broader Google attached-page flow visible before the route narrows into the shorter issue #3 helper chain."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  Google attached flow:  {0}") -f $entrypoint.helper_commands.google_attached_html_validation_flow)' -Purpose "Companion helper output keeps the broader Google attached-page flow visible from the compact entrypoint surface."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\check_attached_html_local_asset_closure.ps1 -GoogleStyle' -Purpose "Read-first entrypoint note keeps the deeper asset audit visible before the issue-specific checker takes over."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_attached_html_validation_flow.ps1' -Purpose "Read-first entrypoint note keeps the broader Google attached-page flow helper visible before the issue-specific bridge narrows into shortcuts."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_google_attached_html_entrypoint.ps1 -RepoRoot ''<repo-root>'' -SummaryPath ''<saved-summary-path>'' -InputPath ''<bundle-html-or-folder>'' -PreferredInitialPage ''<preferred-page>'' -BrowserExe ''<browser-exe>''' -Purpose "The written context-preserving route keeps the preferred page and non-default headed binary pinned on the issue-specific Google attached-html entrypoint itself."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\run_attached_html_target_bundle_validation.ps1 -InputPath ''<bundle-html-or-folder>'' -PreferredInitialPage ''<preferred-page>'' -BrowserExe ''<browser-exe>'' -Wait' -Purpose "The pinned bundle route keeps the preferred page and headed binary visible through the delegated bundle runner instead of dropping them after the issue-specific bridge.")
)
