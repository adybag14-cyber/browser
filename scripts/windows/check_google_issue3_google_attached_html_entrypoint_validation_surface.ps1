...
$references = @(
    ...
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/attached_pages_sidecar_audit.py" -Kind "file" -Purpose "Sidecar-bundle audit that should stay visible in the read-first Google attached-html route before the deeper local asset crawl.")
)
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  6. Broader surface:      {0}") -f $entrypoint.helper_commands.broader_google_attached_html_surface_check)' -Purpose "Top-level bridge output prints the broader Google attached-html surface checker before the asset audit and issue-specific checker."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  7. Asset closure:        {0}") -f $entrypoint.helper_commands.google_attached_html_asset_closure)' -Purpose "Top-level bridge output prints the deeper asset-closure audit before the issue-specific checker."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_google_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  8. Issue-specific check: {0}") -f $entrypoint.helper_commands.google_attached_html_surface_check)' -Purpose "Top-level bridge output still prints the issue-specific fail-fast checker after the broader Google-shaped checks."),
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md" -Snippet 'python .\tmp-browser-smoke\attached-pages\attached_pages_sidecar_audit.py --root ''<attached-html-root>''' -Purpose "Read-first entrypoint note keeps the lighter sidecar-bundle audit visible before the broader Google-shaped surface checker and deeper asset crawl."),
