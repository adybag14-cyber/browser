...
$helper = [ordered]@{
    issue = 'Google issue #3 validation-suite attached HTML quickstart'
    purpose = 'Print the shortest bridge from the top-level headed validation attached-HTML route into the newer issue #3 top-level attached-page quickstart, suite-router attached-page quickstart, bundle-first helper, and follow-up replay helpers while preserving repo-root, saved-summary, and pinned bundle-input context.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Start with attached_html_change_area when the top-level headed validation catalog is already narrowed to the generic attached localhost route and you want the shortest follow-up bridge into the newer issue #3 helpers.',
        'Use google_attached_html_change_area when the attached-page replay still needs the broader Google-shaped issue #3 helper route printed before you drop into the tighter top-level and suite-router attached-page quickstarts.',
        'Use top_level_attached_html_quickstart as the default follow-up when no pinned bundle inputs, saved summary, or non-default repo root need to take precedence, because it keeps the compact top-level attached-page route visible before the suite-router attached-page quickstart, the attached-page shortcut, replay shortcuts, the next-step matrix, contextual flow, or the bundle-first helper widen the route again.',
        'Use top_level_attached_html_entrypoint when you want the broader top-level attached-page bridge visible before the shorter top-level quickstart narrows the route again.',
        'Use suite_router_attached_html_quickstart when the replay is already clearly inside attached-page follow-up and you want the shortest suite-router-side bridge before reopening the catalog-attached bridge, the Google-attached helper, the attached-page shortcut, replay shortcuts, or the next-step matrix.',
        'Use suite_catalog_attached_html_entrypoint when the suite-catalog-side attached-page bridge should stay visible before the route narrows again.',
        'Use google_attached_html_entrypoint when the replay still needs the broader issue-specific attached-page helper visible before dropping to the shorter attached-page shortcut or replay shortcuts.',
        'Use attached_html_shortcut when the route is already clearly inside attached-page follow-up and you want the tightest bridge before reopening replay shortcuts, the next-step matrix, contextual flow, or the safe-route helper.',
        'Use attached_bundle_change_area and attached_bundle_first when the current replay should stay pinned to the known three-page compatibility bundle before widening back into the broader issue #3 attached-page helper chain.',
        'Use contextual_flow when RepoRoot, SummaryPath, or pinned InputPath values already matter and the next helper should preserve that context before choosing the narrower replay surface.',
        ...
        'Keep the Windows replay quickstart note, the Windows full-use attached-html route note, the top-level attached-page quickstart note, the suite-router attached-page quickstart note, and the validation-chain note nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between the top-level attached-page quickstart, the suite-router attached-page quickstart, replay shortcuts, the next-step matrix, the bundle-first helper, or the safe-route helper.'
} else {
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so jump straight from the validation-suite attached-page route into the top-level attached-page quickstart and keep the narrower helper chain visible before widening back out.'
}
...
Write-Host 'Google issue #3 validation-suite attached HTML quickstart'
Write-Host ''
if ($helper.repo_root) {
...
Write-Host ''
Write-Host 'Validation-suite entrypoints:'
