...
$artifacts = @(
    ...
    (New-ArtifactRecord -Path (Join-Path $RepoRoot ".zig-cache-recover") -Category "Build cache" -DefaultClean $true -Reason "Fresh-cache recovery build cache from the Windows self-recovery routine. Safe to delete after the retry is done." -Kind "directory"),
    (New-ArtifactRecord -Path (Join-Path $RepoRoot ".zig-global-cache-recover") -Category "Build cache" -DefaultClean $true -Reason "Fresh-cache global recovery cache from the Windows self-recovery routine. Safe to delete after the retry is done." -Kind "directory"),
    ...
    (New-ArtifactRecord -Path (Join-Path $RepoRoot "tmp-current-build.stdout.txt") -Category "Slice output" -DefaultClean $true -Reason "Captured stdout from the standard Windows build-log recovery command." -Kind "file"),
    (New-ArtifactRecord -Path (Join-Path $RepoRoot "tmp-current-build.stderr.txt") -Category "Slice output" -DefaultClean $true -Reason "Captured stderr from the standard Windows build-log recovery command." -Kind "file"),
