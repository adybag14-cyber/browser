...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_html_target_bundle_proof_entrypoint.ps1" -Snippet 'Add-SharedArgument -Arguments $reentryArguments -Name SummaryPath -Value $SummaryPath' -Purpose "Proof-entrypoint helper still preserves SummaryPath on the nearby suite-surface and bundle-first re-entry helpers so the same replay summary survives before or after proof."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_html_target_bundle_proof_entrypoint.ps1" -Snippet 'When SummaryPath is already pinned, this helper now preserves it on the suite-surface and bundle-first re-entry commands so the narrower issue #3 bundle bridge keeps the same replay context before or after proof.' -Purpose "Usage notes still explain that SummaryPath remains pinned on the nearby re-entry helpers before or after proof.")
)
