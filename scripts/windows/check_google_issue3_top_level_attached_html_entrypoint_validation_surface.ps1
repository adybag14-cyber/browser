...
$references = @(
    ...
    (New-ValidationReference -Path "scripts/windows/check_google_issue3_google_attached_html_entrypoint_validation_surface.ps1" -Kind "file" -Purpose "Issue-specific Google attached-html checker surfaced after the broader Google-shaped checker and before the issue-specific bridge narrows further."),
    ...
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/start_attached_pages_catalog.py" -Kind "file" -Purpose "Launcher-backed attached-pages sidecar audit surfaced before the broader Google checker or the narrower issue-specific bridge take over."),
...
$contentExpectations = @(
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'function Format-AttachedPagesSidecarAuditCommand {' -Purpose "Top-level attached-html helper keeps a dedicated launcher-backed sidecar-audit command builder."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet '$attachedPagesSidecarAuditCommand = Format-AttachedPagesSidecarAuditCommand -RepoRoot $RepoRoot -InputPath $InputPath' -Purpose "Top-level attached-html helper builds the launcher-backed sidecar audit command from the current repo-root and input-path context."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet '$googleIssue3AttachedHtmlSurfaceCheckCommand = Format-HelperCommandWithRepoRootEnv -ScriptName ''check_google_issue3_google_attached_html_entrypoint_validation_surface.ps1'' -RepoRootOverride $RepoRoot' -Purpose "Top-level attached-html helper wires the issue-specific Google checker into the shared command surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'attached_pages_sidecar_audit = $attachedPagesSidecarAuditCommand' -Purpose "Top-level attached-html helper exposes the launcher-backed sidecar audit through its helper command map."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'google_issue3_attached_html_surface_check = $googleIssue3AttachedHtmlSurfaceCheckCommand' -Purpose "Top-level attached-html helper exposes the issue-specific Google checker through its helper command map."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Write-Host ((" 13. Attached pages audit:       {0}") -f $entrypoint.helper_commands.attached_pages_sidecar_audit)' -Purpose "Printed top-level route keeps the launcher-backed sidecar audit visible before the broader Google-shaped checker and the narrower issue-specific checker."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Write-Host ((" 15. Issue-specific Google:      {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)' -Purpose "Printed top-level route keeps the issue-specific Google checker visible after the launcher-backed sidecar audit and broader Google-shaped checker."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  Attached pages audit:     {0}") -f $entrypoint.helper_commands.attached_pages_sidecar_audit)' -Purpose "Companion helper output keeps the launcher-backed sidecar audit visible from the broader attached-page bridge."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Write-Host (("  Issue-specific Google:    {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)' -Purpose "Companion helper output keeps the issue-specific Google checker visible from the broader attached-page bridge."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Use attached_pages_sidecar_audit when the current export may simply be missing its whole sibling `_files` directory and you want that simpler failure mode ruled in or out before the broader Google-shaped checks or the narrower issue-specific helpers take over.' -Purpose "Usage notes explain when to rerun the launcher-backed sidecar audit from the broader top-level route."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_entrypoint.ps1" -Snippet 'Use google_issue3_attached_html_surface_check when the replay already includes a Google-like attached page and you want the narrower issue-specific Google attached-page checker reprinted after the launcher-backed sidecar audit and broader Google-shaped checker but before dropping into the issue-specific bridge or shortcut ladder.' -Purpose "Usage notes explain when to rerun the issue-specific Google checker from the broader top-level route.")
)
...
Write-Host "Repair the broader attached-page bridge, its dedicated top-level checker surfacing, the launcher-backed sidecar-audit surfacing, the issue-specific Google checker surfacing, the compact change-area checker surfacing, the delegated helper chain, or the companion notes before trusting the issue #3 top-level attached-html entrypoint route."
exit 1
