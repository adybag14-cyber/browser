...
$entrypoint = [ordered]@{
    ...
    notes = @(
        ...
        'Use contextual_flow when RepoRoot or SummaryPath is already in play and the next helper surface should keep that replay context aligned while you choose between the broader Windows route, the validation bridge, the replay-side surface check, the Windows replay attached-html quickstart, the attached-html change-area quickstart, the broader attached-page flow helper, the Google-style attached-page flow helper, the compact top-level quickstart, the validation-router quickstart, the top-level catalog quickstart, the suite-catalog-to-top-level catalog quickstart, the suite-router attached-page quickstart, the issue-specific attached-page bridge, replay shortcuts, the next-step matrix, the bundle-first branch, or the safe-route helpers.',
...
Write-Host ((" 11. Google flow helper:         {0}") -f $entrypoint.helper_commands.google_attached_html_flow)
Write-Host ((" 12. Top-level shortcut:         {0}") -f $entrypoint.top_level_commands.top_level_shortcut_entrypoint)
Write-Host ((" 13. Top-level quickstart:       {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host ((" 14. Validation-router quick:    {0}") -f $entrypoint.helper_commands.validation_router_attached_html_quickstart)
Write-Host ((" 15. Replay attached quick:      {0}") -f $entrypoint.helper_commands.windows_replay_attached_html_quickstart)
Write-Host ((" 16. Top-level catalog quick:    {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host ((" 17. Suite-catalog top-level qk: {0}") -f $entrypoint.helper_commands.suite_catalog_top_level_attached_html_catalog_quickstart)
Write-Host ((" 18. Router attached quick:      {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host ((" 19. Attached bridge:            {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
Write-Host ((" 20. Suite-catalog bridge:       {0}") -f $entrypoint.helper_commands.suite_catalog_entrypoints)
Write-Host ((" 21. Catalog attached:           {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
Write-Host ((" 22. Attached shortcut:          {0}") -f $entrypoint.helper_commands.attached_html_shortcut)
Write-Host ((" 23. Router shortcut:            {0}") -f $entrypoint.helper_commands.suite_router_shortcut_entrypoint)
Write-Host ((" 24. Replay-route helper:        {0}") -f $entrypoint.helper_commands.replay_route_shortcut)
Write-Host ((" 25. Replay shortcuts:           {0}") -f $entrypoint.helper_commands.replay_shortcuts)
Write-Host ((" 26. Next-step matrix:           {0}") -f $entrypoint.helper_commands.suite_router_next_steps)
Write-Host ((" 27. Bundle first:               {0}") -f $entrypoint.helper_commands.attached_bundle_first)
Write-Host ''
