...
function Format-AttachedPagesSidecarAuditCommand {
    ...
    $command = 'python .\\tmp-browser-smoke\\attached-pages\\start_attached_pages_catalog.py --google-style --audit-sidecars'
...
$topLevelAttachedHtmlSurfaceCheckCommand = Format-HelperCommandWithRepoRootEnv -ScriptName 'check_google_issue3_top_level_attached_html_entrypoint_validation_surface.ps1' -RepoRootOverride $RepoRoot
...
$entrypoint = [ordered]@{
    ...
    purpose = 'Keep the shortest attached-page route visible from the top-level headed validation suite router, including the route''s own fail-fast checker, the broader Windows full-use attached-page route, its route-level surface check, the Windows-to-validation-router attached-html bridge, the replay-side attached-html surface check, the newer Windows replay attached-html quickstart, the attached-html change-area quickstart surface check, the attached-html change-area quickstart, the broader attached-page flow helper, the lighter sibling `_files` sidecar-bundle audit, the dedicated Google-style attached-page surface check, the issue-specific Google attached-page surface check, the dedicated Google-style attached-page flow helper and note, the top-level shortcut surface, the compact top-level attached-page quickstart, the validation-router attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the suite-router attached-page quickstart, and the compact bundle-suite helper plus pinned bundle-reference note before the replay narrows into the issue-specific attached-page bridge, the replay-route shortcut companion, or the bundle-first branch.'
    repo_root = $RepoRoot
    ...
    top_level_commands = [ordered]@{
        ...
        attached_html_change_area_surface_check = Format-HelperCommandWithRepoRootEnv -ScriptName 'check_google_issue3_attached_html_change_area_quickstart_validation_surface.ps1' -RepoRootOverride $RepoRoot
        ...
        top_level_shortcut_surface_check = $topLevelShortcutFirstSurfaceCheckCommand
    ...
    notes = @(
        ...
        'Use attached_bundle_change_area, attached_bundle_suite_surface, or attached_bundle_first when the current pages are still the known three-page compatibility bundle, and reopen docs/ISSUE3_ATTACHED_HTML_TARGET_BUNDLE_REFERENCE.md first so that pinned branch stays visible before widening back into the broader Google-only helper stack.',
        ...
        'Run top_level_attached_html_surface_check before suite_router_attached_html_quickstart or google_attached_html_entrypoint when you want this broader top-level attached-page route, its companion notes, and its delegated helper chain to fail fast before the replay narrows again.',
        ...
        'Run attached_html_change_area_surface_check before attached_html_change_area_quickstart when you want the compact change-area note, helper, and follow-up attached-page chain to fail fast before widening back into the broader attached-page flow helper or the shorter issue #3 quickstarts.',
        ...
        'Use attached_pages_sidecar_audit when the current export may simply be missing its whole sibling `_files` directory and you want that simpler failure mode ruled in or out before the broader Google-shaped checks or the narrower issue-specific helpers take over.',
        'Run top_level_shortcut_surface_check before top_level_shortcut_entrypoint when the replay is about to narrow into the shorter top-level shortcut-first ladder and you want that shortcut note family plus its attached-page, replay-route, bundle, and suite-router companions to fail fast before the route collapses further.',
        'Use google_attached_html_flow when the replay already includes a Google-like attached page and you want the dedicated Google-style flow helper plus docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md visible from this top-level bridge before the route narrows into the issue-specific attached-page entrypoint or shortcut chain.',
        'Use google_issue3_attached_html_surface_check when the replay already includes a Google-like attached page and you want the narrower issue-specific Google attached-page checker reprinted after the launcher-backed sidecar audit and broader Google-shaped checker but before dropping into the issue-specific bridge or shortcut ladder.',
        ...
        'Use suite_router_attached_html_quickstart as the default next helper when no bundle inputs are pinned, because it keeps the shortest suite-router-side attached-page bridge visible after the broader Windows route, the validation bridge, the replay-side surface check, this top-level attached-page surface check, the Windows replay attached-html quickstart, the newer change-area quickstart surface check, the newer change-area quickstart, the broader attached-page flow helper, the launcher-backed sidecar audit, the Google-style attached-page surface check, the issue-specific Google attached-page checker, the Google-style attached-page flow helper, the validation-router quickstart, the newer catalog quickstarts, fail-fast checkers, compact top-level quickstart, and compact bundle-suite helper before you decide whether to drop to the issue-specific attached-page bridge, attached_html_shortcut, replay_route_shortcut, replay_shortcuts, the next-step matrix, or the safe-route map.',
        ...
        'Use contextual_flow when RepoRoot, SummaryPath, BrowserExe, or pinned InputPath values already matter and the next helper surface should keep that replay context aligned while you choose between the broader Windows route, the validation bridge, the replay-side surface check, this top-level attached-page surface check, the Windows replay attached-html quickstart, the attached-html change-area quickstart surface check, the attached-html change-area quickstart, the broader attached-page flow helper, the launcher-backed sidecar audit, the Google-style attached-page surface check, the issue-specific Google attached-page checker, the Google-style attached-page flow helper, the compact top-level quickstart, the validation-router quickstart, the top-level catalog quickstart, the suite-catalog-to-top-level catalog quickstart, the suite-router attached-page quickstart, the issue-specific attached-page bridge, replay shortcuts, the next-step matrix, the bundle-first branch, or the safe-route helpers.',
        ...
        'Pass -BrowserExe when the shorter issue #3 top-level attached-page bridge should stay pinned to a non-default Windows headed build through the broader Google-shaped flow helper, the replay-side quickstart, the issue-specific attached-page bridge, the bundle-suite helper, and the bundle-first branch instead of drifting back to .\\zig-out\\bin\\lightpanda.exe.',
        'Keep the attached-html change-area quickstart note, the top-level attached-page quickstart note, the broader top-level attached-page bridge note, docs/ISSUE3_GOOGLE_ATTACHED_HTML_VALIDATION_FLOW.md, the top-level attached-page catalog quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the top-level attached-page companion note, the top-level shortcut-first note, the top-level shortcut bridge note, the attached-page shortcut note, the attached-html target bundle reference note, the attached-html target-bundle suite-surface note, the replay-side attached-html quickstart note, the broader Windows full-use attached-page route note, the Windows-to-validation-router attached-html bridge note, the validation-router attached-page quickstart note, the Windows replay quickstart, the replay discovery handoff, the suite-router attached-page quickstart, the suite-router bridge, the suite-catalog guide, the suite-catalog attached-page bridge, the validation-chain note, and the Windows runbook nearby when you want the written route beside these commands.'
    )
}

$entrypoint.recommended_next_key = if ($entrypoint.explicit_input_path_count -gt 0) {
    'attached_bundle_first'
} elseif (-not [string]::IsNullOrWhiteSpace($entrypoint.summary_path)) {
    'contextual_flow'
} else {
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    'Explicit input paths are already in play, so reopen the attached-html target bundle reference note, keep the compact bundle-suite helper visible, and stay pinned to the known three-page compatibility bundle before widening back into the broader issue #3 helper chain.'
} elseif ($entrypoint.recommended_next_key -eq 'contextual_flow') {
    'A saved summary is already in play, so keep that replay context aligned before choosing between the broader Windows route, the validation bridge, the replay-side surface check, this top-level attached-page surface check, the Windows replay attached-html quickstart, the attached-html change-area quickstart surface check, the attached-html change-area quickstart, the broader attached-page flow helper, the launcher-backed sidecar audit, the Google-style attached-page surface check, the issue-specific Google attached-page checker, the Google-style attached-page flow helper, the compact top-level quickstart, the validation-router quickstart, the top-level catalog quickstart, the suite-catalog-to-top-level catalog quickstart, the suite-router attached-page quickstart, the issue-specific attached-page bridge, the compact bundle-suite helper, replay shortcuts, the next-step matrix, the bundle-first branch, or the safe-route helpers.'
} else {
    'No pinned bundle inputs or saved summary are in play yet, so jump straight from the broader top-level attached-page bridge into the shorter suite-router attached-page quickstart while keeping the broader Windows route, the validation bridge, the replay-side surface check, this top-level attached-page surface check, the Windows replay attached-html quickstart, the newer change-area quickstart surface check, the newer change-area quickstart, the broader attached-page flow helper, the launcher-backed sidecar audit, the Google-style attached-page surface check, the issue-specific Google attached-page checker, the Google-style attached-page flow helper, the top-level shortcut-first surface check, the validation-router quickstart, the newer catalog quickstarts, fail-fast checkers, compact top-level quickstart, and compact bundle-suite helper visible on the same surface.'
}
...
Write-Host (("  8. Top-level surface check:    {0}") -f $entrypoint.top_level_commands.top_level_attached_html_surface_check)
Write-Host (("  9. Attached shortcut check:    {0}") -f $entrypoint.top_level_commands.attached_html_shortcut_surface_check)
Write-Host ((" 10. Change-area surface check:  {0}") -f $entrypoint.top_level_commands.attached_html_change_area_surface_check)
Write-Host ((" 11. Change-area quickstart:     {0}") -f $entrypoint.top_level_commands.attached_html_change_area_quickstart)
Write-Host ((" 12. Attached flow helper:       {0}") -f $entrypoint.top_level_commands.attached_html_flow)
Write-Host ((" 13. Attached pages audit:       {0}") -f $entrypoint.helper_commands.attached_pages_sidecar_audit)
Write-Host ((" 14. Google surface check:       {0}") -f $entrypoint.helper_commands.google_attached_html_surface_check)
Write-Host ((" 15. Issue-specific Google:      {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)
Write-Host ((" 16. Google flow helper:         {0}") -f $entrypoint.helper_commands.google_attached_html_flow)
Write-Host ((" 17. Top-level shortcut check:   {0}") -f $entrypoint.top_level_commands.top_level_shortcut_surface_check)
Write-Host ((" 18. Top-level shortcut:         {0}") -f $entrypoint.top_level_commands.top_level_shortcut_entrypoint)
Write-Host ((" 19. Top-level quickstart:       {0}") -f $entrypoint.helper_commands.top_level_attached_html_quickstart)
Write-Host ((" 20. Validation-router quick:    {0}") -f $entrypoint.helper_commands.validation_router_attached_html_quickstart)
Write-Host ((" 21. Replay attached quick:      {0}") -f $entrypoint.helper_commands.windows_replay_attached_html_quickstart)
Write-Host ((" 22. Top-level catalog quick:    {0}") -f $entrypoint.helper_commands.top_level_attached_html_catalog_quickstart)
Write-Host ((" 23. Suite-catalog top-level qk: {0}") -f $entrypoint.helper_commands.suite_catalog_top_level_attached_html_catalog_quickstart)
Write-Host ((" 24. Router attached quick:      {0}") -f $entrypoint.helper_commands.suite_router_attached_html_quickstart)
Write-Host ((" 25. Attached bridge:            {0}") -f $entrypoint.helper_commands.google_attached_html_entrypoint)
Write-Host ((" 26. Suite-catalog bridge:       {0}") -f $entrypoint.helper_commands.suite_catalog_entrypoints)
Write-Host ((" 27. Catalog attached:           {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
Write-Host ((" 28. Attached shortcut:          {0}") -f $entrypoint.helper_commands.attached_html_shortcut)
Write-Host ((" 29. Router shortcut:            {0}") -f $entrypoint.helper_commands.suite_router_shortcut_entrypoint)
Write-Host ((" 30. Replay-route helper:        {0}") -f $entrypoint.helper_commands.replay_route_shortcut)
...
Write-Host ((" 33. Bundle suite helper:        {0}") -f $entrypoint.helper_commands.attached_bundle_suite_surface)
...
Write-Host (("  Top-level surface check:  {0}") -f $entrypoint.top_level_commands.top_level_attached_html_surface_check)
Write-Host (("  Attached shortcut check:  {0}") -f $entrypoint.top_level_commands.attached_html_shortcut_surface_check)
Write-Host (("  Change-area surface check:{0}") -f (' ' + $entrypoint.top_level_commands.attached_html_change_area_surface_check))
...
Write-Host (("  Attached pages audit:     {0}") -f $entrypoint.helper_commands.attached_pages_sidecar_audit)
Write-Host (("  Google surface check:     {0}") -f $entrypoint.helper_commands.google_attached_html_surface_check)
Write-Host (("  Issue-specific Google:    {0}") -f $entrypoint.helper_commands.google_issue3_attached_html_surface_check)
Write-Host (("  Google flow helper:       {0}") -f $entrypoint.helper_commands.google_attached_html_flow)
Write-Host (("  Top-level shortcut check: {0}") -f $entrypoint.top_level_commands.top_level_shortcut_surface_check)
Write-Host (("  Top-level shortcut:       {0}") -f $entrypoint.top_level_commands.top_level_shortcut_entrypoint)
...
Write-Host (("Companion notes:            {0}") -f (' ' + $entrypoint.top_level_attached_html_companion_note_path))
Write-Host (("Shortcut-first note:        {0}") -f (' ' + $entrypoint.top_level_shortcut_first_entrypoint_note_path))
...
Write-Host (("Attached bundle note:       {0}") -f (' ' + $entrypoint.attached_html_target_bundle_reference_note_path))
...
Write-Host (("Windows full-use route:     {0}") -f (' ' + $entrypoint.windows_full_use_attached_html_route_note_path))
Write-Host (("Validation-router note:     {0}") -f (' ' + $entrypoint.validation_router_attached_html_quickstart_note_path))
Write-Host (("Windows replay note:        {0}") -f (' ' + $entrypoint.quickstart_note_path))
Write-Host (("Replay discovery handoff:   {0}") -f (' ' + $entrypoint.replay_discovery_handoff_note_path))
Write-Host (("Suite-router quickstart:    {0}") -f (' ' + $entrypoint.suite_router_attached_html_quickstart_note_path))
...
Write-Host (("Suite-catalog bridge:       {0}") -f (' ' + $entrypoint.suite_catalog_attached_html_bridge_note_path))
Write-Host (("Validation chain note:      {0}") -f (' ' + $entrypoint.validation_chain_note_path))
Write-Host (("Windows runbook note:       {0}") -f (' ' + $entrypoint.windows_runbook_note_path))
Write-Host ''
Write-Host 'Notes:'
