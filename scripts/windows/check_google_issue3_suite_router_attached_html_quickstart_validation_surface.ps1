...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_SUITE_ROUTER_ENTRYPOINT_GUIDE.md" -Kind "file" -Purpose "Short suite-router guide note that now stays adjacent to the attached-html quickstart as the smallest written bridge from the top-level validation suite map."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_SUITE_ROUTER_ATTACHED_HTML_QUICKSTART.md" -Snippet 'docs/ISSUE3_SUITE_ROUTER_ENTRYPOINT_GUIDE.md' -Purpose "Quickstart note keeps the suite-router entrypoint guide visible beside the shorter attached-html bridge."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'google_attached_html_flow = Format-HelperCommandWithRepoRootEnv -ScriptName ''show_google_attached_html_validation_flow.ps1'' -Arguments $attachedHtmlFlowArguments -RepoRootOverride $RepoRoot' -Purpose "Helper command map keeps the broader Google attached-html flow helper wired into the quickstart surface."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'Write-Host (("  Google attached surface:      {0}") -f $helper.commands.google_attached_html_surface_check)' -Purpose "Attached-page follow-up output still prints the broader Google attached-html surface checker before the issue-specific route narrows again."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'Write-Host (("  Google attached flow helper:  {0}") -f $helper.commands.google_attached_html_flow)' -Purpose "Attached-page follow-up output still prints the broader Google attached-html flow helper before the issue-specific bridge is trusted."),
