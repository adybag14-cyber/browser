...
$references = @(
    ...
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/attached_pages_sidecar_audit.py" -Kind "file" -Purpose "Lighter attached-pages sidecar audit surfaced before the broader Google checker and the narrower issue-specific bridge take over."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'attached_pages_sidecar_audit = $attachedPagesSidecarAuditCommand' -Purpose "Helper command map keeps the lighter attached-pages sidecar audit wired into the quickstart surface."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'Write-Host (("  Attached pages audit:        {0}") -f $helper.commands.attached_pages_sidecar_audit)' -Purpose "Broader attached-page handoff output still prints the lighter attached-pages sidecar audit before the Google-specific checks take over."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_suite_router_attached_html_quickstart.ps1" -Snippet 'Write-Host (("  Attached pages audit:         {0}") -f $helper.commands.attached_pages_sidecar_audit)' -Purpose "Attached-page follow-up output still prints the lighter attached-pages sidecar audit before the narrower bridge is trusted."),
...
Write-Host "Repair the missing attached-html note, sidecar-audit command, helper output contract, broader Google attached-page checks, issue-specific Google bridge surface, or bundle-route companion before trusting this narrowed issue #3 surface."
exit 1
