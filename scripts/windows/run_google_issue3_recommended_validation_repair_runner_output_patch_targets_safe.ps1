...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-runner-output-patch-targets-safe.json'
}
...
foreach ($helperPath in @($runnerOutputSafeScript, $patchTargetsSafeScript, $patchTargetsScript)) {
...
$patchTargetsSafeArguments = @('-SummaryPath', $SummaryPath, '-Json')
...
$patchTargetsSafeStep = $null
if ($summaryExistsAfterRunnerOutputSafe) {
    $patchTargetsSafeStep = Invoke-ScriptStep -Name 'runner-output-patch-targets-safe' -ScriptPath $patchTargetsSafeScript -Arguments $patchTargetsSafeArguments -ExpectJson
    $steps.Add($patchTargetsSafeStep) | Out-Null
} else {
    $patchTargetsSafeStep = New-SkippedStep -Name 'runner-output-patch-targets-safe' -ScriptPath $patchTargetsSafeScript -Arguments $patchTargetsSafeArguments -Reason 'The runner-output contract safe wrapper did not leave a current summary artifact, so the safe patch-target helper had no current saved state to inspect.' -RecommendedCommand $runnerOutputSafeCommand -RecommendedGuideCommand $summaryGuideCommand -NextFocus 'Get the broader issue #3 runner-output repair flow to leave a fresh summary artifact before asking whether raw patch-target guidance is safe.' -NextArtifactToOpen $ArtifactPath
    $steps.Add($patchTargetsSafeStep) | Out-Null
}

$rawPatchTargetsAllowed = [bool]($patchTargetsSafeStep.success -and $patchTargetsSafeStep.status -like 'safe-to-run-patch-targets*')
$patchTargetsStep = $null
if ($rawPatchTargetsAllowed) {
    ...
    $patchTargetsStep = New-SkippedStep -Name 'runner-output-patch-targets' -ScriptPath $patchTargetsScript -Arguments $patchTargetsArguments -Reason 'The safe patch-target helper did not clear the current summary and manifest for raw strict-mode patch-target guidance yet.' -RecommendedCommand $(if ($patchTargetsSafeStep) { $patchTargetsSafeStep.recommended_command } else { $runnerOutputSafeCommand }) -RecommendedGuideCommand $(if ($patchTargetsSafeStep) { $patchTargetsSafeStep.recommended_guide_command } else { $summaryGuideCommand }) -NextFocus $(if ($patchTargetsSafeStep) { $patchTargetsSafeStep.next_focus } else { 'Keep the issue #3 runner-output flow on the safe patch-target gate until the raw helper is trustworthy again.' }) -NextArtifactToOpen $(if ($patchTargetsSafeStep) { $patchTargetsSafeStep.next_artifact_to_open } else { $ArtifactPath })
    $steps.Add($patchTargetsStep) | Out-Null
}
...
if (-not $summaryExistsAfterRunnerOutputSafe) {
    ...
    $reason = 'The issue #3 runner-output contract safe wrapper did not leave a summary artifact, so the safe patch-target gate could not continue from the current replay.'
...
} elseif (-not $patchTargetsSafeStep.success) {
    $status = 'patch-targets-safe-failed'
    $reason = 'The runner-output repair flow completed, but the safe patch-target gate did not finish cleanly, so the next replay should reopen that bounded helper before trusting raw patch-target guidance.'
} elseif ($rawPatchTargetsAllowed -and -not $patchTargetsStep.success) {
    $status = 'patch-targets-failed'
    $reason = 'The safe patch-target gate cleared the current summary and manifest, but the raw patch-target helper did not finish cleanly, so the next replay should reopen the saved safe-gate artifact first.'
} elseif ($runnerPatchStillRequired) {
    $status = 'patch-targets-ready'
    $reason = 'The runner-output repair flow completed, the safe patch-target gate cleared the saved artifacts for strict-mode inspection, and the raw patch-target helper narrowed the remaining direct runner-output contract gap to exact fields in the recommended validation runner.'
} elseif ($patchTargetsSafeStep.status -eq 'already-direct') {
    $status = 'already-direct'
    $reason = 'The safe patch-target gate reports that the summary and manifest already expose the direct refresh and handoff runner-output contract, so there is nothing left for the raw patch-target helper to prepare.'
    ...
    $reason = 'The safe patch-target gate and the raw patch-target helper agree that the next Windows replay can trust runner-output wiring checks instead of reopening direct patch-target work.'
} else {
    $status = 'follow-up-needed'
    $reason = Get-FirstNonEmptyValue -Values @(
        if ($patchTargetsStep) { $patchTargetsStep.reason },
        if ($patchTargetsSafeStep) { $patchTargetsSafeStep.reason },
        if ($runnerOutputSafeStep) { $runnerOutputSafeStep.reason },
        'The issue #3 runner-output repair flow reached the safe patch-target gate, but the next replay still needs the follow-up command reported by the saved artifacts.'
    )
}
...
$recommendedCommand = Get-FirstNonEmptyValue -Values @(
    ...
    if ($patchTargetsSafeStep.status -eq 'already-direct') { $patchTargetsSafeStep.recommended_command },
    if ($patchTargetsSafeStep) { $patchTargetsSafeStep.recommended_command },
...
$recommendedGuideCommand = Get-FirstNonEmptyValue -Values @(
    ...
    if ($patchTargetsSafeStep.status -eq 'already-direct') { $patchTargetsSafeStep.recommended_guide_command },
    if ($patchTargetsSafeStep) { $patchTargetsSafeStep.recommended_guide_command },
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    if ($runnerPatchStillRequired) { 'Use the saved raw patch-target artifact to wire the remaining direct refresh and handoff fields into scripts/windows/run_google_issue3_recommended_validation.ps1, then rerun the narrower runner-output wiring audit.' },
    if ($patchTargetsSafeStep.status -eq 'already-direct') { 'The direct runner-output contract already looks complete, so move to the safe runner-output wiring audit or the narrower refresh-status chain instead of reopening patch-target work.' },
    if ($patchTargetsStep.status -eq 'fully-wired' -or $patchTargetsStep.status -eq 'saved-artifacts-stale-runner-already-wired') { 'The direct runner-output contract looks wired, so reopen the runner-output wiring helpers or regenerate the saved outputs on Windows instead of reopening patch-target guidance.' },
    if ($patchTargetsStep) { $patchTargetsStep.next_focus },
    if ($patchTargetsSafeStep) { $patchTargetsSafeStep.next_focus },
    if ($runnerOutputSafeStep) { $runnerOutputSafeStep.next_focus },
    if (-not $summaryExistsAfterRunnerOutputSafe) { 'Regenerate the issue #3 recommended validation summary before asking whether raw patch-target guidance is safe.' }
)
$nextArtifactToOpen = Get-FirstNonEmptyValue -Values @(
    if ($patchTargetsStep) { $patchTargetsStep.next_artifact_to_open },
    if ($patchTargetsSafeStep) { $patchTargetsSafeStep.next_artifact_to_open },
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation repair plus runner output patch targets safe gate'
    purpose = 'Run the issue #3 runner-output contract safe flow, stop at the strict-mode-safe patch-target gate first, and only reopen the raw patch-target helper when the saved summary and manifest are ready for it.'
    ...
    patch_targets_safe_status = $patchTargetsSafeStep.status
    raw_patch_targets_allowed = [bool]$rawPatchTargetsAllowed
...
if ($Json) {
    ...
    if (@('runner-output-safe-no-summary', 'runner-output-safe-failed', 'patch-targets-safe-failed', 'patch-targets-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation repair plus runner output patch targets safe gate'
Write-Host ''
...
Write-Host ("Patch targets safe status: {0}" -f $report.patch_targets_safe_status)
Write-Host ("Raw patch targets allowed: {0}" -f $report.raw_patch_targets_allowed)
