...
$helper = [ordered]@{
    ...
    purpose = 'Print the shortest bridge from the broader headed validation router into the newer suite-catalog-to-top-level attached-page catalog quickstart and top-level attached-page quickstarts for issue #3, while also surfacing the validation-router surface check, the attached-html change-area quickstart before the route narrows again, the Google-shaped attached-page branch, the pinned bundle branch, and the Windows full-use attached-page route when those broader reopening surfaces still matter.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Keep the Windows replay quickstart note, the Windows full-use attached-page route note, the validation-router attached-html quickstart note, the attached-html change-area quickstart note, the top-level attached-page quickstart note, the top-level attached-page catalog quickstart note, the suite-catalog top-level attached-page catalog quickstart note, and the suite-router attached-page quickstart note nearby when you want the written route beside these commands.'
    )
}
