...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-manifest-coherency.json'
}

$repairChainScript = Join-Path $PSScriptRoot 'run_google_issue3_recommended_validation_repair_chain.ps1'
$manifestPathScript = Join-Path $PSScriptRoot 'show_google_issue3_validation_manifest_path_coherency.ps1'
...
$manifestPathCommand = 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_validation_manifest_path_coherency.ps1'
...
foreach ($helperPath in @($repairChainScript, $manifestPathScript)) {
...
$manifestPathArguments = @('-SummaryPath', $SummaryPath, '-Json')
...
$manifestPathStep = $null
if ($summaryExistsAfterRepairChain) {
    $manifestPathStep = Invoke-ScriptStep -Name 'manifest-path-coherency' -ScriptPath $manifestPathScript -Arguments $manifestPathArguments -ExpectJson
    $steps.Add($manifestPathStep) | Out-Null
} else {
    $manifestPathStep = New-SkippedStep -Name 'manifest-path-coherency' -ScriptPath $manifestPathScript -Arguments $manifestPathArguments -Reason 'The combined repair chain did not leave a current summary artifact, so the manifest path check had no replay state to inspect.' -RecommendedCommand $repairChainCommand -RecommendedGuideCommand $summaryGuideCommand -NextFocus 'Get the broader issue #3 validation chain to leave a fresh summary artifact before checking manifest path coherency.' -NextArtifactToOpen $ArtifactPath
    $steps.Add($manifestPathStep) | Out-Null
}

$manifestPathsCoherent = [bool](
    $manifestPathStep.record -and
    ((Get-OptionalPropertyValue -Object $manifestPathStep.record -Name 'status') -eq 'coherent')
)
$manifestDriftLabels = @(
    if ($manifestPathStep.record) {
        Get-OptionalPropertyValue -Object $manifestPathStep.record -Name 'drift_labels'
    }
)
...
if (-not $summaryExistsAfterRepairChain) {
    ...
    $reason = 'The broader issue #3 validation repair chain did not leave a summary artifact, so the follow-up manifest path check could not continue.'
} elseif (-not $repairChainStep.success) {
    $status = 'repair-chain-failed'
    $reason = 'The broader issue #3 validation repair chain did not finish cleanly, so the next replay still needs direct repair-chain attention before the raw manifest guide is trusted.'
} elseif (-not $manifestPathStep.success) {
    $status = 'manifest-path-check-failed'
    $reason = 'The validation repair chain completed, but the manifest path coherency helper did not finish cleanly, so the next replay should stay on the saved repair artifact first.'
} elseif ($manifestPathsCoherent) {
    $status = 'ready-for-manifest-guide'
    $reason = 'The broader validation repair chain completed, and the manifest path helper confirms that the current summary-backed and manifest-backed artifact paths are coherent.'
} else {
    $status = 'follow-up-needed'
    $reason = Get-FirstNonEmptyValue -Values @(
        if ($manifestPathStep) { $manifestPathStep.reason },
        if ($repairChainStep) { $repairChainStep.reason },
        'The broader validation repair chain and manifest path check completed, but the next issue #3 replay still needs the follow-up command reported by the saved artifacts.'
    )
}

$recommendedCommand = Get-FirstNonEmptyValue -Values @(
    if ($manifestPathsCoherent) { $manifestGuideCommand },
    if ($manifestPathStep) { $manifestPathStep.recommended_command },
    if ($repairChainStep) { $repairChainStep.recommended_command },
    $repairChainCommand
)
$recommendedGuideCommand = Get-FirstNonEmptyValue -Values @(
    if ($manifestPathsCoherent) { $manifestSafeCommand },
    if ($manifestPathStep) { $manifestPathStep.recommended_guide_command },
    if ($repairChainStep) { $repairChainStep.recommended_guide_command },
    $summaryGuideCommand
)
$nextFocus = Get-FirstNonEmptyValue -Values @(
    if ($manifestPathStep) { $manifestPathStep.next_focus },
    if ($repairChainStep) { $repairChainStep.next_focus },
    if (-not $summaryExistsAfterRepairChain) { 'Regenerate the issue #3 recommended validation summary before asking the manifest path helper to narrow the next replay.' }
)
$nextArtifactToOpen = Get-FirstNonEmptyValue -Values @(
    if ($manifestPathStep) { $manifestPathStep.next_artifact_to_open },
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation repair plus manifest coherency'
    purpose = 'Run the broader issue #3 validation repair chain, then immediately confirm whether the saved manifest paths are coherent enough for the next Windows replay to trust the raw manifest guide directly.'
    ...
    manifest_path_status = $manifestPathStep.status
    manifest_paths_coherent = [bool]$manifestPathsCoherent
    manifest_drift_labels = @($manifestDriftLabels)
    recommended_command = $recommendedCommand
    recommended_guide_command = $recommendedGuideCommand
    repair_chain_command = $repairChainCommand
    manifest_path_command = $manifestPathCommand
...
if ($Json) {
    ...
    if (@('repair-chain-no-summary', 'repair-chain-failed', 'manifest-path-check-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation repair plus manifest coherency'
Write-Host ''
...
Write-Host ("Manifest path: {0}" -f $report.manifest_path_status)
Write-Host ("Manifest paths coherent: {0}" -f $report.manifest_paths_coherent)
if ($report.manifest_drift_labels.Count -gt 0) {
    Write-Host ("Manifest drift labels: {0}" -f ($report.manifest_drift_labels -join ', '))
}
Write-Host ''
