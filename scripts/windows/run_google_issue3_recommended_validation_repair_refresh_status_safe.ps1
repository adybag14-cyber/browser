...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-refresh-status-safe.json'
}
...
if (-not $summaryExistsAfterRepairChain) {
    ...
    $reason = 'The broader issue #3 validation repair chain did not leave a summary artifact, so the follow-up safe refresh-status check could not continue.'
