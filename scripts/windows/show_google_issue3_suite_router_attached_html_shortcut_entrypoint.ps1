...
$entrypoint = [ordered]@{
    issue = 'Google issue #3 suite-router attached HTML shortcut entrypoint'
    purpose = 'Print the shortest bridge from the suite-router attached-page path into the attached HTML shortcut helper, while preserving repo-root, saved-summary, and pinned attached-bundle context when it is already in play.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Use this helper when the replay is already narrowed to the suite-router attached-page branch and you want the dedicated attached HTML shortcut kept visible before widening back into the broader issue #3 helper chain.',
        'Use attached_html_change_area when the current replay still needs the broader attached-page compatibility route reprinted from the top-level suite router before it narrows again.',
        'Use google_attached_html_change_area when the issue-specific attached-page route should stay visible from the suite router before you reopen the shortcut surface.',
        'Use suite_router_attached_html_quickstart as the read-first bridge when you want the shorter suite-router-side attached-page route visible before dropping into attached_html_shortcut, replay_shortcuts, the next-step matrix, or the bundle-first branch.',
        'Use top_level_attached_html_quickstart or top_level_attached_html_entrypoint when you want to widen back out to the top-level attached-page helpers without reopening the broader suite catalog first.',
        'Use attached_html_shortcut as the default next helper when no pinned bundle inputs, saved summary, or non-default repo root need to take precedence, because it keeps the shortest attached-page follow-up visible before deciding whether to widen into replay_shortcuts, suite_router_next_steps, suite_catalog_attached_html_entrypoint, or suite_router_shortcut_entrypoint.',
        'Use contextual_flow instead when RepoRoot or SummaryPath is already in play and you want the next helper surface to keep that replay context aligned while you choose between replay_shortcuts, suite_router_next_steps, attached_bundle_first, or the safe-route map.',
        'Use attached_bundle_change_area, attached_bundle_first, attached_bundle_flow, and attached_bundle_runner when the current saved or attached pages are already the known three-page compatibility bundle and that pinned branch should stay visible all the way through the delegated localhost validation route.',
        'Use replay_shortcuts after attached_html_shortcut when the route is already known to stay inside issue #3 and no extra attached-page context needs to remain visible first.',
        'Use suite_catalog_attached_html_entrypoint when you want the suite-catalog-side attached-page bridge reprinted before choosing between attached_html_shortcut, replay_shortcuts, the next-step matrix, contextual_flow, or the bundle-first branch.',
        'Use suite_router_shortcut_entrypoint when the route is already known to stay inside issue #3 but you want the broader shortcut-first bridge printed again before widening into replay_shortcuts or contextual_flow.',
        'Keep the replay quickstart, the suite-router attached-page quickstart note, the top-level attached-page quickstart note, the top-level attached-page bridge note, the suite-catalog attached-page bridge note, and the validation-chain note nearby when you want the written route beside these commands.'
    )
}
...
$entrypoint.recommended_next_reason = if ($entrypoint.recommended_next_key -eq 'attached_bundle_first') {
    'Explicit input paths are already in play, so stay pinned to the known three-page compatibility bundle before widening back into the broader Google-only issue #3 route.'
} elseif ($entrypoint.recommended_next_key -eq 'contextual_flow') {
    'A non-default repo root or saved summary is already in play, so keep that replay context aligned before choosing between replay shortcuts, the next-step matrix, the bundle-first branch, or the safe-route helpers.'
} else {
    'No pinned bundle inputs, saved summary, or non-default repo root are in play yet, so jump straight from the suite-router attached-page branch into the dedicated attached HTML shortcut helper.'
}
...
Write-Host 'Google issue #3 suite-router attached HTML shortcut entrypoint'
Write-Host ''
...
Write-Host (("  4. Router attached quick:{0}") -f (' ' + $entrypoint.top_level_commands.suite_router_attached_html_quickstart))
Write-Host (("  5. Top-level quickstart: {0}") -f $entrypoint.top_level_commands.top_level_attached_html_quickstart)
Write-Host (("  6. Top-level bridge:     {0}") -f $entrypoint.top_level_commands.top_level_attached_html_entrypoint)
Write-Host (("  7. Attached shortcut:    {0}") -f $entrypoint.helper_commands.attached_html_shortcut)
Write-Host (("  8. Replay shortcuts:     {0}") -f $entrypoint.helper_commands.replay_shortcuts)
Write-Host (("  9. Next-step matrix:     {0}") -f $entrypoint.helper_commands.suite_router_next_steps)
Write-Host ((" 10. Catalog attached:     {0}") -f $entrypoint.helper_commands.suite_catalog_attached_html_entrypoint)
...
Write-Host ''
Write-Host (("Replay quickstart:      {0}") -f $entrypoint.quickstart_note_path)
