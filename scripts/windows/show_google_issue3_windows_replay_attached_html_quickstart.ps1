...
$helper = [ordered]@{
    ...
    purpose = 'Print the narrow attached-localhost ladder that matches the current Windows replay route for issue #3, while keeping the replay-side fail-fast checker, the Windows full-use route-level surface check, the Windows-to-validation-router bridge, the Windows-first attached-html catalog step, the wider suite-catalog guide, the suite-catalog-to-top-level attached-html catalog quickstart, and the newer top-level shortcut bridge visible before the route narrows back into the compact attached-page helpers.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Start here when docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md already narrowed the next replay to the attached localhost branch and you want the shortest helper ladder printed in one place with the replay-side surface check and the top-level shortcut bridge kept visible.',
