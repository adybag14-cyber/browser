...
$helper = [ordered]@{
    ...
    purpose = 'Print the narrow attached-localhost ladder that matches the current Windows replay route for issue #3 without reopening the broader suite catalog or wrapper-heavy safe-route notes first.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use validation_router_attached_html_quickstart first when the replay still needs the broader validation-router attached-page bridge visible before the top-level attached-page quickstarts.',
        'Then prefer top_level_attached_html_quickstart and top_level_attached_html_entrypoint as the default middle of the ladder so the compact top-level route stays visible before you drop into the suite-catalog bridge or the shorter attached-page shortcut.',
        ...
        'Keep suite_router_attached_html_quickstart nearby as the sidecar helper when the route needs to widen back toward the suite-router surface instead of narrowing directly into the top-level attached-page ladder.',
        'Use attached_html_shortcut only after the top-level attached-page quickstart or bridge is already in view and the replay is ready to stay inside the narrower issue #3 helper chain.',
        ...
        'Use windows_full_use_attached_html_route when the replay came from docs/WINDOWS_FULL_USE.md first and you want the broader Windows runbook attached-page bridge visible beside this shorter replay ladder.',
        'Keep the Windows replay quickstart note, the validation-router attached-page note, the top-level attached-page quickstart note, the top-level attached-page bridge note, the top-level attached-page catalog quickstart note, the suite-catalog attached-page bridge note, the suite-router attached-page quickstart note, the Windows full-use attached-page route note, and the validation-chain note nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so reopen the validation-router attached-page quickstart first and then walk the shorter top-level attached-page ladder.'
}
...
Write-Host ''
Write-Host 'Broader companion route:'
Write-Host (("  Windows full-use bridge:      {0}") -f $helper.commands.windows_full_use_attached_html_route)
Write-Host ''
