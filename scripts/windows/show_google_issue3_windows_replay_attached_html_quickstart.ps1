...
$helper = [ordered]@{
    ...
    purpose = 'Print the narrow attached-localhost ladder that matches the current Windows replay route for issue #3, while keeping the Windows full-use route-level surface check, the Windows-to-validation-router bridge, the Windows-first attached-html catalog step, the wider suite-catalog guide, and the suite-catalog-to-top-level attached-html catalog quickstart visible before the route narrows back into the compact attached-page helpers.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Keep the Windows replay quickstart note, the Windows full-use attached-page route note, the Windows full-use validation-router attached-html bridge note, the Windows full-use attached-html catalog quickstart note, the validation-router attached-page note, the attached-html change-area quickstart note, the top-level attached-page quickstart note, the top-level attached-page bridge note, the top-level attached-page catalog quickstart note, the suite-catalog-to-top-level attached-html catalog quickstart note, the suite-catalog entrypoints guide, the suite-catalog attached-page bridge note, the suite-router attached-page quickstart note, and the validation-chain note nearby when you want the written route beside these commands.'
    )
}
...
$helper.recommended_next_reason = if ($helper.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so reopen the Windows full-use validation-router attached-html bridge first and keep the route-level surface checker plus the Windows-first attached-html catalog quickstart nearby before dropping into the smaller attached-page quickstarts.'
}
