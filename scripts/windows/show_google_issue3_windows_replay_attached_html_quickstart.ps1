...
$helper = [ordered]@{
    ...
    purpose = 'Print the narrow attached-localhost ladder that matches the current Windows replay route for issue #3, while keeping the replay-side fail-fast checker, the broader attached-page flow helper, the dedicated Google-shaped attached-page surface check, the issue-specific Google attached-page bridge, the top-level attached-html, Google-attached-html, and bundle-aware re-entry points, the Windows full-use route-level surface check, the Windows-to-validation-router bridge, the Windows-first attached-html catalog step, the broader top-level companion-note map, the wider suite-catalog guide, the suite-catalog-to-top-level attached-html catalog quickstart, the newer top-level shortcut bridge, the compact bundle-suite surface helper, the pinned bundle proof-route checker and helper, and the replay-route shortcut bridge visible before the route narrows back into the compact attached-page helpers.'
    repo_root = $RepoRoot
    ...
    notes = @(
        'Start here when docs/ISSUE3_WINDOWS_REPLAY_QUICKSTART.md already narrowed the next replay to the attached localhost branch and you want the shortest helper ladder printed in one place with the replay-side surface check, the broader attached-page flow helper, the top-level attached-html re-entry points, the top-level shortcut bridge, the compact bundle-suite surface helper, the pinned bundle proof-route checker and helper, and the replay-route shortcut bridge kept visible.',
        ...
        'Keep the Windows replay quickstart note, the replay-side attached-html quickstart note, the Windows full-use attached-page route note, the Windows full-use validation-router attached-html bridge note, the Windows full-use attached-html catalog quickstart note, the validation-router attached-page note, the attached-html change-area quickstart note, the top-level attached-page quickstart note, the top-level attached-page bridge note, the top-level attached-page catalog quickstart note, the top-level attached-page companion-notes map, the suite-catalog-to-top-level attached-html catalog quickstart note, the top-level shortcut bridge note, the suite-catalog entrypoints guide, the suite-catalog attached-page bridge note, the Google attached-page validation-flow note, the suite-router attached-page quickstart note, the suite-router shortcut bridge note, the replay-route shortcut bridge note, the attached-html target bundle reference note, the attached-html target-bundle suite-surface note, the attached-html target-bundle proof-entrypoint note, and the validation-chain note nearby when you want the written route beside these commands.'
    )
}
