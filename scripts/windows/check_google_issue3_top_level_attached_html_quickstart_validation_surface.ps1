...
$references = @(
    ...
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/start_attached_pages_catalog.py" -Kind "file" -Purpose "Launcher-backed attached-pages sidecar audit surfaced before the broader Google checker or narrower issue-specific bridge take over."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet '$command = ''python .\\tmp-browser-smoke\\attached-pages\\start_attached_pages_catalog.py --audit-sidecars''' -Purpose "Top-level attached-html quickstart now builds the launcher-backed attached-pages sidecar-audit command before wiring the helper surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet '$attachedPagesSidecarAuditCommand = Format-AttachedPagesSidecarAuditCommand -RepoRoot $RepoRoot -InputPath $InputPath' -Purpose "Top-level attached-html quickstart now passes repo-root and pinned-input context through the launcher-backed sidecar-audit command builder."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet 'attached_pages_sidecar_audit = $attachedPagesSidecarAuditCommand' -Purpose "Top-level attached-html quickstart exposes the launcher-backed attached-pages sidecar audit through the helper command map."),
...
Write-Host "Repair the missing compact top-level note, attached-page companion, helper surface, launcher-backed sidecar-audit surfacing, issue-specific Google checker surfacing, bundle proof note surfacing, or bundle fallback before trusting this issue #3 attached-page route."
exit 1