...
$references = @(
    ...
    (New-ValidationReference -Path "scripts/windows/start_attached_pages_catalog.ps1" -Kind "file" -Purpose "Windows wrapper surfaced by the compact top-level route when the cheaper sidecar-bundle audit should run before the broader Google checker or narrower issue-specific bridge."),
    (New-ValidationReference -Path "tmp-browser-smoke/attached-pages/start_attached_pages_catalog.py" -Kind "file" -Purpose "Cross-platform attached-pages launcher that still needs to stay available for lower-level or non-Windows replay paths."),
...
$contentExpectations = @(
    (New-ValidationContentExpectation -Path "docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_QUICKSTART.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\start_attached_pages_catalog.ps1 -AuditSidecars -InputPath ''<attached-html-root>''' -Purpose "Quickstart note keeps the wrapper-backed attached-pages sidecar audit visible before the broader Google checker or narrower issue-specific bridge take over."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_TOP_LEVEL_ATTACHED_HTML_QUICKSTART.md" -Snippet '- `docs/ISSUE3_GOOGLE_ATTACHED_HTML_ENTRYPOINT.md`' -Purpose "Quickstart note keeps the issue-specific Google attached-html entrypoint note visible in the nearby companion list."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet '$wrapperArguments = [System.Collections.Generic.List[string]]::new()' -Purpose "Top-level attached-html quickstart builds a Windows wrapper-backed sidecar-audit command before wiring the helper surface."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet 'Add-SharedArgument -Arguments $wrapperArguments -Name RepoRoot -Value $RepoRoot' -Purpose "Top-level attached-html quickstart now passes repo-root context through the wrapper-backed sidecar-audit command builder."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet 'return Format-HelperCommand -ScriptName ''start_attached_pages_catalog.ps1'' -Arguments $wrapperArguments -Switches @(''AuditSidecars'')' -Purpose "Top-level attached-html quickstart now prefers the Windows wrapper-backed sidecar audit command when surfacing the attached-pages preflight."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_top_level_attached_html_quickstart.ps1" -Snippet 'attached_pages_sidecar_audit = $attachedPagesSidecarAuditCommand' -Purpose "Top-level attached-html quickstart exposes the wrapper-backed attached-pages sidecar audit through the helper command map."),
...
Write-Host "Repair the missing compact top-level note, attached-page companion, helper surface, wrapper-backed sidecar-audit surfacing, issue-specific Google checker surfacing, bundle proof note surfacing, or bundle fallback before trusting this issue #3 attached-page route."
exit 1
