...
if (-not $ArtifactPath) {
    $ArtifactPath = Join-Path $artifactRoot 'google-issue3-recommended-validation-repair-handoff-safe.json'
}
...
foreach ($helperPath in @($repairChainScript, $handoffSafeScript)) {
...
if ($summaryExistsAfterRepairChain) {
    ...
    $handoffSafeStep = New-SkippedStep -Name 'handoff-safe-status' -ScriptPath $handoffSafeScript -Arguments $handoffSafeArguments -Reason 'The combined repair chain did not leave a current summary artifact, so the safe handoff check had no replay state to inspect.' -RecommendedCommand $repairChainCommand -RecommendedGuideCommand $summaryGuideCommand -NextFocus 'Get the broader issue #3 validation chain to leave a fresh summary artifact before checking the safe handoff gate.' -NextArtifactToOpen $ArtifactPath
    $steps.Add($handoffSafeStep) | Out-Null
}
...
if (-not $summaryExistsAfterRepairChain) {
    ...
    $reason = 'The broader issue #3 validation repair chain did not leave a summary artifact, so the follow-up safe handoff check could not continue.'
    ...
    $reason = 'The validation repair chain completed, but the safe handoff helper did not finish cleanly, so the next replay should stay on the saved repair artifact first.'
} elseif ($handoffSafeStep.status -eq 'safe-to-run-handoff') {
    $status = 'ready-for-handoff'
    $reason = 'The broader validation repair chain completed, and the safe handoff helper says the narrower issue #3 handoff path is ready for the next Windows replay.'
} else {
    ...
    $reason = Get-FirstNonEmptyValue -Values @(
        ...
        'The broader validation repair chain and safe handoff check completed, but the next issue #3 replay still needs the follow-up command reported by the saved artifacts.'
    )
}
...
$nextFocus = Get-FirstNonEmptyValue -Values @(
    ...
    if (-not $summaryExistsAfterRepairChain) { 'Regenerate the issue #3 recommended validation summary before asking the safe handoff helper to narrow the next replay.' }
)
...
$report = [ordered]@{
    issue = 'Google issue #3 recommended validation repair plus safe handoff'
    purpose = 'Run the broader issue #3 validation repair chain, then immediately confirm whether the safe handoff checkpoint is clear enough for the next Windows replay to trust the narrower handoff helper.'
...
if ($Json) {
    ...
    if (@('repair-chain-no-summary', 'repair-chain-failed', 'handoff-safe-check-failed') -contains $status) {
        exit 1
    }
    exit 0
}

Write-Host 'Google issue #3 recommended validation repair plus safe handoff'
Write-Host ''
