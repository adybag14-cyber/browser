...
$entrypoint = [ordered]@{
    ...
    purpose = 'Print the shortest top-level route from the headed validation suite catalog into the newer issue #3 suite-router shortcut entrypoint, while also surfacing the attached-page compatibility branch, the suite-catalog attached-page bridge, and preserving repo-root, saved-summary, and pinned bundle-input context when it is already in play.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use suite_catalog_attached_html_entrypoint when the replay is already entering issue #3 from the attached-page route and you want the dedicated suite-catalog attached-page bridge printed before you narrow into the issue-specific attached-page or shortcut-first helpers.',
        ...
        'Keep the quickstart, replay-discovery, suite-router bridge, suite-catalog guide, suite-catalog attached-html bridge, and validation-chain notes nearby when you want the written route beside these commands.'
    )
}
