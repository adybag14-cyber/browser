...
$shortcuts = [ordered]@{
    ...
    purpose = 'Keep the top-level issue #3 read-first commands, the Windows full-use attached-page route, the Windows full-use validation-router attached-page bridge, the Windows full-use attached-page catalog quickstart, the replay-shortcuts-to-Windows-replay attached-page bridge, the Windows replay attached-page quickstart, the broader attached-page route, the broader attached-page flow helper, the Google-style attached-page flow helper, the attached-html change-area quickstart, the newer top-level attached-page quickstart and top-level attached-page catalog quickstart, the broader top-level attached-page bridge, the newer top-level shortcut-first entrypoint, the replay-route shortcut bridge, the compact bundle-suite surface helper, the dedicated attached-page shortcut helper, the newer suite-router shortcut-first entrypoint, the suite-catalog bridge, the compact next-step matrix, the context-preserving helper, the narrower safe-route helper, the attached three-page bundle route, and the runner-state next-step helper on one compact command surface.'
    repo_root = $RepoRoot
    ...
    read_first_commands = [ordered]@{
        ...
        windows_full_use_validation_router_attached_html_bridge = Format-HelperCommand -ScriptName 'show_google_issue3_windows_full_use_validation_router_attached_html_bridge.ps1' -Arguments $bundleFirstArguments
        ...
        top_level_shortcut_entrypoint = Format-HelperCommand -ScriptName 'show_google_issue3_top_level_shortcut_first_entrypoint.ps1' -Arguments $bundleFirstArguments
        replay_route_shortcut = Format-HelperCommand -ScriptName 'show_google_issue3_replay_route_shortcut_entrypoint.ps1' -Arguments $bundleFirstArguments
        attached_html_target_bundle_suite_surface = Format-HelperCommand -ScriptName 'show_google_issue3_attached_html_target_bundle_suite_surface.ps1' -Arguments $bundleFirstArguments
    ...
    notes = @(
        ...
        'Use windows_full_use_validation_router_attached_html_bridge when the broader Windows-first attached-page route is already in focus and you want the Windows-to-validation-router handoff printed before the compact Windows-full-use-to-catalog bridge, the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, or the narrower attached-page helpers.',
        'Use windows_full_use_attached_html_catalog_quickstart when the broader Windows-first attached-page route and validation-router bridge are already in focus and you want the compact Windows-full-use-to-catalog bridge printed before you reopen the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, the top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the attached-page shortcut, replay shortcuts, contextual flow, or the safe-route map.',
        ...
        'Use windows_replay_attached_html_quickstart when the route is already narrow enough to stay on the replay-side attached-page ladder directly after the replay-shortcuts bridge, the Windows validation bridge, or the broader Windows full-use route surfaces.',
        ...
        'Use attached_html_flow when the replay is already narrowed to the broader attached-page compatibility route and you want that broader attached-page flow helper surfaced directly from replay_shortcuts before dropping into the issue-specific attached-page quickstarts, the top-level attached-page bridge, replay shortcuts, contextual flow, the pinned bundle-first path, or the safe-route map.',
        'Use google_attached_html_flow when the current attached-page inputs already include a Google-like page and you want the narrower Google-style attached-page guide and helper surfaced directly from replay_shortcuts before you widen back into the top-level attached-page quickstarts, the broader attached-page flow helper, the replay-shortcuts-to-Windows-replay bridge, the attached-page shortcut helper, or the safe-route map.',
        ...
        'Use top_level_attached_html_quickstart when the replay has already re-entered from the top-level validation router and you want the compact top-level attached-page helper kept visible before deciding between the broader top-level attached-page bridge, the top-level attached-page catalog quickstart, the top-level shortcut-first entrypoint, the replay-route shortcut bridge, the compact bundle-suite surface helper, the attached-page shortcut helper, replay shortcuts, the next-step matrix, suite-router handoff, or the pinned bundle-first path.',
        ...
        'Use top_level_shortcut_entrypoint when the current replay already favors the newer top-level shortcut bridge and you want that narrower handoff visible before widening back into replay shortcuts, replay-route shortcut follow-up, the compact bundle-suite helper, the attached-page shortcut helper, or the safe-route map.',
        'Use replay_route_shortcut when the replay-route shortcut bridge is already the next obvious move from replay_shortcuts and you want that narrower shortcut surface visible before widening back into replay route, replay shortcuts, contextual flow, or the bundle-first branch.',
        'Use suite_router_shortcut_entrypoint when the higher-level suite router has already made issue #3 obvious and you want the shorter bridge back into replay shortcuts, the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, the broader attached-page flow helper, the attached-page shortcut helper, contextual flow, the next-step matrix, or the pinned bundle-first branch before reopening the wider compact helpers.',
        'Use attached_html_target_bundle_suite_surface when the current replay is already close to the known three-page compatibility bundle and you want the compact bundle-side surface printed before the bundle-first helper, the bundle flow helper, the bundle runner, or a broader replay-route fallback.',
        ...
        'Use suite_router_handoff when you want the higher-level suite-router entrypoints, the Windows full-use attached-page route, the Windows full-use validation-router attached-page bridge, the Windows full-use attached-page catalog quickstart, the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, the attached-page route, the broader attached-page flow helper, the Google-style attached-page flow helper, the attached-html change-area quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the top-level attached-page catalog quickstart, the top-level shortcut-first entrypoint, the replay-route shortcut bridge, the compact bundle-suite helper, the shortcut-first suite-router entrypoint, the suite-catalog bridge, and the current replay-shortcuts helper reprinted together in one compact surface before you drop into the next-step matrix, replay-route, attached-page shortcut, contextual-flow, safe-route, or bundle-first helpers.',
        'Use suite_router_next_steps when you want the compact next-step matrix that keeps the current start points, the Windows-first attached-page route, the Windows validation-router bridge, the Windows-full-use catalog bridge, the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, the attached-page route, the broader attached-page flow helper, the Google-style attached-page flow helper, the attached-html change-area quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the top-level attached-page catalog quickstart, the top-level shortcut-first bridge, the replay-route shortcut bridge, the compact bundle-suite helper, the shortcut-first suite-router bridge, replay-route branch, attached-bundle branch, contextual-flow branch, and runner-state helper choices together before you pick one narrower replay path.',
        'Use replay_route when you want the attached-bundle branch, the safe-route bridge, the context-preserving helper, and the repo-root-aware runner next-step helper printed in one slightly broader surface before you return to the narrower replay-shortcuts, replay-shortcuts-to-Windows-replay, Windows replay attached-page quickstart, broader attached-page flow helper, Google-style attached-page flow helper, or attached-page shortcut helpers.',
        'Use contextual_flow when you already know repo-root overrides, saved-summary state, or pinned attached pages should stay visible while you choose between the recommended runner, replay shortcuts, the replay-shortcuts-to-Windows-replay bridge, the Windows replay attached-page quickstart, the Windows full-use attached-page route, the Windows full-use validation-router attached-page bridge, the Windows full-use attached-page catalog quickstart, the broader attached-page flow helper, the Google-style attached-page flow helper, the attached-html change-area quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the top-level attached-page catalog quickstart, top-level shortcut follow-up, replay-route shortcut follow-up, attached-page shortcut, live trace, or bundle follow-up commands.',
        ...
        'Keep quickstart_note_path open for the shortest current replay note, windows_runbook_note_path when the route is being resumed from the broader Windows headed runbook, windows_full_use_attached_html_route_note_path for the shorter Windows-first attached-page route, windows_full_use_validation_router_attached_html_bridge_note_path for the Windows-to-validation-router handoff, windows_full_use_attached_html_catalog_quickstart_note_path for the compact Windows-full-use-to-catalog bridge, replay_shortcuts_windows_replay_attached_html_bridge_note_path for the replay-shortcuts-to-Windows-replay handoff, windows_replay_attached_html_quickstart_note_path for the replay-side attached-page ladder itself, google_attached_html_flow_note_path for the narrower Google-style attached-page guide, validation_chain_note_path for wrapper precedence, suite_router_bridge_note_path for the narrow prose bridge into replay shortcuts, suite_catalog_entrypoint_note_path for the shortest bridge from the top-level suite router into the current helper chain, top_level_attached_html_quickstart_note_path for the compact top-level attached-page handoff, top_level_attached_html_bridge_note_path for the broader top-level attached-page route, top_level_attached_html_catalog_quickstart_note_path for the compact top-level plus suite-catalog handoff, top_level_shortcut_bridge_note_path for the newer top-level shortcut follow-up, replay_route_shortcut_bridge_note_path for the replay-route shortcut follow-up, attached_html_target_bundle_reference_note_path plus attached_html_target_bundle_suite_surface_note_path for the compact pinned-bundle branch, decision_table_note_path when the replay lands on ready-for-runner-patch, already-direct, or runner-already-wired-regenerate-outputs, and patch_rules_note_path when the replay is already narrowed to a direct runner-source edit.'
    )
}
...
Write-Host (("  Windows validation bridge:   {0}") -f $shortcuts.read_first_commands.windows_full_use_validation_router_attached_html_bridge)
...
Write-Host (("  Attached HTML flow:          {0}") -f $shortcuts.read_first_commands.attached_html_flow)
Write-Host (("  Google attached flow:        {0}") -f $shortcuts.read_first_commands.google_attached_html_flow)
...
Write-Host (("  Top-level shortcut:          {0}") -f $shortcuts.read_first_commands.top_level_shortcut_entrypoint)
Write-Host (("  Replay-route shortcut:       {0}") -f $shortcuts.read_first_commands.replay_route_shortcut)
Write-Host (("  Bundle-suite helper:         {0}") -f $shortcuts.read_first_commands.attached_html_target_bundle_suite_surface)
...
Write-Host (("  Windows validation bridge:   {0}") -f $shortcuts.helper_commands.windows_full_use_validation_router_attached_html_bridge)
...
Write-Host (("  Attached HTML flow:          {0}") -f $shortcuts.helper_commands.attached_html_flow)
Write-Host (("  Google attached flow:        {0}") -f $shortcuts.helper_commands.google_attached_html_flow)
...
Write-Host (("  Top-level shortcut:          {0}") -f $shortcuts.helper_commands.top_level_shortcut_entrypoint)
Write-Host (("  Replay-route shortcut:       {0}") -f $shortcuts.helper_commands.replay_route_shortcut)
Write-Host (("  Bundle-suite helper:         {0}") -f $shortcuts.helper_commands.attached_html_target_bundle_suite_surface)
...
Write-Host (("Windows bridge note:      {0}") -f $shortcuts.windows_full_use_validation_router_attached_html_bridge_note_path)
...
Write-Host (("Google attached note:     {0}") -f $shortcuts.google_attached_html_flow_note_path)
...
Write-Host (("Top-level catalog note:   {0}") -f (' ' + $shortcuts.top_level_attached_html_catalog_quickstart_note_path))
Write-Host (("Top-level shortcut note:  {0}") -f $shortcuts.top_level_shortcut_bridge_note_path)
Write-Host (("Replay-route note:        {0}") -f $shortcuts.replay_route_shortcut_bridge_note_path)
Write-Host (("Bundle reference note:    {0}") -f $shortcuts.attached_html_target_bundle_reference_note_path)
Write-Host (("Bundle-suite note:        {0}") -f $shortcuts.attached_html_target_bundle_suite_surface_note_path)
