...
$traceGuideCommand = Join-Command -Prefix ("powershell -ExecutionPolicy Bypass -File {0}" -f $traceGuide) -Arguments $sharedArgs
...
$flow = [ordered]@{
    ...
    steps = @(
        ...
        [ordered]@{
            name = 'trace-guide'
            goal = 'Keep the shared Enter-order trace guide nearby when the reduced probe shows that focus, typing, or submit still diverges before the full live Google pass.'
    ...
    notes = @(
        ...
        'Keep the same InputText across the reduced probe and the shared Enter-order helpers so query-value comparisons stay easy to read in logs and issue notes.',
