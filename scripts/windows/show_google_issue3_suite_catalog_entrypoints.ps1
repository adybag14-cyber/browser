...
$entrypoints = [ordered]@{
    ...
    purpose = 'Keep the exact top-level show_headed_validation_suites entrypoints, the Windows replay attached-html surface check, the newer Windows full-use attached-page catalog quickstart, the Windows replay attached-html quickstart, the newer validation-router attached-page quickstart, the newer attached-page change-area quickstart, the newer suite-catalog-to-top-level attached-page catalog quickstart, the newer top-level attached-page quickstart, the newer top-level attached-page catalog quickstart, the newer suite-catalog attached-page bridge, the newer suite-router attached-page quickstart, the newer top-level shortcut and top-level attached-page bridges, the current Google flow helper, the issue-specific attached-page helpers, the current shortcut-first issue #3 replay helper, the compact next-step matrix, the context-preserving issue #3 replay flow, and the current safe-route replay helpers on one compact command surface before the route narrows into replay-route or the wrapper-heavy safe-route helpers.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use attached_html_change_area when the next replay is already narrowed to attached-page follow-up but pinned bundle inputs are not yet locked, so the route can still prefer the attached-page change-area quickstart, the Windows replay attached-html quickstart, the new validation-router attached-page quickstart, the newer suite-catalog-to-top-level attached-page catalog quickstart, the top-level attached-page quickstart, the newer top-level attached-page catalog quickstart, and the newer top-level attached-page bridge before it falls back to the bundle-first branch.',
        ...
        'Use windows_full_use_attached_html_catalog_quickstart when the replay is re-entering from docs/WINDOWS_FULL_USE.md and you want the broader Windows-side attached-page catalog path kept visible between the replay surface check and the replay-side attached-html quickstart before the route narrows into the suite-catalog-to-top-level attached-page catalog quickstart or the smaller attached-page helpers.',
        ...
        'Use replay_shortcuts as the default next helper after the top-level suite catalog when no pinned bundle inputs, repo-root override, or saved summary need to take precedence, because it keeps the newer shortcut-first issue #3 bridge visible before you decide whether to reopen the Windows full-use attached-html catalog quickstart, the Windows replay attached-html quickstart, the attached-page change-area quickstart, the validation-router attached-page quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the newer top-level attached-page catalog quickstart, the issue-specific attached-page entrypoint, the attached-page shortcut, the next-step matrix, or the bundle-first branch.',
        ...
        'Keep the Windows replay attached-html quickstart note, the Windows full-use attached-html route note, the Windows full-use attached-html catalog quickstart note, the top-level attached-page quickstart note, the top-level attached-page catalog quickstart note, the validation-router attached-page quickstart note, the attached-page change-area quickstart note, the suite-catalog top-level attached-page catalog quickstart note, the Windows replay quickstart note, the suite-router attached-page quickstart note, the suite-router bridge, the suite-catalog guide, the top-level attached-page bridge, and the validation-chain notes nearby when you want the written route beside these commands without reopening the broader Windows runbook first.'
    )
}
...
Write-Host (("  Windows catalog quick:     {0}") -f $entrypoints.helper_commands.windows_full_use_attached_html_catalog_quickstart)
