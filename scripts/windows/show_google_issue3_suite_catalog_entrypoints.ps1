...
$attachedBundleSurfaceCheckArguments = [System.Collections.Generic.List[string]]::new()
Add-SharedArgument -Arguments $attachedBundleSurfaceCheckArguments -Name RepoRoot -Value $RepoRoot
...
$suiteRouterHandoffSurfaceCheckCommand = Format-HelperCommandWithRepoRootEnv -ScriptName 'check_google_issue3_suite_router_handoff_validation_surface.ps1' -RepoRootOverride $RepoRoot
...
$attachedBundleSurfaceCheckCommand = Format-HelperCommand -ScriptName 'check_attached_html_target_bundle_validation_surface.ps1' -Arguments $attachedBundleSurfaceCheckArguments
$attachedBundleFlowCommand = Format-HelperCommand -ScriptName 'show_attached_html_target_bundle_validation_flow.ps1' -Arguments $attachedBundleFlowArguments
...
$entrypoints = [ordered]@{
    ...
    purpose = 'Keep the exact top-level show_headed_validation_suites entrypoints, the suite-catalog surface check, the broader Windows full-use attached-page route, its route-level surface check, the Windows-to-validation-router bridge, the Windows full-use attached-page catalog quickstart, the Windows replay attached-html surface check, the Windows replay attached-html quickstart, the broader attached-page flow helper, the dedicated Google attached-page surface checker, the dedicated Google attached-page flow helper, the validation-router attached-page quickstart, the attached-page change-area quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the top-level attached-page quickstart, the top-level attached-page surface check, the top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the suite-router quickstarts, the suite-router attached-page surface check, the suite-router handoff helper and surface check, the top-level shortcut and top-level attached-page bridges, the current Google flow helper, the issue-specific attached-page helpers, the compact next-step matrix, the context-preserving issue #3 replay flow, the replay-route shortcut helper, the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, and the current safe-route replay helpers on one compact command surface before the route narrows into replay-route, bundle-first, or the wrapper-heavy safe-route helpers.'
    repo_root = $RepoRoot
    ...
    helper_commands = [ordered]@{
        ...
        suite_router_handoff_surface_check = $suiteRouterHandoffSurfaceCheckCommand
        ...
        attached_bundle_surface_check = $attachedBundleSurfaceCheckCommand
        attached_bundle_flow = $attachedBundleFlowCommand
    ...
    notes = @(
        ...
        'Use attached_html_flow when the broader attached-page localhost helper still needs to stay visible from the same change-area branch before you commit to the dedicated Google attached-page surface checker, the dedicated Google attached-page flow helper, the validation-router attached-page quickstart, or the narrower suite-catalog ladders.',
        'Use google_attached_html_surface_check when the dedicated Google-shaped attached-page helper stack may have drifted and you want the fail-fast checker reprinted before the dedicated Google attached-page flow helper or the narrower validation-router and shortcut-first branches.',
        'Use google_attached_html_validation_flow when the broader Google-shaped attached-page asset-closure route and helper output should stay visible after the dedicated surface checker and before the narrower validation-router quickstart, issue-specific attached-page bridge, or shortcut-first branch takes over.',
        ...
        'Use suite_router_attached_html_surface_check when the route is already narrowed to attached localhost follow-up from the top-level suite router and you want that compact helper family to fail fast before widening into the top-level quickstart, the top-level catalog quickstart, or the shorter replay helpers.',
        'Use top_level_attached_html_surface_check when the route is already narrowed to the compact top-level attached-page quickstart and you want that helper family to fail fast before dropping into the top-level shortcut bridge, the suite-catalog-side bridge, or replay shortcuts.',
        'Use suite_router_handoff_surface_check when the compact suite-router handoff helper is already in view and you want that wider bridge to fail fast before widening into replay_route, replay_route_shortcut, or the safe-route map.',
        'Use replay_route_shortcut when replay-route follow-up is already open and you want the printed shortcut bridge visible before the compact bundle suite surface, the pinned bundle surface checker, the bundle flow helper, or the bundle-first route takes over.',
        'Use attached_bundle_suite_surface when the replay is already close to the known three-page compatibility bundle and you want the compact bundle-specific re-entry helper visible before the route narrows into the pinned bundle surface checker, the pinned bundle flow helper, the bundle-first branch, or the delegated bundle runner.',
        'Use attached_bundle_surface_check next when you want the pinned three-page route to fail fast before the bundle flow helper or the bundle-first branch takes over from the suite-catalog surface.',
        'Use attached_bundle_flow when you want the printed pinned-bundle helper ladder kept visible between the compact bundle suite surface and the narrower bundle-first branch.',
        'Use replay_shortcuts as the default next helper after the top-level suite catalog when no pinned bundle inputs, saved summary, or non-default repo root need to take precedence, because it keeps the newer shortcut-first issue #3 bridge visible before you decide whether to reopen the attached-page ladders, the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, or the bundle-first branch.',
        'Use contextual_flow when RepoRoot, SummaryPath, or fixed InputPath values already matter and you want the next helper surface to keep that context aligned while you choose between the attached-page ladders, replay shortcuts, replay route, the replay-route shortcut, the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, the bundle-first branch, or the safe-route helpers.',
        'Keep the Windows replay attached-html quickstart note, the Windows full-use attached-html route note, the validation-router attached-page quickstart note, the attached-page change-area quickstart note, the dedicated Google attached-page validation-flow note, the suite-catalog top-level attached-page catalog quickstart note, the suite-catalog guide, the suite-router handoff guide, the top-level attached-page bridge, the attached-bundle suite-surface note, and the validation-chain notes nearby when you want the written route beside these commands.'
    )
}
...
$entrypoints.recommended_next_reason = if ($entrypoints.recommended_next_key -eq 'attached_bundle_first') {
    'Explicit input paths are already in play, so stay pinned to the known three-page compatibility bundle and keep the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, and the replay-route shortcut nearby before widening back into the broader issue #3 helper chain.'
} elseif ($entrypoints.recommended_next_key -eq 'contextual_flow') {
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between the Windows full-use attached-html route, the route-level surface check, the Windows-to-validation-router bridge, the Windows full-use attached-html catalog quickstart, the Windows replay attached-html quickstart, the attached-page change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the validation-router attached-page quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the suite-router attached-page surface check, the suite-router handoff and its surface check, the top-level attached-page quickstart, the top-level attached-page surface check, the top-level attached-page catalog quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the issue-specific attached-page entrypoint, replay shortcuts, the next-step matrix, contextual flow, replay route, the replay-route shortcut, the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, attached bundle, or the safe-route helpers.'
} else {
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so jump straight from the suite catalog into replay shortcuts before widening back into the Windows full-use attached-html route, the route-level surface check, the Windows-to-validation-router bridge, the Windows full-use attached-html catalog quickstart, the Windows replay attached-html quickstart, the attached-page change-area quickstart, the broader attached-page flow helper, the dedicated Google attached-page flow helper, the validation-router attached-page quickstart, the suite-catalog-to-top-level attached-page catalog quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the suite-router attached-page surface check, the suite-router handoff and its surface check, the top-level attached-page quickstart, the top-level attached-page surface check, the top-level attached-page catalog quickstart, the top-level attached-page bridge, the issue-specific attached-page entrypoint, the next-step matrix, contextual flow, the replay-route shortcut, the compact attached-bundle suite surface, the pinned bundle surface checker, the pinned bundle flow helper, or the bundle-first branch.'
}
...
Write-Host (("  8. Google attached surface check:    {0}") -f $entrypoints.bridge_sequence.google_attached_html_surface_check)
Write-Host (("  9. Google attached flow:             {0}") -f $entrypoints.bridge_sequence.google_attached_html_validation_flow)
Write-Host ((" 10. Replay surface check:             {0}") -f $entrypoints.bridge_sequence.windows_replay_attached_html_surface_check)
Write-Host ((" 11. Windows catalog quickstart:       {0}") -f $entrypoints.bridge_sequence.windows_full_use_attached_html_catalog_quickstart)
Write-Host ((" 12. Replay attached quickstart:       {0}") -f $entrypoints.bridge_sequence.windows_replay_attached_html_quickstart)
Write-Host ((" 13. Validation-router quick:          {0}") -f $entrypoints.bridge_sequence.validation_router_attached_html_quickstart)
Write-Host ((" 14. Catalog top-level quickstart:     {0}") -f $entrypoints.bridge_sequence.suite_catalog_top_level_attached_html_catalog_quickstart)
Write-Host ((" 15. Catalog attached bridge:          {0}") -f $entrypoints.bridge_sequence.suite_catalog_attached_html_entrypoint)
Write-Host ((" 16. Router quickstart:                {0}") -f $entrypoints.bridge_sequence.suite_router_quickstart)
Write-Host ((" 17. Router attached quickstart:       {0}") -f $entrypoints.bridge_sequence.suite_router_attached_html_quickstart)
Write-Host ((" 18. Router attached surface check:    {0}") -f $entrypoints.bridge_sequence.suite_router_attached_html_surface_check)
Write-Host ((" 19. Top-level attached quickstart:    {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_quickstart)
Write-Host ((" 20. Top-level attached surface check: {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_surface_check)
Write-Host ((" 21. Top-level catalog quickstart:     {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_catalog_quickstart)
Write-Host ((" 22. Top-level shortcut:               {0}") -f $entrypoints.bridge_sequence.top_level_shortcut_entrypoint)
Write-Host ((" 23. Top-level attached bridge:        {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_entrypoint)
Write-Host ((" 24. Google flow:                      {0}") -f $entrypoints.bridge_sequence.google_flow)
Write-Host ((" 25. Google attached bridge:           {0}") -f $entrypoints.bridge_sequence.google_attached_html_entrypoint)
Write-Host ((" 26. Attached shortcut:                {0}") -f $entrypoints.bridge_sequence.attached_html_shortcut)
Write-Host ((" 27. Replay shortcuts:                 {0}") -f $entrypoints.bridge_sequence.replay_shortcuts)
Write-Host ((" 28. Next-step matrix:                 {0}") -f $entrypoints.bridge_sequence.suite_router_next_steps)
Write-Host ((" 29. Contextual flow:                  {0}") -f $entrypoints.bridge_sequence.contextual_flow)
Write-Host ((" 30. Suite handoff:                    {0}") -f $entrypoints.bridge_sequence.suite_router_handoff)
Write-Host ((" 31. Suite handoff check:              {0}") -f $entrypoints.bridge_sequence.suite_router_handoff_surface_check)
Write-Host ((" 32. Replay route:                     {0}") -f $entrypoints.bridge_sequence.replay_route)
Write-Host ((" 33. Replay-route shortcut:            {0}") -f $entrypoints.bridge_sequence.replay_route_shortcut)
Write-Host ((" 34. Bundle suite surface:             {0}") -f $entrypoints.bridge_sequence.attached_bundle_suite_surface)
Write-Host ((" 35. Bundle surface check:             {0}") -f $entrypoints.bridge_sequence.attached_bundle_surface_check)
Write-Host ((" 36. Bundle flow helper:               {0}") -f $entrypoints.bridge_sequence.attached_bundle_flow)
Write-Host ((" 37. Bundle-first route:               {0}") -f $entrypoints.bridge_sequence.attached_bundle_first)
Write-Host ''
...
Write-Host (("  Google attached surface:   {0}") -f $entrypoints.helper_commands.google_attached_html_surface_check)
...
Write-Host (("  Router quick check:        {0}") -f $entrypoints.helper_commands.suite_router_attached_html_surface_check)
Write-Host (("  Top-level quickstart:      {0}") -f $entrypoints.helper_commands.top_level_attached_html_quickstart)
Write-Host (("  Top-level quick check:     {0}") -f $entrypoints.helper_commands.top_level_attached_html_surface_check)
...
Write-Host (("  Suite handoff check:       {0}") -f $entrypoints.helper_commands.suite_router_handoff_surface_check)
Write-Host (("  Replay route:              {0}") -f $entrypoints.helper_commands.replay_route)
Write-Host (("  Replay-route shortcut:     {0}") -f $entrypoints.helper_commands.replay_route_shortcut)
Write-Host (("  Bundle suite surface:      {0}") -f $entrypoints.helper_commands.attached_bundle_suite_surface)
Write-Host (("  Bundle surface check:      {0}") -f $entrypoints.helper_commands.attached_bundle_surface_check)
Write-Host (("  Bundle flow helper:        {0}") -f $entrypoints.helper_commands.attached_bundle_flow)
