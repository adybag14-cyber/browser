...
$entrypoints = [ordered]@{
    ...
    purpose = 'Keep the exact top-level show_headed_validation_suites entrypoints, the Windows replay attached-html surface check, the Windows replay attached-html quickstart, the newer validation-router attached-page quickstart, the newer suite-catalog attached-page bridge, the newer suite-router attached-page quickstart, the newer top-level attached-page quickstart, the newer top-level shortcut and top-level attached-page bridges, the current Google flow helper, the issue-specific attached-page helpers, the current shortcut-first issue #3 replay helper, the compact next-step matrix, the context-preserving issue #3 replay flow, and the current safe-route replay helpers on one compact command surface before the route narrows into replay-route or the wrapper-heavy safe-route helpers.'
    repo_root = $RepoRoot
    ...
    notes = @(
        ...
        'Use attached_html_change_area when the next replay is already narrowed to attached-page follow-up but pinned bundle inputs are not yet locked, so the route can still prefer the Windows replay attached-html quickstart, the new validation-router attached-page quickstart, the new suite-catalog attached-page bridge, the top-level attached-page quickstart, and the newer top-level attached-page bridge before it falls back to the bundle-first branch.',
        ...
        'Use windows_replay_attached_html_quickstart as the default replay-side bridge when the next rerun already came through the Windows replay attached localhost branch and you want that narrower route reprinted before widening back into the suite-catalog or replay-shortcut helpers.',
        'Use replay_shortcuts as the default next helper after the top-level suite catalog when no pinned bundle inputs, repo-root override, or saved summary need to take precedence, because it keeps the newer shortcut-first issue #3 bridge visible before you decide whether to reopen the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the issue-specific attached-page entrypoint, the attached-page shortcut, the next-step matrix, or the bundle-first branch.',
        ...
        'Use suite_catalog_attached_html_entrypoint when you want the suite-catalog surface to stay focused on attached-page follow-up before you decide whether to narrow into the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the top-level attached-page quickstart, attached_html_shortcut, or widen back to replay_shortcuts, the next-step matrix, or the bundle-first branch.',
        ...
        'Use google_flow when you want the localhost-first issue #3 ladder printed before you decide whether to narrow into the shortcut-first replay helper, the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the issue-specific attached-page entrypoint, the next-step matrix, context-preserving flow, replay route, or the wrapper-heavy safe-route branches, while keeping LIGHTPANDA_REPO_ROOT aligned to the current non-default checkout when RepoRoot is already in play.',
        ...
        'Use replay_shortcuts as the default next helper after the top-level suite catalog, the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the top-level attached-page quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the suite-router attached-page quickstart, or attached-page helpers when the current route is already known to be issue #3 and no pinned bundle inputs, saved summary, or non-default repo root need to take precedence first.',
        ...
        'Use contextual_flow when RepoRoot, SummaryPath, or fixed InputPath values already matter and you want the next helper surface to keep that context aligned while you choose between the recommended runner, the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the issue-specific attached-page entrypoint, replay route, replay shortcuts, live trace, attached bundle, or later-stage follow-up commands.',
        ...
        'Use safe_route_entrypoints after the shortcut-first bridge, the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the issue-specific attached-page helpers, the next-step matrix, contextual flow, suite-router handoff, or replay-route helper when you want the current wrapper-heavy issue #3 path, notes, and next-state helpers surfaced in one place.',
        ...
        'Keep the Windows replay attached-html quickstart note, the Windows full-use attached-html route note, the top-level attached-page quickstart note, the validation-router attached-page quickstart note, the Windows replay quickstart note, the suite-router attached-page quickstart note, the suite-router bridge, the suite-catalog guide, the top-level attached-page bridge, and the validation-chain notes nearby when you want the written route beside these commands without reopening the broader Windows runbook first.'
    )
}
...
$entrypoints.recommended_next_reason = if ($entrypoints.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level shortcut bridge, the top-level attached-page bridge, the issue-specific attached-page entrypoint, replay shortcuts, the next-step matrix, replay route, attached bundle, or the safe-route helpers.'
} else {
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so jump straight from the suite catalog into replay shortcuts before widening back into the Windows replay attached-html quickstart, the validation-router attached-page quickstart, the suite-catalog attached-page bridge, the suite-router attached-page quickstart, the top-level attached-page quickstart, the top-level attached-page bridge, the issue-specific attached-page entrypoint, the next-step matrix, contextual flow, or the bundle-first branch.'
}
...
Write-Host (("  8. Catalog attached bridge:     {0}") -f $entrypoints.bridge_sequence.suite_catalog_attached_html_entrypoint)
Write-Host (("  9. Router quickstart:           {0}") -f $entrypoints.bridge_sequence.suite_router_quickstart)
Write-Host ((" 10. Router attached quickstart:  {0}") -f $entrypoints.bridge_sequence.suite_router_attached_html_quickstart)
Write-Host ((" 11. Top-level attached quick:    {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_quickstart)
Write-Host ((" 12. Top-level shortcut:          {0}") -f $entrypoints.bridge_sequence.top_level_shortcut_entrypoint)
Write-Host ((" 13. Top-level attached:          {0}") -f $entrypoints.bridge_sequence.top_level_attached_html_entrypoint)
Write-Host ((" 14. Google flow:                 {0}") -f $entrypoints.bridge_sequence.google_flow)
Write-Host ((" 15. Google attached bridge:      {0}") -f $entrypoints.bridge_sequence.google_attached_html_entrypoint)
Write-Host ((" 16. Attached shortcut:           {0}") -f $entrypoints.bridge_sequence.attached_html_shortcut)
Write-Host ((" 17. Replay shortcuts:            {0}") -f $entrypoints.bridge_sequence.replay_shortcuts)
Write-Host ((" 18. Next-step matrix:            {0}") -f $entrypoints.bridge_sequence.suite_router_next_steps)
Write-Host ((" 19. Contextual flow:             {0}") -f $entrypoints.bridge_sequence.contextual_flow)
Write-Host ((" 20. Suite handoff:               {0}") -f $entrypoints.bridge_sequence.suite_router_handoff)
Write-Host ((" 21. Replay route:                {0}") -f $entrypoints.bridge_sequence.replay_route)
Write-Host ((" 22. Bundle-first route:          {0}") -f $entrypoints.bridge_sequence.attached_bundle_first)
Write-Host ''
