...
$entrypoints = [ordered]@{
    ...
    notes = @(
        ...
        'Use replay_route after the shortcut-first bridge or next-step matrix when you want the smallest read-first helper that keeps the suite catalog entrypoints, Google flow, contextual flow, attached-bundle branch, and safe-route bridge on one surface before narrowing further.',
        ...
        'Use safe_route_entrypoints after the shortcut-first bridge, next-step matrix, contextual flow, suite-router handoff, or replay-route helper when you want the current wrapper-heavy issue #3 path, notes, and next-state helpers surfaced in one place.',
...
$entrypoints.recommended_next_reason = if ($entrypoints.recommended_next_key -eq 'attached_bundle_first') {
    ...
    'A non-default repo root or saved summary is already in play, so open the context-preserving helper next and keep that replay state aligned before choosing between the shortcut-first bridge, next-step matrix, replay route, attached bundle, or safe-route branches.'
} else {
    'No pinned bundle inputs, non-default repo root, or saved summary are in play yet, so jump straight from the suite catalog and Google flow into the shortcut-first replay helper before widening back into the next-step matrix, replay route, contextual flow, or the safe-route helpers.'
}
