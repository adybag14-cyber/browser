...
$references = @(
    ...
    (New-ValidationReference -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Kind "file" -Purpose "Launcher companion helper that keeps the wrapper-backed sidecar audit route visible from the replay ladder."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_replay_attached_html_quickstart.ps1" -Snippet 'Use attached_bundle_proof_surface_check and attached_bundle_proof_entrypoint when the replay is already pinned to the known three-page compatibility bundle and you want the executable proof-only checker and helper pair reprinted before or after the narrower bundle-first follow-up.' -Purpose "Replay quickstart helper notes preserve when to prefer the proof-entrypoint checker/helper pair beside the pinned bundle route."),
