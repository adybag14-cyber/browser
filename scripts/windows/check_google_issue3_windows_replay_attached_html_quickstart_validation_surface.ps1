...
$references = @(
    ...
    (New-ValidationReference -Path "docs/ISSUE3_REPLAY_ROUTE_BUNDLE_FIRST_BRIDGE.md" -Kind "file" -Purpose "Replay-route bundle-first bridge note kept nearby when the replay-side attached-html route re-enters the fixed three-page bundle branch before widening again."),
...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_REPLAY_ATTACHED_HTML_QUICKSTART.md" -Snippet 'docs/ISSUE3_REPLAY_ROUTE_BUNDLE_FIRST_BRIDGE.md' -Purpose "Replay attached-html quickstart keeps the replay-route bundle-first bridge note visible beside the pinned bundle proof surfaces before the route narrows into the fixed three-page bundle branch."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'Write-Host (("  Windows replay quick: {0}") -f $helper.helper_commands.windows_replay_quickstart)' -Purpose "Launcher companion helper output prints the Windows replay re-entry helper once preflight is complete."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_attached_pages_launcher_companion.ps1" -Snippet 'Write-Host (("  Replay-route helper: {0}") -f $helper.helper_commands.replay_route_shortcut)' -Purpose "Launcher companion helper output prints the narrower replay-route re-entry helper once preflight is complete."),
