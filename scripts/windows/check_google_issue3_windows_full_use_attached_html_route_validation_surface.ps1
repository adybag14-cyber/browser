...
$contentExpectations = @(
    ...
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_suite_router_quickstart.ps1' -Purpose "Route note keeps the shorter issue #3 quickstart visible when the broader Windows runbook has already narrowed replay to issue #3 but not yet all the way to the attached-html branch."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\check_google_issue3_suite_router_attached_html_quickstart_surface.ps1' -Purpose "Route note keeps the narrower suite-router attached-html surface checker visible before the attached-html branch is trusted from the suite-router side."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_suite_router_attached_html_quickstart.ps1' -Purpose "Route note keeps the narrower suite-router attached-html quickstart visible before the route narrows into the suite-catalog attached-html bridge or shorter shortcuts."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\check_google_issue3_suite_router_attached_html_quickstart_surface.ps1 -RepoRoot ''<repo-root>''' -Purpose "Route note keeps the repo-root-preserving suite-router attached-html surface checker visible when replay is already running from a non-default checkout."),
    (New-ValidationContentExpectation -Path "docs/ISSUE3_WINDOWS_FULL_USE_ATTACHED_HTML_ROUTE.md" -Snippet 'powershell -ExecutionPolicy Bypass -File .\scripts\windows\show_google_issue3_suite_router_attached_html_quickstart.ps1 -RepoRoot ''<repo-root>'' -SummaryPath ''<saved-summary-path>'' -InputPath ''<bundle-html-or-folder>''' -Purpose "Route note keeps the repo-root-preserving suite-router attached-html quickstart visible when replay is already pinned to a non-default checkout or bundle input."),
    ...
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'windows_full_use_attached_html_route_surface_check = Format-HelperCommandWithRepoRootEnv -ScriptName ''check_google_issue3_windows_full_use_attached_html_route_validation_surface.ps1'' -RepoRootOverride $RepoRoot' -Purpose "Route helper keeps the route-level surface checker repo-root-preserving when replay is running from a non-default checkout."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'attached_html_flow = Format-HelperCommandWithRepoRootEnv -ScriptName ''show_attached_html_validation_flow.ps1'' -Arguments $attachedHtmlFlowArguments -RepoRootOverride $RepoRoot' -Purpose "Route helper keeps the broader attached-page flow repo-root-preserving when the helper reprints that follow-up from a non-default checkout."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'Add-SharedArgument -Arguments $googleAttachedHtmlFlowArguments -Name RepoRoot -Value $RepoRoot' -Purpose "Route helper keeps the dedicated Google attached-page flow wired with an explicit RepoRoot argument before it narrows the replay further."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'suite_router_quickstart = Format-HelperCommand -ScriptName ''show_google_issue3_suite_router_quickstart.ps1'' -Arguments $bundleArguments' -Purpose "Route helper keeps the shorter issue #3 quickstart wired into its helper command map before the route narrows fully into the attached-html branch."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'Write-Host ((" 14. Issue #3 quickstart:   {0}") -f $route.helper_commands.suite_router_quickstart)' -Purpose "Route helper prints the shorter issue #3 quickstart on the surfaced Windows full-use route before the narrower top-level attached-html helpers take over."),
    (New-ValidationContentExpectation -Path "scripts/windows/show_google_issue3_windows_full_use_attached_html_route.ps1" -Snippet 'Use suite_router_quickstart when the broader Windows runbook or the top-level validation router already narrowed the replay to issue #3, but not yet all the way to the attached-html branch, and you want the shortest bridge back into the current replay helper stack before deciding whether to widen into the attached-page chain, replay shortcuts, or the safe-route map.' -Purpose "Route helper notes preserve when to reopen the shorter issue #3 quickstart before deciding whether to widen back into the attached-page chain."),
