...
fn walk(self: @This(), node: *Node, xpath_buffer: *std.ArrayList(u8), parent_name: ?[]const u8, visitor: anytype, index: usize, listener_targets: interactive.ListenerTargetMap) !void {
    ...
    var xpath_writer = std.Io.Writer.Allocating.fromArrayList(self.arena, xpath_buffer);
    try appendXPathSegment(node, &xpath_writer.writer, index);
    xpath_buffer.* = xpath_writer.toArrayList();
...
fn extractSelectOptions(node: *Node, page: *Page, arena: std.mem.Allocator) ![]OptionData {
    var options = std.ArrayListUnmanaged(OptionData).empty;
    var it = node.childrenIterator();
    while (it.next()) |child| {
        if (child.is(Element)) |el| {
            if (el.getTag() == .option) {
