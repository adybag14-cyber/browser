...
fn getAndMakeAppDir(allocator: Allocator) ?[]const u8 {
    ...
    std.Io.Dir.cwd().createDirPath(std.Options.debug_io, app_dir_path) catch |err| {
        allocator.free(app_dir_path);
...
fn getAppDataDir(allocator: Allocator, app_name: []const u8) ![]const u8 {
    switch (builtin.os.tag) {
        .linux, .freebsd, .netbsd, .openbsd, .dragonfly, .haiku, .serenity, .illumos, .solaris => {
            if (std.c.getenv("XDG_DATA_HOME")) |xdg| {
                return std.fmt.allocPrint(allocator, "{s}/{s}", .{ std.mem.span(xdg), app_name });
            }
            const home = std.c.getenv("HOME") orelse return error.EnvironmentVariableNotFound;
            return std.fmt.allocPrint(allocator, "{s}/.local/share/{s}", .{ std.mem.span(home), app_name });
        },
        .macos, .ios, .tvos, .watchos, .visionos => {
            const home = std.c.getenv("HOME") orelse return error.EnvironmentVariableNotFound;
            return std.fmt.allocPrint(allocator, "{s}/Library/Application Support/{s}", .{ std.mem.span(home), app_name });
        },
        .windows => {
            const base = std.c.getenv("LOCALAPPDATA") orelse std.c.getenv("APPDATA") orelse return error.EnvironmentVariableNotFound;
            return std.fmt.allocPrint(allocator, "{s}\\{s}", .{ std.mem.span(base), app_name });
        },
