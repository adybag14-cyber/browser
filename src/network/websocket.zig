...
pub const WsConnection = struct {
    ...
    pub fn init(socket: posix.socket_t, allocator: Allocator, json_version_response: []const u8, timeout_ms: u32) !WsConnection {
        const socket_flags = try compat_posix.fcntlGetFlags(socket);
    ...
    pub fn send(self: *WsConnection, data: []const u8) !void {
        ...
        defer if (changed_to_blocking) {
            ...
            compat_posix.fcntlSetFlags(self.socket, self.socket_flags) catch |err| {
                log.err(.app, "ws restore nonblocking", .{ .err = err });
            };
        };

        LOOP: while (pos < data.len) {
            const written = compat_posix.write(self.socket, data[pos..]) catch |err| switch (err) {
                error.WouldBlock => {
                    ...
                    try compat_posix.fcntlSetFlags(self.socket, self.socket_flags & ~@as(u32, @bitCast(posix.O{ .NONBLOCK = true })));
                    continue :LOOP;
                },
                else => return err,
            };
    ...
    pub fn getAddress(self: *WsConnection) !std.Io.net.IpAddress {
        ...
        switch (storage.family) {
            posix.AF.INET => {
                ...
                const addr_bytes: [4]u8 = @bitCast(sa.addr);
                return .{ .ip4 = .{ .bytes = addr_bytes, .port = std.mem.bigToNative(u16, sa.port) } };
            },
            posix.AF.INET6 => {
                const sa: *const posix.sockaddr.in6 = @ptrCast(@alignCast(&storage));
                return .{ .ip6 = .{ .bytes = sa.addr, .port = std.mem.bigToNative(u16, sa.port), .flow = sa.flowinfo, .interface = @enumFromInt(sa.scope_id) } };
            },
    ...
    pub fn shutdown(self: *WsConnection) void {
        compat_posix.shutdown(self.socket, .recv) catch {};
    }
    ...
    pub fn setBlocking(self: *WsConnection, blocking: bool) !void {
        if (blocking) {
            ...
            try compat_posix.fcntlSetFlags(self.socket, self.socket_flags);
        }
    }
};
