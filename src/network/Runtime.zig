...
conn_mutex: @import("lightpanda").compat_sync.Mutex = .{},
...
fn pipe2Compat(flags: posix.O) ![2]posix.fd_t {
...
pub fn init(allocator: Allocator, config: *const Config) !Runtime {
    ...
    const pipe = try pipe2Compat(.{ .NONBLOCK = true, .CLOEXEC = true });
...
pub fn bind(
    ...
    const server = try address.listen(std.Options.debug_io, .{
        .reuse_address = true,
        .kernel_backlog = @intCast(self.config.maxPendingConnections()),
    });
...
pub fn run(self: *Runtime) void {
    ...
    if (self.listener) |listener| {
        compat_posix.shutdown(listener.socket, .both) catch |err| blk: {
        ...
        compat_posix.close(listener.socket);
    }
}
...
fn wakeupPoll(self: *Runtime) void {
    _ = compat_posix.write(self.wakeup_pipe[1], &.{1}) catch {};
}
...
fn loadCerts(allocator: Allocator) !libcurl.CurlBlob {
    ...
    try bundle.rescan(allocator, std.Options.debug_io, std.Io.Timestamp.now(std.Options.debug_io, .real));
    ...
    ;
    try aw.ensureUnusedCapacity(buffer_size);
    ...
    while (it.next()) |index| {
        ...
        var line_writer = LineWriter{ .inner = &aw.writer };
        const encoded_len = encoder.calcSize(cert.slice.end - index.*);
        const encoded = try allocator.alloc(u8, encoded_len);
        defer allocator.free(encoded);
        const encoded_slice = encoder.encode(encoded, bytes[index.*..cert.slice.end]);
        try line_writer.writeAll(encoded_slice);
        try writer.writeAll("\n-----END CERTIFICATE-----\n");
    }

    // Final encoding should not be larger than our initial size estimate
    lp.assert(buffer_size > aw.written().len, "Http loadCerts", .{ .estimate = buffer_size, .len = aw.written().len });
