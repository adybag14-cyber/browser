...
pub const Extension = union(enum) {
    ...
    const Kind = blk: {
        ...
        var values: [fields.len]std.math.IntFittingRange(0, if (fields.len == 0) 0 else fields.len - 1) = undefined;
        ...
        break :blk @Enum(
            std.math.IntFittingRange(0, if (fields.len == 0) 0 else fields.len - 1),
            .exhaustive,
            &names,
            &values,
        );
