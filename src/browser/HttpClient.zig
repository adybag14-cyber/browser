...
transfer_pool: std.heap.memory_pool.Managed(Transfer),
...
pub fn init(allocator: Allocator, network: *Network) !*Client {
    var transfer_pool = std.heap.memory_pool.Managed(Transfer).init(allocator);
...
pub const RequestCookie = struct {
    ...
    pub fn headersForRequest(self: *const RequestCookie, temp: Allocator, url: [:0]const u8, headers: *Net.Headers) !void {
        ...
        if (aw.written().len > 0) {
            try aw.writer.writeByte(0); // null terminate
            headers.cookies = @as([*c]const u8, @ptrCast(aw.written().ptr));
        }
    }
};
...
pub const Transfer = struct {
    ...
    pub fn replaceRequestHeaders(self: *Transfer, allocator: Allocator, headers: []const Net.Header) !void {
        ...
        for (headers) |hdr| {
            ...
            try std.fmt.format(&buf.writer, "{s}: {s}", .{ hdr.name, hdr.value });
            try buf.writer.writeByte(0); // null terminated
            try new_headers.add(buf.written()[0 .. buf.written().len - 1 :0]);
        }
