...
$outPng = Join-Path $root "wrapped-flow.png"
$browserOut = Join-Path $root "wrapped.browser.stdout.txt"
$browserErr = Join-Path $root "wrapped.browser.stderr.txt"
$serverOut = Join-Path $root "wrapped.server.stdout.txt"
$serverErr = Join-Path $root "wrapped.server.stderr.txt"
...
try {
  ...
  if (-not $ready) { throw "wrapped inline probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-wrap"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  try {
    ...
    if (-not $pngReady) { throw "wrapped inline screenshot did not become ready" }
    ...
    try {
      ...
      if ($null -eq $red.min_y -or $null -eq $green.min_y) { throw "wrapped inline probe did not find the red and green chips" }

      $later = $green
      $firstRowBlack = Count-BlackInBand $bmp $red.min_x ([Math]::Max(0, $red.min_y - 8)) ([Math]::Min($bmp.Height - 1, $red.max_y + 4))
      $laterRowBlack = Count-BlackInBand $bmp $later.min_x ([Math]::Max(0, $later.min_y - 8)) ([Math]::Min($bmp.Height - 1, $later.max_y + 4))
      $belowBlack = Count-BlackInBand $bmp $bmp.Width ([Math]::Min($bmp.Height - 1, $later.max_y + 96)) ([Math]::Min($bmp.Height - 1, $later.max_y + 170))

      $wrapped = $later.min_y -gt ($red.min_y + 8)
      $firstRowWorked = $firstRowBlack -ge 20
      $laterRowWorked = $laterRowBlack -ge 20
      $belowWorked = $belowBlack -ge 20
      if (-not $wrapped) { throw "wrapped inline probe did not observe a lower mixed row" }
      if (-not $firstRowWorked) { throw "wrapped inline probe did not observe direct text on the first mixed row" }
      if (-not $laterRowWorked) { throw "wrapped inline probe did not observe direct text on the lower mixed row" }
      if (-not $belowWorked) { throw "wrapped inline probe did not observe the following paragraph below the wrapped content" }
