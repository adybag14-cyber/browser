...
$outPng = Join-Path $root "checkbox-link.png"
$browserOut = Join-Path $root "checkbox-link.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-link.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-link.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-link.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-link.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "checkbox-link probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-link"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","--browser_mode","headed","http://127.0.0.1:$port/checkbox-link.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "checkbox-link screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "checkbox-link window handle not found" }
  ...
  if ($null -eq $checkbox.min_y) { throw "checkbox-link probe did not isolate the checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterClick -like "Inline Checkbox True*") {
      $clickWorked = $true
      break
    }
  }
  if (-not $clickWorked) { throw "checkbox-link probe checkbox did not toggle true on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterSpace -like "Inline Checkbox False*") {
      $spaceWorked = $true
      break
    }
  }
  if (-not $spaceWorked) { throw "checkbox-link probe checkbox did not toggle false on space" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeEnter
  $titleAfterLink = $titleAfterSpace
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
