...
$outPng = Join-Path $root "checkbox-button-link.png"
$browserOut = Join-Path $root "checkbox-button-link.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-button-link.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-button-link.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-button-link.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-button-link.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline checkbox button link probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-button-link"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","--browser_mode","headed","http://127.0.0.1:$port/checkbox-button-link.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline checkbox button link screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox button link window handle not found" }
  ...
  try {
    ...
    for ($y = 0; $y -lt $bmp.Height; $y++) {
      for ($x = 0; $x -lt $bmp.Width; $x++) {
        ...
        } elseif ($c.R -ge 180 -and $c.R -le 210 -and $c.G -ge 40 -and $c.G -le 85 -and $c.B -ge 165 -and $c.B -le 200) {
  ...
  if ($null -eq $checkbox.min_y) { throw "inline checkbox button link probe did not isolate the checkbox control" }
  if ($null -eq $button.min_y) { throw "inline checkbox button link probe did not isolate the button control" }

  Show-SmokeWindow $hwnd
  Start-Sleep -Milliseconds 250
  $titleBefore = Get-SmokeWindowTitle $hwnd

  $centerX = [int][Math]::Floor(($checkbox.min_x + $checkbox.max_x) / 2)
  $centerY = [int][Math]::Floor(($checkbox.min_y + $checkbox.max_y) / 2)
  $checkboxClickPoint = Invoke-SmokeClientClick $hwnd $centerX $centerY
  $titleAfterCheckbox = $titleBefore
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterCheckbox = Get-SmokeWindowTitle $hwnd
    if ($titleAfterCheckbox -like "Dense Checkbox True*") {
      $checkboxWorked = $true
      break
    }
  }
  if (-not $checkboxWorked) { throw "inline checkbox button link probe checkbox did not toggle true on click" }
  ...
  $titleAfterButton = $titleAfterCheckbox
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterButton = Get-SmokeWindowTitle $hwnd
    if ($titleAfterButton -like "Dense Checkbox Button 1*") {
      $buttonWorked = $true
      break
    }
  }
  if (-not $buttonWorked) { throw "inline checkbox button link probe button did not activate on click" }
  ...
  [ordered]@{
    ...
    button_click_screen = $buttonClickPoint
