...
$outPng = Join-Path $root "dense-focus.png"
$browserOut = Join-Path $root "dense-focus.browser.stdout.txt"
$browserErr = Join-Path $root "dense-focus.browser.stderr.txt"
$serverOut = Join-Path $root "dense-focus.server.stdout.txt"
$serverErr = Join-Path $root "dense-focus.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/dense-focus.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "dense inline focus probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-dense-focus"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","--browser_mode","headed","http://127.0.0.1:$port/dense-focus.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "dense inline focus screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "dense inline focus window handle not found" }
  ...
  if ($null -eq $button.min_y) { throw "dense inline focus probe did not isolate the button fragment" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterButton -like "Dense Button 1*") {
      $buttonWorked = $true
      break
    }
  }
  if (-not $buttonWorked) { throw "dense inline focus probe button activation did not work" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeText "QZ"
  $titleAfterInput = $titleAfterButton
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterInput = Get-SmokeWindowTitle $hwnd
    if ($titleAfterInput -like "Dense Input QZ*") {
      $inputWorked = $true
      break
    }
  }
  if (-not $inputWorked) { throw "dense inline focus probe input typing did not work after tab" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeEnter
  $titleAfterLink = $titleAfterInput
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
