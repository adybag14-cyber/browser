...
$outPng = Join-Path $root "radio-button-link.png"
$browserOut = Join-Path $root "radio-button-link.browser.stdout.txt"
$browserErr = Join-Path $root "radio-button-link.browser.stderr.txt"
$serverOut = Join-Path $root "radio-button-link.server.stdout.txt"
$serverErr = Join-Path $root "radio-button-link.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/radio-button-link.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline radio button link probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-radio-button-link"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","--browser_mode","headed","http://127.0.0.1:$port/radio-button-link.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline radio button link screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline radio button link window handle not found" }
  ...
  if ($null -eq $radio.min_y) { throw "inline radio button link probe did not isolate the radio control" }
  if ($null -eq $button.min_y) { throw "inline radio button link probe did not isolate the button control" }

  Show-SmokeWindow $hwnd
  Start-Sleep -Milliseconds 250
  $titleBefore = Get-SmokeWindowTitle $hwnd

  $radioCenterX = [int][Math]::Floor(($radio.min_x + $radio.max_x) / 2)
  $radioCenterY = [int][Math]::Floor(($radio.min_y + $radio.max_y) / 2)
  $radioClickPoint = Invoke-SmokeClientClick $hwnd $radioCenterX $radioCenterY
  $titleAfterRadio = $titleBefore
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterRadio = Get-SmokeWindowTitle $hwnd
    if ($titleAfterRadio -like "Dense Radio true*") {
      $radioWorked = $true
      break
    }
  }
  if (-not $radioWorked) { throw "inline radio button link probe radio did not select on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterButton -like "Dense Radio Button 1*") {
      $buttonWorked = $true
      break
    }
  }
  if (-not $buttonWorked) { throw "inline radio button link probe button did not activate on click" }
  ...
  [ordered]@{
    ...
    radio_click_screen = $radioClickPoint
