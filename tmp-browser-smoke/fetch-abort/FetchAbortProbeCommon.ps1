...
$script:Root = Join-Path $script:Repo "tmp-browser-smoke\fetch-abort"
...
function Start-FetchAbortServer([int]$Port, [string]$Stdout, [string]$Stderr) {
  return Start-Process -FilePath "python" -ArgumentList (Join-Path $script:Root "fetch_abort_server.py"),"$Port" -WorkingDirectory $script:Root -PassThru -RedirectStandardOutput $Stdout -RedirectStandardError $Stderr
}
