...
$port = 8164
$serverOut = Join-Path $root "private-font-woff2.server.stdout.txt"
$serverErr = Join-Path $root "private-font-woff2.server.stderr.txt"
...
function Run-BrowserCapture([string]$Name, [string]$Url) {
  $profileRoot = Join-Path $root ("profile-private-font-woff2-" + $Name)
  $appDataRoot = Join-Path $profileRoot "lightpanda"
  $outPng = Join-Path $root ("private-font-woff2-" + $Name + ".png")
  $browserOut = Join-Path $root ("private-font-woff2-" + $Name + ".browser.stdout.txt")
  $browserErr = Join-Path $root ("private-font-woff2-" + $Name + ".browser.stderr.txt")
...
try {
  ...
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 250
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/private-woff2-loaded.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "woff2 private font render server did not become ready" }

  $loaded = Run-BrowserCapture "loaded" "http://127.0.0.1:$port/private-woff2-loaded.html"
  $missing = Run-BrowserCapture "missing" "http://127.0.0.1:$port/private-woff2-missing.html"
  $widthDelta = [math]::Abs([int]$loaded.bounds.width - [int]$missing.bounds.width)
  $fontWorked = $widthDelta -ge 10
  if (-not $fontWorked) {
    throw "private woff2 font render probe did not observe a sufficient width delta"
  }
