...
$port = 8163
$serverOut = Join-Path $root "private-font.server.stdout.txt"
$serverErr = Join-Path $root "private-font.server.stderr.txt"
...
function Run-BrowserCapture([string]$Name, [string]$Url) {
  $profileRoot = Join-Path $root ("profile-private-font-" + $Name)
  $appDataRoot = Join-Path $profileRoot "lightpanda"
  $outPng = Join-Path $root ("private-font-" + $Name + ".png")
  $browserOut = Join-Path $root ("private-font-" + $Name + ".browser.stdout.txt")
  $browserErr = Join-Path $root ("private-font-" + $Name + ".browser.stderr.txt")
...
try {
  ...
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 250
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/private-loaded.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "private font render server did not become ready" }

  $loaded = Run-BrowserCapture "loaded" "http://127.0.0.1:$port/private-loaded.html"
  ...
  if (-not $fontWorked) {
    throw "private font render probe did not observe a sufficient width delta"
  }
  ...
  $serverCommand = Get-ProcessCommandLine $server.Id
