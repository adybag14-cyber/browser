...
$port = 8163
$serverOut = Join-Path $root "private-font-fallback.server.stdout.txt"
$serverErr = Join-Path $root "private-font-fallback.server.stderr.txt"
...
function Run-BrowserCapture([string]$Name, [string]$Url) {
  $profileRoot = Join-Path $root ("profile-private-font-fallback-" + $Name)
  $appDataRoot = Join-Path $profileRoot "lightpanda"
  $outPng = Join-Path $root ("private-font-fallback-" + $Name + ".png")
  $browserOut = Join-Path $root ("private-font-fallback-" + $Name + ".browser.stdout.txt")
  $browserErr = Join-Path $root ("private-font-fallback-" + $Name + ".browser.stderr.txt")
...
try {
  ...
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 250
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/private-fallback.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "private fallback font render server did not become ready" }

  $fallback = Run-BrowserCapture "fallback" "http://127.0.0.1:$port/private-fallback.html"
  $missing = Run-BrowserCapture "missing" "http://127.0.0.1:$port/private-missing.html"
  $widthDelta = [math]::Abs([int]$fallback.bounds.width - [int]$missing.bounds.width)
  $fontWorked = $widthDelta -ge 20
  if (-not $fontWorked) {
    throw "private fallback font render probe did not observe a sufficient width delta"
  }
