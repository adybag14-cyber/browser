...
$browserOut = Join-Path $root "google-fixture.browser.stdout.txt"
$browserErr = Join-Path $root "google-fixture.browser.stderr.txt"
$serverOut = Join-Path $root "google-fixture.server.stdout.txt"
$serverErr = Join-Path $root "google-fixture.server.stderr.txt"
$pngPath = Join-Path $root "google-fixture.before.png"
...
try {
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/google-home.html","--window_width","860","--window_height","720","--screenshot_png",$pngPath -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
  ...
  $titleAfterType = Wait-ForTitleLike $hwnd "TYPED:lightpanda*"
  $typedWorked = $null -ne $titleAfterType
  if (-not $typedWorked) { throw "google fixture input did not receive typed text" }

  Send-SmokeEnter
  $titleAfterSubmit = Wait-ForTitleLike $hwnd "SUBMIT:lightpanda*"
  $submittedWorked = $null -ne $titleAfterSubmit
  if (-not $submittedWorked) { throw "google fixture Enter did not trigger submit title" }
} catch {
