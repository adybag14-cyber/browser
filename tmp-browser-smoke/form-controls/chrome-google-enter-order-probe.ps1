...
try {
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/google-enter-order.html","--window_width","420","--window_height","520","--screenshot_png",$pngPath -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
  ...
  Send-SmokeText "n"
  $titleAfterType = Wait-ForTitleLike $hwnd "Google Enter VALUE:n*"
  $typedWorked = $null -ne $titleAfterType
  if (-not $typedWorked) { throw "google enter probe input did not receive typed text" }

  Send-SmokeEnter
  $titleAfterSubmit = Wait-ForTitleLike $hwnd "Submitted *"
  if (Test-Path $serverErr) {
    $serverLog = Get-Content $serverErr -Raw
    $serverSawSubmit = $serverLog -match 'GOOGLE_ENTER_SUBMIT q=n phase=keypress'
    $submitLog = ($serverLog -split "`r?`n" | Where-Object { $_ -like "GOOGLE_ENTER_SUBMIT*" } | Select-Object -Last 1)
    if ($submitLog) {
      $logWorked = $submitLog -match 'events=.*KD:Enter:n.*KP:Enter:n.*SUBMIT:n'
    }
  }
  $submittedWorked = ($null -ne $titleAfterSubmit) -or $serverSawSubmit
  $phaseWorked = $serverSawSubmit
  if (-not $submittedWorked) { throw "pressing Enter did not submit the Google-style form" }
  if (-not $phaseWorked) { throw "submit happened before the keypress phase was observed" }
  if (-not $logWorked) { throw "submit log did not preserve the expected Enter ordering" }
} catch {
  ...
  [ordered]@{
    ...
    phase_worked = $phaseWorked
    log_worked = $logWorked
