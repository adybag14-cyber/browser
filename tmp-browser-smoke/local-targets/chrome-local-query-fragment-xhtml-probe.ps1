...
$config = Resolve-TabProbeConfig -StartPath $PSScriptRoot -RepoRoot $RepoRoot -BrowserExe $BrowserExe -ProfileName "profile-local-query-fragment-xhtml"
$root = Join-Path $config.RepoRoot "tmp-browser-smoke\local-targets"
$cases = @(
  [ordered]@{
    name = "query"
    target = "tmp-browser-smoke\local-targets\query-fragment-local-target.xhtml?case=1"
    expected_title = "Local XHTML Target query case=1"
  },
  [ordered]@{
    name = "fragment"
    target = "tmp-browser-smoke\local-targets\query-fragment-local-target.xhtml#focus-probe"
    expected_title = "Local XHTML Target fragment focus-probe"
  }
)
...
try {
  ...
  foreach ($case in $cases) {
    $browserOut = Join-Path $root ("chrome-local-query-fragment-xhtml.{0}.browser.stdout.txt" -f $case.name)
    $browserErr = Join-Path $root ("chrome-local-query-fragment-xhtml.{0}.browser.stderr.txt" -f $case.name)
    ...
    try {
      ...
      if ($hwnd -eq [IntPtr]::Zero) {
        throw "local xhtml $($case.name) target window handle not found"
      }

      Show-SmokeWindow $hwnd
      $title = Wait-TabTitle $browser.Id $case.expected_title -Attempts 40 -PollMilliseconds $PollMilliseconds
      if (-not $title) {
        throw "local xhtml $($case.name) target did not reach expected title"
      }
    } catch {
