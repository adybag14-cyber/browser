...
$pageUrl = "http://$Host`:$Port/flex-order.html"
...
try {
  ...
  if (-not (Wait-LightpandaHttpReady -Url $pageUrl -TimeoutSeconds $ServerReadyTimeoutSeconds -PollMilliseconds $PollMilliseconds)) { throw "flex order smoke server did not become ready" }
  $env:APPDATA = $profileRoot
  $env:LOCALAPPDATA = $profileRoot
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","--browser_mode","headed",$pageUrl,"--window_width","480","--window_height","240","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  try {
    ...
    if (-not (Wait-TabTitle -ProcessId $browser.Id -Needle "Flex Order - Lightpanda Browser" -Attempts 20 -PollMilliseconds $PollMilliseconds)) { throw "flex order initial title did not stabilize" }
    ...
    $afterTitle = Wait-TabTitle -ProcessId $browser.Id -Needle "Flex Order Blue Target - Lightpanda Browser" -Attempts 30 -PollMilliseconds $PollMilliseconds
