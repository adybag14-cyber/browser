...
$pageUrl = "http://$Host`:$port/webgl-triangle.html"
$outPng = Join-Path $root 'canvas-webgl-triangle.png'
$browserOut = Join-Path $root 'canvas-webgl-triangle.browser.stdout.txt'
$browserErr = Join-Path $root 'canvas-webgl-triangle.browser.stderr.txt'
$serverOut = Join-Path $root 'canvas-webgl-triangle.server.stdout.txt'
$serverErr = Join-Path $root 'canvas-webgl-triangle.server.stderr.txt'
$profileRoot = Join-Path $root 'profile-canvas-webgl-triangle'
...
try {
  ...
  if (-not $ready) { throw 'canvas webgl triangle server did not become ready' }
  ...
  for ($i = 0; $i -lt $WindowReadyAttempts; $i++) {
    ...
    if ($proc -and $proc.MainWindowHandle -ne 0 -and $proc.MainWindowTitle -like '*Canvas WebGL Triangle Ready*') {
      $titleReady = $true
      break
    }
  }
  if (-not $titleReady) { throw 'webgl triangle page did not reach the ready title' }

  $pngReady = Wait-LightpandaFileReady -Path $outPng -Attempts $ScreenshotReadyAttempts -PollMilliseconds $PollMilliseconds
  if (-not $pngReady) { throw 'canvas webgl triangle screenshot did not become ready' }
  ...
  try {
    ...
    for ($y = 0; $y -lt $bmp.Height; $y++) {
      for ($x = 0; $x -lt $bmp.Width; $x++) {
        $c = $bmp.GetPixel($x, $y)
        if (([math]::Abs($c.R - 255) -le 20) -and ($c.G -le 40) -and ($c.B -le 40)) { $redCount++ }
    ...
    $result = [ordered]@{
      ...
      triangle_worked = $titleReady -and ($redCount -gt 200) -and ($whiteCount -gt 2000)
    }
    if (-not $result.triangle_worked) { throw 'webgl triangle pixels were not observed in screenshot' }
