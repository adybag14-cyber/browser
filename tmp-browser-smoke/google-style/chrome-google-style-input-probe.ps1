...
$root = "C:\Users\adyba\src\lightpanda-browser\tmp-browser-smoke\google-style"
$repo = "C:\Users\adyba\src\lightpanda-browser"
$port = 8158
$browserExe = Join-Path $repo "zig-out\bin\lightpanda.exe"
$pagePath = "src/browser/tests/page/headed_google_style_input_probe.html"
$pageUrl = "http://127.0.0.1:$port/$pagePath"
$outPng = Join-Path $root "google-style-input.png"
$browserOut = Join-Path $root "google-style-input.browser.stdout.txt"
$browserErr = Join-Path $root "google-style-input.browser.stderr.txt"
$serverOut = Join-Path $root "google-style-input.server.stdout.txt"
$serverErr = Join-Path $root "google-style-input.server.stderr.txt"
...
function Wait-ForWindowTitleLike([IntPtr]$Hwnd, [string]$Pattern, [int]$Attempts = 40, [int]$SleepMs = 150) {
...
try {
  ...
  $profileRoot = Join-Path $root "profile-google-style-input"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse",$pageUrl,"--window_width","860","--window_height","640","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
  ...
  Send-SmokeText "Query"
  $titleAfterType = Wait-ForWindowTitleLike $hwnd "typed:Query*"
  $typedWorked = $null -ne $titleAfterType
  if (-not $typedWorked) { throw "google-style probe did not preserve full typed text through focus churn" }

  Send-SmokeEnter
  $titleAfterSubmit = Wait-ForWindowTitleLike $hwnd "submitted:Query*"
  $submittedWorked = $null -ne $titleAfterSubmit
  if (-not $submittedWorked) { throw "google-style probe did not submit after Enter" }
}
catch {
