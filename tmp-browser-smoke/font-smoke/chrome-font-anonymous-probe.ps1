...
"@ | Set-Content -Path (Join-Path $appDataRoot 'browse-settings-v1.txt') -NoNewline
  ...
  if (-not $fontEntry) { throw 'font anonymous probe did not capture the font request' }
  if (-not $loadedEntry) { throw 'font anonymous probe did not capture the load completion signal' }
} catch {
