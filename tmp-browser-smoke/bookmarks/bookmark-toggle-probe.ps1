...
$config = Resolve-TabProbeConfig -StartPath $PSScriptRoot -RepoRoot $RepoRoot -BrowserExe $BrowserExe -ProfileName "profile-bookmark-toggle"
...
$readyPng = Join-Path $root "bookmark-toggle.ready.png"
$browserOut = Join-Path $root "bookmark-toggle.browser.stdout.txt"
$browserErr = Join-Path $root "bookmark-toggle.browser.stderr.txt"
$serverOut = Join-Path $root "bookmark-toggle.server.stdout.txt"
$serverErr = Join-Path $root "bookmark-toggle.server.stderr.txt"
...
try {
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "bookmark toggle probe window handle not found" }
