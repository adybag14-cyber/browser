...
$pngPath = Join-Path $root "chrome-google-home-title.before.png"
$url = "http://127.0.0.1:$port/google_home_title_probe.html"
...
try {
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse",$url,"--window_width","1280","--window_height","720","--screenshot_png",$pngPath -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
  ...
  $focusedTitle = Wait-ForTitleLike $hwnd "*|Q=INPUT:q:*|*" 20 250
  $focusedReady = $null -ne $focusedTitle
  if (-not $focusedReady) { throw "google title probe query input was not bound in the headed window" }

  Send-SmokeText "n"
  $titleAfterType = Wait-ForTitleLike $hwnd "*|V=n|*" 20 250
  $typedWorked = $null -ne $titleAfterType
  if (-not $typedWorked) { throw "google title probe typed text did not land in the query input" }

  Send-SmokeEnter
  $titleAfterSubmit = Wait-ForTitleLike $hwnd "SUBMIT:n*" 20 250
  $submittedWorked = $null -ne $titleAfterSubmit
  if (-not $submittedWorked) { throw "google title probe Enter did not trigger submit state" }
} catch {
