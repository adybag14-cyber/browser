...
def normalize_bundle_root(root: Path) -> tuple[Path, list[Path]]:
    ...
    if resolved_root.is_dir():
        html_files = sorted(path for path in resolved_root.rglob("*") if path.is_file() and is_html_export(path))
        return resolved_root, html_files

    if resolved_root.is_file() and is_html_export(resolved_root):
...
def normalize_selected_html_files(selected_files: list[Path]) -> tuple[Path, list[Path]]:
    ...
    for candidate in selected_files:
        ...
        if not is_html_export(resolved):
            raise ValueError(f"selected file is not an .html or .htm export: {resolved}")
        if resolved in seen:
            continue
        seen.add(resolved)
...
def classify_reference_value(reference: str) -> tuple[str, str, bool] | None:
    ...
    if not normalized or normalized in (".", ".."):
        return None
    return ("local", normalized, rooted)
...
def extract_reference_candidates(content: str) -> tuple[list[tuple[str, bool]], list[str]]:
    ...
    def add_candidate(value: str) -> None:
        ...
        if classified is None:
            return
        kind, normalized, rooted = classified
        if kind == "local":
            candidate = (normalized, rooted)
...
def build_asset_audit(root: Path | None = None, *, selected_files: list[Path] | None = None) -> dict[str, object]:
    ...
    for html_path in manifest:
        ...
        while pending:
            ...
            for reference, rooted in local_references:
                base_dir = bundle_root if rooted else current_path.parent
                resolved = ensure_within_root(bundle_root, base_dir / unquote(reference))
                display_path = unquote(reference) if rooted else Path(current_relative).parent.joinpath(unquote(reference)).as_posix()
