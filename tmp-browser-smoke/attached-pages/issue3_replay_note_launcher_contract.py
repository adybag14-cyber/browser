...
SUITE_ROUTER_ENTRYPOINT_GUIDE_NOTE = "docs/ISSUE3_SUITE_ROUTER_ENTRYPOINT_GUIDE.md"
SUITE_ROUTER_QUICKSTART_SURFACE_CHECK = (
    "check_google_issue3_suite_router_quickstart_validation_surface.ps1"
)
SUITE_ROUTER_QUICKSTART_HELPER = "show_google_issue3_suite_router_quickstart.ps1"
...
def audit_paths(paths: list[Path], repo_root: Path) -> dict[str, object]:
    ...
    counts = {
        ...
        "suite_router_entrypoint_guide_note_count": 0,
        "suite_router_quickstart_surface_check_count": 0,
        "suite_router_quickstart_helper_count": 0,
    ...
    for path in paths:
        ...
        hits = {
            ...
            "suite_router_entrypoint_guide_note_references": [],
            "suite_router_quickstart_surface_checks": [],
            "suite_router_quickstart_helpers": [],
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            if SUITE_ROUTER_ENTRYPOINT_GUIDE_NOTE in line:
                hits["suite_router_entrypoint_guide_note_references"].append(
                    {"line_number": line_number, "line": stripped}
                )
            if SUITE_ROUTER_QUICKSTART_SURFACE_CHECK in line:
                hits["suite_router_quickstart_surface_checks"].append(
                    {"line_number": line_number, "line": stripped}
                )
            if SUITE_ROUTER_QUICKSTART_HELPER in line:
                hits["suite_router_quickstart_helpers"].append(
                    {"line_number": line_number, "line": stripped}
                )
        ...
        counts["suite_router_entrypoint_guide_note_count"] += len(
            hits["suite_router_entrypoint_guide_note_references"]
        )
        counts["suite_router_quickstart_surface_check_count"] += len(
            hits["suite_router_quickstart_surface_checks"]
        )
        counts["suite_router_quickstart_helper_count"] += len(
            hits["suite_router_quickstart_helpers"]
        )
...
def failure_reasons(audit: dict[str, object]) -> list[str]:
    ...
    if audit["suite_router_entrypoint_guide_note_count"] == 0:
        reasons.append("replay notes do not keep the suite-router entrypoint guide visible")
    if audit["suite_router_quickstart_surface_check_count"] == 0:
        reasons.append("replay notes do not keep the suite-router quickstart surface checker visible")
    if audit["suite_router_quickstart_helper_count"] == 0:
        reasons.append("replay notes do not keep the suite-router quickstart helper visible")
...
def render_text(audit: dict[str, object], reasons: list[str]) -> str:
    lines = [
        ...
        f"Suite-router entrypoint guide references: {audit['suite_router_entrypoint_guide_note_count']}",
        f"Suite-router quickstart surface-check references: {audit['suite_router_quickstart_surface_check_count']}",
        f"Suite-router quickstart helper references: {audit['suite_router_quickstart_helper_count']}",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "the suite-catalog entrypoints surface checker and helper, the suite-router entrypoint guide, the suite-router quickstart surface checker and helper, "
            "the replay-route shortcut companion, the replay-quickstart shortcut note, the replay-shortcuts helpers, or the safe-route entrypoints helper."
        )
    )
