...
New-ValidationContentExpectation -Path "docs/ISSUE3_RUNTIME_REENTRY_GATES.md" -Snippet "scripts/check_linux_build_readiness.py" -Purpose "gate note readiness"
...
New-ValidationContentExpectation -Path "docs/WINDOWS_FULL_USE.md" -Snippet "google-form-controls-enter-order" -Purpose "windows google route"
...
class GoogleIssue3RuntimeRevalidationSurfaceTest(unittest.TestCase):
    ...
    def test_surface_checker_keeps_key_content_expectations_in_scope(self) -> None:
        for fragment in (
            ...
            'scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet \'linux_build_readiness_skip_zig = $linuxBuildReadinessSkipZigCommand\'',
            'scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet \'surface_check = Format-RepoRootCommand -ScriptPath "scripts\\\\windows\\\\check_google_issue3_enter_submit_runtime_revalidation_surface.ps1"\'',
            'scripts/windows/show_google_issue3_enter_submit_runtime_revalidation.ps1" -Snippet "Stale queued suppression entries do not drop real later text_input bytes."',
        ):
            self.assertIn(fragment, self.surface_checker)

    def test_gate_note_keeps_runtime_checker_and_readiness_route_visible(self) -> None:
        for fragment in (
            "docs/ISSUE3_ENTER_SUBMIT_RUNTIME_REVALIDATION.md",
