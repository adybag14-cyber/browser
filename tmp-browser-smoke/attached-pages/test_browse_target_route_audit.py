...
from browse_target_route_audit import EXPECTATIONS, audit


class BrowseTargetRouteAuditTests(unittest.TestCase):
    ...
    def test_audit_reports_missing_snippet_for_existing_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            repo_root = Path(tmp_dir)
            content = "\n\n".join(
                "# missing loopback classifier" if index == 2 else expectation["snippet"]
            ...
            self.assertFalse(result["checks"][2]["present"])
            self.assertTrue(result["checks"][2]["exists"])
    ...
    def test_audit_keeps_attached_page_and_localhost_surfaces_in_scope(self) -> None:
        covered_labels = {expectation["label"] for expectation in EXPECTATIONS}
        self.assertTrue(
            {
                "bare_local_html_helper",
                "implicit_loopback_helper",
                "attached_html_local_path_test",
                "scheme_less_localhost_loopback_test",
