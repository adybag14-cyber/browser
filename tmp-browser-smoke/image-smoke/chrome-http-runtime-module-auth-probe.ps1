...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $ready = Wait-LightpandaHttpReady -Url "http://127.0.0.1:$port/auth-module-page.html" -TimeoutSeconds $ServerReadyTimeoutSeconds -PollMilliseconds $PollMilliseconds
  ...
  if (-not $pngReady) { throw "module auth screenshot did not become ready" }
