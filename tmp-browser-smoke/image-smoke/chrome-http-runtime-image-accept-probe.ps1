...
$profileRoot = Join-Path $root "profile-http-runtime-accept"
$appDataRoot = Join-Path $profileRoot "lightpanda"
$browserExe = Resolve-LightpandaBrowserExe $repo $BrowserExe
$serverScript = Join-Path $root "http_runtime_server.py"
$outPng = Join-Path $root "http-runtime-accept.png"
$browserOut = Join-Path $root "http-runtime-accept.browser.stdout.txt"
$browserErr = Join-Path $root "http-runtime-accept.browser.stderr.txt"
$serverOut = Join-Path $root "http-runtime-accept.server.stdout.txt"
$serverErr = Join-Path $root "http-runtime-accept.server.stderr.txt"
$requestLog = Join-Path $root "http-runtime.requests.jsonl"
$pageUrl = "http://127.0.0.1:$Port/accept-page.html"
...
try {
  ...
  if (-not $ready) { throw "localhost image accept server did not become ready" }
  ...
  if (-not $pngReady) { throw "image accept screenshot did not become ready" }
  ...
  $imageEntries = @($requestEntries | Where-Object { $_.path -eq "/accept-red.png" })
  ...
  [ordered]@{
    ...
    image_accept = if ($lastImage) { [string]$lastImage.accept } else { "" }
