...
$pageUrl = "http://127.0.0.1:$Port/auth-page.html"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  if (-not $ready) { throw "localhost image auth server did not become ready" }
  ...
  if (-not $pngReady) { throw "image auth screenshot did not become ready" }
