...
$profileRoot = Join-Path $root "profile-http-runtime-auth-inherit"
$appDataRoot = Join-Path $profileRoot "lightpanda"
$browserExe = Resolve-LightpandaBrowserExe $repo $BrowserExe
$serverScript = Join-Path $root "http_runtime_server.py"
$outPng = Join-Path $root "http-runtime-auth-inherit.png"
$browserOut = Join-Path $root "http-runtime-auth-inherit.browser.stdout.txt"
$browserErr = Join-Path $root "http-runtime-auth-inherit.browser.stderr.txt"
$serverOut = Join-Path $root "http-runtime-auth-inherit.server.stdout.txt"
$serverErr = Join-Path $root "http-runtime-auth-inherit.server.stderr.txt"
$requestLog = Join-Path $root "http-runtime.requests.jsonl"
$pageUrl = "http://img%20user:p%40ss@127.0.0.1:$Port/auth-inherit-page.html"
$readyUrl = "http://127.0.0.1:$Port/auth-inherit-page.html"
...
try {
  ...
  if (-not $ready) { throw "localhost image inherited auth server did not become ready" }
  ...
  if (-not $pngReady) { throw "image inherited auth screenshot did not become ready" }
  ...
  $imageEntries = @($requestEntries | Where-Object { $_.path -eq "/auth-inherit-red.png" })
