...
try {
  ...
  $ready = Wait-LightpandaHttpReady -Url "$origin/anchor-index.html" -TimeoutSeconds $ServerReadyTimeoutSeconds -PollMilliseconds $PollMilliseconds
  ...
  $titles.initial = Wait-TabTitle -ProcessId $browser.Id -Needle "Popup Anchor Start" -Attempts 40 -PollMilliseconds $PollMilliseconds
