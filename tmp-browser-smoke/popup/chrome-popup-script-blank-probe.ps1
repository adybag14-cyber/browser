...
$config = Resolve-TabProbeConfig -StartPath $PSScriptRoot -RepoRoot $RepoRoot -BrowserExe $BrowserExe -ProfileName "profile-script-blank"
...
try {
  ...
  $titles.initial = Wait-TabTitle -ProcessId $browser.Id -Needle "Popup Script Blank Start" -Attempts 6 -PollMilliseconds $PollMilliseconds
  Start-Sleep -Milliseconds 900
  Send-SmokeCtrlDigit 2
  $titles.result = Wait-TabTitle -ProcessId $browser.Id -Needle "Popup Script Blank Result" -Attempts 24 -PollMilliseconds $PollMilliseconds
  $popupWorked = [bool]$titles.result
  if (-not $popupWorked) { throw "script blank popup did not open result tab" }

  Send-SmokeCtrlDigit 1
  $titles.returned = Wait-TabTitle -ProcessId $browser.Id -Needle "Popup Script Blank Start" -Attempts 12 -PollMilliseconds $PollMilliseconds
